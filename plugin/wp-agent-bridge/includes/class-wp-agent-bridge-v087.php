...
final class WP_Agent_Bridge_V087
{
    private const VERSION = '0.8.7';
    private const INTERNAL_NAMESPACE = 'wpab-v087/v1';
    private const INTERNAL_ROUTE = '/wpab-v087/v1/manage';
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v087_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v087_json', 'JSON body is required.', ['status' => 400]);
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v087_unknown_action', 'Unknown or blocked v0.8.7 action.', ['status' => 400, 'action' => $action]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'post.table.locate':
                    $response = WP_Agent_Bridge_V087_Table_Locator::locate($params);
                    break;
                case 'post.table.smart.patch.preview':
                    $response = WP_Agent_Bridge_V087_Table_Locator::smart_preview($params);
                    break;
                case 'post.table.smart.patch.apply':
                    $response = WP_Agent_Bridge_V087_Table_Locator::smart_apply($params);
                    WP_Agent_Bridge_V07_Audit::record(self::$request_id, $action, self::audit_params($params), $response);
                    break;
                default:
                    $response = new WP_Error('wpab_v087_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v087_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return $response;
    }
