...
final class WP_Agent_Bridge_V089_Secure
{
    private const VERSION = '0.8.9';
    private const PROTOCOL = 'rsa_aes_gcm_v1';
    private const MANAGE_NAMESPACE = 'wpab-v089/v1';
    private const MANAGE_ROUTE = '/wpab-v089/v1/manage';
    private const OUTER_ROUTE = '/wp-agent-bridge/v1/execute';
    private const SECURE_ROUTE = '/wp-agent-bridge/v1/secure';
    private const HEALTH_ROUTE = '/wp-agent-bridge/v1/health';
    private const OPTION_SECRET = 'wpab_secret';
    private const OPTION_USER_ID = 'wpab_user_id';
    private const OPTION_PRIVATE_KEY = 'wpab_secure_rsa_private_v1';
    private const OPTION_PUBLIC_KEY = 'wpab_secure_rsa_public_v1';
    private const OPTION_KEY_CREATED = 'wpab_secure_rsa_created_v1';
    ...
    public static function register_routes(): void
    {
        ...
        register_rest_route('wp-agent-bridge/v1', '/secure', [
            'methods' => WP_REST_Server::CREATABLE,
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v089_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function secure_permission(WP_REST_Request $request)
    {
        if (!self::crypto_available()) {
            return new WP_Error('wpab_secure_crypto_unavailable', 'Required OpenSSL RSA and AES-GCM functions are unavailable.', ['status' => 501]);
        }
        if (!self::valid_hmac($request, self::SECURE_ROUTE, true)) {
            return new WP_Error('wpab_secure_auth', 'Secure Bridge authentication failed.', ['status' => 401]);
        }
        return true;
    }
    ...
    public static function dispatch_manage(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v089_json', 'JSON body is required.', ['status' => 400]);
        $action = isset($json['action']) && is_string($json['action']) ? trim($json['action']) : '';
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v089_unknown_action', 'Unknown or blocked v0.8.9 action.', ['status' => 400, 'action' => $action]);
        }
        ...
        try {
            ...
            return new WP_Error('wpab_v089_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return new WP_Error('wpab_v089_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }
    ...
    public static function dispatch_secure(WP_REST_Request $request)
    {
        try {
            $json = $request->get_json_params();
            if (!is_array($json)) return new WP_Error('wpab_secure_json', 'JSON body is required.', ['status' => 400]);
            ...
            if ($protocol !== self::PROTOCOL) {
                return new WP_Error('wpab_secure_protocol', 'Unsupported secure transport protocol.', ['status' => 400]);
            }
            if ($request_id === '' || strlen($request_id) > 128 || !preg_match('/^[A-Za-z0-9._:-]+$/', $request_id)) {
                return new WP_Error('wpab_secure_request_id', 'request_id is invalid.', ['status' => 400]);
            }

            $keys = self::ensure_server_keys();
            if (is_wp_error($keys)) return $keys;
            if ($server_key_id === '' || !hash_equals($keys['key_id'], $server_key_id)) {
                return new WP_Error('wpab_secure_server_key', 'server_key_id does not match the active secure transport key.', [
                    'status' => 409,
            ...
            if ($reply_public === false) {
                return new WP_Error('wpab_secure_reply_key', 'reply_public_key_pem_b64 is not a valid public key.', ['status' => 400]);
            }
            ...
            if (!is_array($reply_details) || ($reply_details['type'] ?? null) !== OPENSSL_KEYTYPE_RSA || (int) ($reply_details['bits'] ?? 0) < 2048) {
                return new WP_Error('wpab_secure_reply_key', 'Reply key must be an RSA public key of at least 2048 bits.', ['status' => 400]);
            }
            ...
            if (strlen($iv) !== 12 || strlen($tag) !== 16) {
                return new WP_Error('wpab_secure_gcm_fields', 'AES-GCM IV and tag lengths are invalid.', ['status' => 400]);
            }

            $private = openssl_pkey_get_private($keys['private_pem']);
            if ($private === false) {
                return new WP_Error('wpab_secure_private_key', 'Stored secure transport private key is invalid.', ['status' => 500]);
            }
            $aes_key = '';
            if (!openssl_private_decrypt($wrapped_key, $aes_key, $private, OPENSSL_PKCS1_OAEP_PADDING) || strlen($aes_key) !== 32) {
                return new WP_Error('wpab_secure_unwrap', 'Could not unwrap the request encryption key.', ['status' => 400]);
            }
            ...
            if (!is_string($plaintext)) {
                return new WP_Error('wpab_secure_decrypt', 'Request ciphertext authentication or decryption failed.', ['status' => 400]);
            }
            if (strlen($plaintext) > self::MAX_REQUEST_PLAINTEXT_BYTES) {
                self::memzero($plaintext);
                return new WP_Error('wpab_secure_request_size', 'Decrypted request exceeds the secure transport size limit.', ['status' => 413]);
            }

            $inner = json_decode($plaintext, true);
            self::memzero($plaintext);
            if (!is_array($inner)) {
                return new WP_Error('wpab_secure_plaintext_json', 'Decrypted request is not valid JSON.', ['status' => 400]);
            }
            if (($inner['request_id'] ?? null) !== $request_id || ($inner['server_key_id'] ?? null) !== $server_key_id || ($inner['reply_public_key_sha256'] ?? null) !== $reply_key_sha) {
                return new WP_Error('wpab_secure_binding', 'Encrypted request metadata does not match the outer secure envelope.', ['status' => 400]);
            }
            $command = isset($inner['command']) && is_array($inner['command']) ? $inner['command'] : null;
            if (!is_array($command)) {
                return new WP_Error('wpab_secure_command', 'Encrypted request is missing command.', ['status' => 400]);
            }
            ...
            if (!is_string($response_plain)) {
                return new WP_Error('wpab_secure_response_json', 'Could not encode encrypted response.', ['status' => 500]);
            }
            if (strlen($response_plain) > self::MAX_RESPONSE_PLAINTEXT_BYTES) {
                $response_plain = wp_json_encode([
                    ...
                    'result' => [
                        ...
                        'headers' => [],
                        'data' => [
                            'code' => 'wpab_secure_response_size',
            ...
            if (!is_string($response_cipher) || strlen($response_tag) !== 16) {
                ...
                return new WP_Error('wpab_secure_response_encrypt', 'Could not encrypt secure response.', ['status' => 500]);
            }
            $response_wrapped_key = '';
            if (!openssl_public_encrypt($response_key, $response_wrapped_key, $reply_public, OPENSSL_PKCS1_OAEP_PADDING)) {
                self::memzero($response_key);
                return new WP_Error('wpab_secure_response_wrap', 'Could not wrap secure response key.', ['status' => 500]);
            }
            self::memzero($response_key);
            ...
            return new WP_Error('wpab_secure_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
    }
    ...
    private static function execute_rest_command(array $command, string $request_id): array
    {
        ...
        if (!in_array($method, ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'], true)) {
            return ['status' => 400, 'headers' => [], 'data' => ['code' => 'wpab_secure_method', 'message' => 'Unsupported REST method.']];
        }
        ...
        if ($route === '' || $route[0] !== '/' || strpos($route, '://') !== false || $route === self::SECURE_ROUTE) {
            return ['status' => 400, 'headers' => [], 'data' => ['code' => 'wpab_secure_route', 'message' => 'Invalid or recursive REST route.']];
        }
    ...
    private static function execute_inner(array $inner): array
    {
        ...
        if ($secret === '') {
            return ['status' => 500, 'headers' => [], 'data' => ['code' => 'wpab_secure_secret', 'message' => 'Bridge secret is unavailable.']];
        }
        $payload_json = wp_json_encode($inner, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        if (!is_string($payload_json)) {
            return ['status' => 500, 'headers' => [], 'data' => ['code' => 'wpab_secure_inner_json', 'message' => 'Could not encode internal Bridge request.']];
        }
        $envelope = [
            'action' => 'envelope',
        ...
        if (!is_string($body)) {
            return ['status' => 500, 'headers' => [], 'data' => ['code' => 'wpab_secure_outer_json', 'message' => 'Could not encode internal Bridge envelope.']];
        }
    ...
    private static function ensure_server_keys()
    {
        ...
        if (($private_pem === '') !== ($public_pem === '')) {
            return new WP_Error('wpab_secure_key_state', 'Secure transport key options are incomplete; refusing silent regeneration.', ['status' => 500]);
        }
        ...
        if ($private_pem === '') {
            ...
            if ($resource === false) {
                return new WP_Error('wpab_secure_keygen', 'Could not generate RSA secure transport key.', ['status' => 500]);
            }
            $exported = openssl_pkey_export($resource, $generated_private);
            $details = openssl_pkey_get_details($resource);
            if (!$exported || !is_array($details) || !isset($details['key'])) {
                return new WP_Error('wpab_secure_keygen', 'Could not export RSA secure transport key.', ['status' => 500]);
            }
        ...
        if (!is_array($details) || ($details['type'] ?? null) !== OPENSSL_KEYTYPE_RSA || !isset($details['key'])) {
            return new WP_Error('wpab_secure_private_key', 'Stored secure transport key is not RSA.', ['status' => 500]);
        }
        $derived_public = (string) $details['key'];
        if (trim($derived_public) !== trim($public_pem)) {
            return new WP_Error('wpab_secure_key_mismatch', 'Stored secure transport public/private keys do not match.', ['status' => 500]);
        }
    ...
    private static function decode_b64_field(array $json, string $field, int $max_bytes)
    {
        ...
        if ($value === '') return new WP_Error('wpab_secure_field', $field . ' is required.', ['status' => 400]);
        $decoded = base64_decode($value, true);
        if (!is_string($decoded)) return new WP_Error('wpab_secure_base64', $field . ' is not valid Base64.', ['status' => 400]);
        if (strlen($decoded) > $max_bytes) return new WP_Error('wpab_secure_field_size', $field . ' exceeds its size limit.', ['status' => 413]);
        return $decoded;
    }
