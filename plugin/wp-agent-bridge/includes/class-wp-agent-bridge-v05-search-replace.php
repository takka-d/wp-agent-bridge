...
final class WP_Agent_Bridge_V05_Search_Replace
{
    ...
    public static function execute(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Search/replace execution requires confirm=true.', ['status' => 400]);
        }
        ...
        if (!preg_match('/^[a-f0-9]{64}$/', $expected_hash)) {
            return new WP_Error('wpab_sr_plan_hash_required', 'A valid plan_hash from db.search_replace.plan is required.', ['status' => 400]);
        }
        ...
        if (!hash_equals($expected_hash, (string) $plan['plan_hash'])) {
            return new WP_Error('wpab_sr_plan_changed', 'Database contents or search/replace parameters changed since the plan was created.', [
                'status' => 409,
        ...
        try {
            ...
            $wpdb->query('ROLLBACK');
            return new WP_Error('wpab_sr_execute_failed', $e->getMessage(), ['status' => 500]);
        }
    ...
    private static function build_search_replace_plan(array $params)
    {
        ...
        if ($search === '') {
            return new WP_Error('wpab_sr_empty_search', 'search must not be empty.', ['status' => 400]);
        }
        if (!array_key_exists('replace', $params) || !is_string($params['replace'])) {
            return new WP_Error('wpab_sr_replace', 'replace must be a string.', ['status' => 400]);
        }
        ...
        foreach ($targets as $target) {
            ...
            if (!is_array($rows)) {
                return new WP_Error('wpab_sr_query_failed', $wpdb->last_error ?: 'Search/replace plan query failed.', ['status' => 500]);
            }
            if (count($rows) > self::MAX_SR_ROWS) {
                return new WP_Error('wpab_sr_plan_too_large', 'A target matched more rows than the per-plan limit; narrow the targets or where filter.', [
                    'status' => 413,
                    'table' => $table,
        ...
        if ($matched_rows > self::MAX_SR_ROWS || $replacement_count > self::MAX_SR_REPLACEMENTS) {
            return new WP_Error('wpab_sr_plan_too_large', 'Search/replace plan is too large; narrow the targets or where filter.', [
                'status' => 413,
        ...
        if ($unsupported_serialized_objects > 0) {
            return new WP_Error('wpab_sr_unsupported_serialized_object', 'Plan encountered serialized objects that cannot be safely rewritten.', [
                'status' => 409,
    ...
    private static function normalize_sr_targets(array $params)
    {
        ...
        foreach ($input as $item) {
            if (!is_array($item) || !isset($item['table']) || !is_string($item['table'])) {
                return new WP_Error('wpab_sr_target', 'Each target requires a table.', ['status' => 400]);
            }
            $table = str_replace('{prefix}', $wpdb->prefix, trim($item['table']));
            if (!isset($allowed[$table])) {
                return new WP_Error('wpab_sr_table_blocked', 'Table is not in the search/replace allowlist.', [
                    'status' => 403,
                    'table' => $table,
                ]);
            }
            if ($table === $wpdb->users || $table === $wpdb->usermeta) {
                return new WP_Error('wpab_sr_user_table_blocked', 'User and usermeta tables are blocked.', ['status' => 403]);
            }
            ...
            if (!$columns || array_diff($columns, $allowed_columns)) {
                return new WP_Error('wpab_sr_column_blocked', 'One or more columns are not in the search/replace allowlist.', [
                    'status' => 403,
                    'table' => $table,
            ...
            if (isset($item['where']) && is_array($item['where'])) {
                ...
                foreach ($item['where'] as $column => $value) {
                    if (!is_string($column) || !in_array($column, $schema_columns, true) || !is_scalar($value)) {
                        return new WP_Error('wpab_sr_where', 'Invalid where filter.', ['status' => 400]);
                    }
                    if (in_array($column, ['user_login', 'user_pass', 'user_email'], true)) {
                        return new WP_Error('wpab_sr_where_sensitive', 'Sensitive user columns are blocked.', ['status' => 403]);
                    }
                    $where[$column] = $value;
                }
            }
