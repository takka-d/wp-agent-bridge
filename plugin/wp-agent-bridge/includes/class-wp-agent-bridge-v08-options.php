...
final class WP_Agent_Bridge_V08_Options
{
    ...
    public static function pluck(array $params)
    {
        ...
        if (!$result['found']) return new WP_Error('wpab_option_path_not_found', 'Option path was not found.', ['status' => 404, 'name' => $name, 'path' => $path]);
    ...
    {
        if (empty($params['confirm'])) return new WP_Error('wpab_confirmation_required', 'Option patch requires confirm=true.', ['status' => 400]);
        $plan = self::build_plan($params);
        if (is_wp_error($plan)) return $plan;
        ...
        if ($expected === '' || !hash_equals((string) $plan['plan_hash'], $expected)) {
            return new WP_Error('wpab_option_plan_changed', 'Option patch plan changed or expected_plan_hash is missing.', ['status' => 409, 'current_plan_hash' => $plan['plan_hash']]);
        }
        ...
        if (!hash_equals((string) $plan['after_sha256'], $after_hash)) {
            return new WP_Error('wpab_option_patch_verify_failed', 'Option value did not match the planned value after write.', ['status' => 500]);
        }
        $response = $plan;
    ...
    private static function build_plan(array $params)
    {
        ...
        if (!self::is_patchable_name($name)) {
            return new WP_Error('wpab_option_patch_protected', 'This option is protected from Bridge patching.', ['status' => 403, 'name' => $name]);
        }
        if (!self::exists($name)) return self::not_found($name);
        $path = self::path($params);
        if (is_wp_error($path)) return $path;
        foreach ($path as $segment) {
            if (is_string($segment) && self::is_sensitive_key($segment)) {
                return new WP_Error('wpab_option_sensitive_path', 'Credential/session-like option paths cannot be accessed.', ['status' => 403]);
            }
        }
        ...
        if (!in_array($operation, ['insert', 'update', 'delete'], true)) {
            return new WP_Error('wpab_option_patch_operation', 'operation must be insert, update, or delete.', ['status' => 400]);
        }
        $before = get_option($name);
        if (!is_array($before)) {
            return new WP_Error('wpab_option_patch_array_only', 'Surgical option patching currently supports array-valued options only.', ['status' => 409, 'stored_type' => gettype($before)]);
        }
        if (strlen(maybe_serialize($before)) > self::MAX_VALUE_BYTES) {
            return new WP_Error('wpab_option_value_too_large', 'Option is too large for surgical patching.', ['status' => 413]);
        }
        $current = self::path_get($before, $path);
        if ($operation === 'insert' && $current['found']) return new WP_Error('wpab_option_path_exists', 'Insert target already exists.', ['status' => 409]);
        if (in_array($operation, ['update', 'delete'], true) && !$current['found']) return new WP_Error('wpab_option_path_not_found', 'Patch target does not exist.', ['status' => 404]);
        if ($operation !== 'delete' && !array_key_exists('value', $params)) return new WP_Error('wpab_option_patch_value_required', 'value is required for insert/update.', ['status' => 400]);
        if ($operation !== 'delete' && self::contains_sensitive_nested_key($params['value'])) {
            return new WP_Error('wpab_option_nested_sensitive_write', 'Option values containing credential/session-like nested keys cannot be written through the GitHub-backed Bridge.', ['status' => 400]);
        }
        ...
        if (!$ok) return new WP_Error('wpab_option_patch_failed', 'Could not construct option patch.', ['status' => 409]);
        if (strlen(maybe_serialize($after)) > self::MAX_VALUE_BYTES) return new WP_Error('wpab_option_value_too_large', 'Patched option would exceed the Bridge size limit.', ['status' => 413]);
    ...
    {
        if (!isset($params['name']) || !is_string($params['name'])) return new WP_Error('wpab_option_name_required', 'name is required.', ['status' => 400]);
        $name = trim($params['name']);
        if ($name === '' || strlen($name) > 191 || strpos($name, "\0") !== false) return new WP_Error('wpab_option_name_invalid', 'Invalid option name.', ['status' => 400]);
        return $name;
    }
    ...
    private static function read_guard(string $name)
    {
        ...
        return new WP_Error('wpab_option_sensitive', 'Sensitive credential/security option values cannot be read through the Bridge.', ['status' => 403, 'name' => $name]);
    }

    private static function is_patchable_name(string $name): bool
    {
        if (self::is_sensitive_name($name)) return false;
        if (strpos($name, 'wpab_') === 0) return false;
    ...
    {
        if (strpos($name, 'wpab_') === 0) return true;
    ...
    {
        if (!array_key_exists('path', $params)) return new WP_Error('wpab_option_path_required', 'path is required.', ['status' => 400]);
        $path = $params['path'];
        if (is_string($path)) {
            if ($path === '') return new WP_Error('wpab_option_path_empty', 'path must not be empty.', ['status' => 400]);
            $path = explode('.', $path);
        }
        if (!is_array($path) || !$path || count($path) > self::MAX_PATH_DEPTH) return new WP_Error('wpab_option_path_invalid', 'path must be a non-empty array or dot path within the depth limit.', ['status' => 400]);
        $out = [];
        foreach ($path as $segment) {
            if (is_int($segment)) {
                $out[] = $segment;
            ...
            if (!is_string($segment) || $segment === '' || strlen($segment) > 191 || strpos($segment, "\0") !== false) return new WP_Error('wpab_option_path_segment', 'Invalid option path segment.', ['status' => 400]);
    ...
    {
        return new WP_Error('wpab_option_not_found', 'Option was not found.', ['status' => 404, 'name' => $name]);
    }
