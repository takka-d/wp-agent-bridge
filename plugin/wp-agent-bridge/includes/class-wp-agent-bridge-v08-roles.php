...
final class WP_Agent_Bridge_V08_Roles
{
    ...
    public static function role_create(array $params)
    {
        ...
        if (is_wp_error($role)) return $role;
        if (get_role($role)) return new WP_Error('wpab_role_exists', 'Role already exists.', ['status' => 409]);
        $name = isset($params['name']) && is_string($params['name']) ? trim($params['name']) : '';
        if ($name === '') return new WP_Error('wpab_role_name_required', 'name is required.', ['status' => 400]);

        $caps = [];
        if (isset($params['clone']) && is_string($params['clone']) && trim($params['clone']) !== '') {
            $clone = sanitize_key($params['clone']);
            $source = get_role($clone);
            if (!$source) return new WP_Error('wpab_role_clone_not_found', 'Clone source role was not found.', ['status' => 404]);
        ...
        if (!$created) return new WP_Error('wpab_role_create_failed', 'WordPress did not create the role.', ['status' => 500]);
    ...
    public static function role_delete_preview(array $params)
    {
        $role = self::resolve_role($params);
        if (is_wp_error($role)) return $role;
        if ($role === 'administrator') return new WP_Error('wpab_administrator_role_protected', 'The administrator role cannot be deleted.', ['status' => 409]);
    ...
    public static function role_delete(array $params)
    {
        ...
        if ((int) $data['assigned_user_count'] > 0) {
            return new WP_Error('wpab_role_in_use', 'Role deletion is blocked while users are assigned to it.', ['status' => 409, 'assigned_user_count' => (int) $data['assigned_user_count']]);
        }
        ...
        if ($expected === '' || !hash_equals((string) $data['impact_hash'], $expected)) {
            return new WP_Error('wpab_role_impact_changed', 'Role deletion impact changed or expected_impact_hash is missing.', ['status' => 409]);
        }
        remove_role((string) $data['role']);
        if (get_role((string) $data['role'])) return new WP_Error('wpab_role_delete_failed', 'Role still exists after deletion.', ['status' => 500]);
    ...
    public static function role_cap_remove(array $params)
    {
        ...
        if ($role === 'administrator' && self::is_critical($cap)) {
            return new WP_Error('wpab_administrator_capability_protected', 'Core administrator capabilities cannot be removed through the Bridge.', ['status' => 409, 'capability' => $cap]);
        }
        $role_obj = get_role($role);
    ...
    {
        $user = WP_Agent_Bridge_V07_Users::resolve($params);
    ...
    public static function user_cap_remove(array $params)
    {
        ...
        $cap = self::capability($params);
        if (is_wp_error($cap)) return $cap;
        if ((int) $user->ID === (int) get_option('wpab_user_id', 0) && self::is_critical($cap)) {
            return new WP_Error('wpab_connected_admin_capability_protected', 'Critical capabilities cannot be removed from the connected Bridge administrator.', ['status' => 409, 'capability' => $cap]);
        }
        $user->remove_cap($cap);
    ...
    private static function resolve_role(array $params)
    {
        ...
        if ($role === '' || !get_role($role)) return new WP_Error('wpab_role_not_found', 'Role was not found.', ['status' => 404, 'role' => $role]);
        return $role;
    }

    private static function new_role_slug(array $params)
    {
        if (!isset($params['role']) || !is_string($params['role'])) return new WP_Error('wpab_role_required', 'role is required.', ['status' => 400]);
        $raw = trim($params['role']);
        $role = sanitize_key($raw);
        if ($role === '' || $role !== $raw || strlen($role) > 64) return new WP_Error('wpab_role_invalid', 'role must already be a safe lowercase WordPress role slug.', ['status' => 400]);
        return $role;
    }

    private static function capability(array $params)
    {
        if (!isset($params['capability']) || !is_string($params['capability'])) return new WP_Error('wpab_capability_required', 'capability is required.', ['status' => 400]);
        $raw = trim($params['capability']);
        $cap = sanitize_key($raw);
        if ($cap === '' || $cap !== $raw || strlen($cap) > 100) return new WP_Error('wpab_capability_invalid', 'capability must already be a safe WordPress capability key.', ['status' => 400]);
        return $cap;
    }
    ...
    {
        return new WP_Error('wpab_sensitive_confirmation_required', $message, ['status' => 400]);
    }
}
