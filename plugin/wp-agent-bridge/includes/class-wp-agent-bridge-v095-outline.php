...
final class WP_Agent_Bridge_V095_Outline
{
    ...
    public static function read_range(array $params)
    {
        ...
        if (!is_array($lines)) {
            return new WP_Error('wpab_v095_lines', 'Could not split the file into lines.', ['status' => 500]);
        }
        $total = count($lines);
        $start = isset($params['start_line']) ? max(1, (int) $params['start_line']) : 1;
        if ($start > max(1, $total)) {
            return new WP_Error('wpab_v095_range', 'start_line is beyond the end of the file.', [
                'status' => 416,
                'total_lines' => $total,
            ]);
        }
    ...
    private static function extract(string $content, string $extension, int $limit)
    {
        ...
        if ($extension === 'json') {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                return new WP_Error('wpab_v095_json_parse', 'JSON file could not be parsed.', [
                    'status' => 400,
    ...
    private static function resolve(array $params)
    {
        ...
        if ($path === '') {
            return new WP_Error('wpab_v095_path', 'path is required.', ['status' => 400]);
        }
        ...
        if ($scope === 'active') {
            ...
            if ($draft_id === '' || !is_array($drafts) || !isset($drafts[$draft_id])) {
                return new WP_Error('wpab_v095_draft', 'Theme draft was not found.', ['status' => 404]);
            }
            $slug = basename((string) ($drafts[$draft_id]['slug'] ?? ''));
            $root = $slug !== '' ? realpath(rtrim(get_theme_root(), '/\\') . DIRECTORY_SEPARATOR . $slug) : false;
        } else {
            return new WP_Error('wpab_v095_scope', 'scope must be active or draft.', ['status' => 400]);
        }
        if ($root === false) {
            return new WP_Error('wpab_v095_theme_root', 'Theme directory was not found.', ['status' => 404]);
        }
        $relative = str_replace('\\', '/', $path);
        if ($relative === '' || $relative[0] === '/' || strpos($relative, "\0") !== false || preg_match('~(^|/)\.\.(/|$)~', $relative)) {
            return new WP_Error('wpab_v095_path_escape', 'Unsafe theme path.', ['status' => 403]);
        }
        $extension = strtolower(pathinfo($relative, PATHINFO_EXTENSION));
        if (!in_array($extension, self::EXTENSIONS, true)) {
            return new WP_Error('wpab_v095_file_type', 'File type is not supported by structural inspection.', ['status' => 403]);
        }
        $file = realpath($root . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $relative));
        if ($file === false || !is_file($file)) {
            return new WP_Error('wpab_v095_file_missing', 'Theme file was not found.', ['status' => 404]);
        }
        $prefix = rtrim(str_replace('\\', '/', $root), '/') . '/';
        if (strpos(str_replace('\\', '/', $file), $prefix) !== 0) {
            return new WP_Error('wpab_v095_path_escape', 'Path escapes the theme directory.', ['status' => 403]);
        }
    ...
    private static function content(string $file)
    {
        ...
        if ($size === false || $size > self::MAX_FILE_BYTES) {
            return new WP_Error('wpab_v095_file_size', 'Theme file exceeds inspection size limit.', [
                'status' => 413,
                'max_bytes' => self::MAX_FILE_BYTES,
            ]);
        }
        $content = file_get_contents($file);
        return is_string($content) ? $content : new WP_Error('wpab_v095_file_read', 'Could not read theme file.', ['status' => 500]);
    }
