 ...
 */
final class TakKa_WordPress_Bridge_Direct_Pending_Reconciler
{
    private const CRON_HOOK = 'takka_bridge_direct_reconcile_cron_v4';
    private const LAST_OPTION = 'takka_bridge_direct_reconcile_detail_v5';
    ...
    private static function reconcile_path(
        ...
        $existing = TakKa_WordPress_Bridge_Direct_GitHub::get_text_file($token, $repository, $branch, $result_path);
        if (!is_wp_error($existing)) {
            return self::summary($path, $id, true, ['reason' => 'result-already-visible', 'recovered' => true]);
        }
        $existing_status = TakKa_WordPress_Bridge_Direct_GitHub_Recovery::error_status($existing);
        ...
        if ($age === null) {
            $last_modified = TakKa_WordPress_Bridge_Direct_GitHub_Recovery::path_last_modified_timestamp($token, $repository, $branch, $path);
        ...
        $after = TakKa_WordPress_Bridge_Direct_GitHub_Recovery::branch_sha($token, $repository, $branch);
        ...
        $executed = TakKa_WordPress_Bridge_Direct_Runtime::webhook($request);
        ...
        $result_after = TakKa_WordPress_Bridge_Direct_GitHub::get_text_file($token, $repository, $branch, $result_path);
        $pending_after = TakKa_WordPress_Bridge_Direct_GitHub::get_content_metadata($token, $repository, $branch, $path);
        $result_visible = !is_wp_error($result_after);
        $pending_visible = !is_wp_error($pending_after);
        $pending_status = is_wp_error($pending_after) ? TakKa_WordPress_Bridge_Direct_GitHub_Recovery::error_status($pending_after) : 200;
