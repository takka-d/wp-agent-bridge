 ...
 */
final class WP_Agent_Bridge_Direct_Hardening
{
    private const PRIMARY_LOCK_OPTION = 'wpab_direct_primary_lock_v3';
    ...
    private const RECONCILE_CRON_SCHEDULE = 'wpab_direct_every_two_minutes';
    private const RECONCILE_CRON_INTERVAL = 120;
    private const RECONCILE_LAST_OPTION = 'wpab_direct_reconcile_last_v4';

    public static function init(): void
    {
        remove_action('admin_post_wpab_generate_key', [WP_Agent_Bridge::class, 'handle_generate_key']);
    ...
    {
        remove_submenu_page('tools.php', 'wp-agent-bridge');
    }
    ...
    public static function ensure_runtime_identity(): void
    {
        ...
        WP_Agent_Bridge_Direct_Runtime_Identity::sync();
    }
    ...
    public static function serialized_webhook(WP_REST_Request $request)
    {
        ...
        if ($event !== 'push') {
            return WP_Agent_Bridge_Direct_Runtime_V2::webhook($request);
        }
    ...
    public static function scheduled_reconcile(): void
    {
        ...
        $repo = WP_Agent_Bridge_Direct_GitHub::github_api(
            'GET',
            '/repos/' . $repository,
            $token
        );
