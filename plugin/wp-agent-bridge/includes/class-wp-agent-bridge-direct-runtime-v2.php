 ...
 */
final class WP_Agent_Bridge_Direct_Runtime_V2
{
    private const LOCK_OPTION = 'wpab_direct_reconcile_lock_v2';
    ...
    {
        register_rest_route(
            WP_Agent_Bridge_Direct_Runtime::NAMESPACE,
            '/github-webhook',
            [
    ...
    public static function webhook(WP_REST_Request $request)
    {
        ...
        $primary = WP_Agent_Bridge_Direct_Runtime::webhook($request);
        ...
        if (!is_array($payload)
            || ($payload['ref'] ?? '') !== 'refs/heads/' . WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH
        ...
        if (is_wp_error($recovery)) {
            $data['pending_recovery'] = [
                ...
                'status' => WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($recovery),
            ];
        } else {
    ...
    private static function reconcile_pending(array $payload)
    {
        ...
        try {
            ...
            $entries = WP_Agent_Bridge_Direct_GitHub_Recovery::list_directory(
                $token,
                $repository,
                WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
    ...
    private static function recover_path(
        ...
        $raw = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $path
        ...
        $existing_result = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $result_path
        ...
        if (WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($existing_result) !== 404) {
        ...
        if ($age === null) {
            $last_modified = WP_Agent_Bridge_Direct_GitHub_Recovery::path_last_modified_timestamp(
                $token,
                $repository,
                WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
                $path
        ...
        $after = WP_Agent_Bridge_Direct_GitHub_Recovery::branch_sha(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH
        );
        ...
        $synthetic_payload = [
            'ref' => 'refs/heads/' . WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            'deleted' => false,
            'after' => $after,
            'repository' => [
                'id' => $repository_id,
        ...
        $synthetic = new WP_REST_Request('POST', WP_Agent_Bridge_Direct_Runtime::WEBHOOK_ROUTE);
        ...
        $executed = WP_Agent_Bridge_Direct_Runtime::webhook($synthetic);
        ...
        $result_after = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $result_path
        );
        $pending_after = WP_Agent_Bridge_Direct_GitHub::get_content_metadata(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $path
        ...
        if (is_wp_error($pending_after)
            && WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($pending_after) === 404
    ...
    private static function repair_bookkeeping(
        ...
        $completed = WP_Agent_Bridge_Direct_GitHub_Recovery::put_if_absent_or_identical(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $completed_path,
        ...
        $deleted = WP_Agent_Bridge_Direct_GitHub_Recovery::delete_if_matches(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $pending_path,
    ...
    private static function quarantine_expired(
        ...
        $archived = WP_Agent_Bridge_Direct_GitHub_Recovery::put_if_absent_or_identical(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $expired_path,
        ...
        $written = WP_Agent_Bridge_Direct_GitHub_Recovery::put_if_absent_or_identical(
            $token,
            $repository,
            WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH,
            $result_path,
    ...
    private static function is_expired_result(string $result_json): bool
    {
        ...
        return is_array($result)
            && (($result['recovery']['code'] ?? '') === 'wpab_pending_expired');
    }
