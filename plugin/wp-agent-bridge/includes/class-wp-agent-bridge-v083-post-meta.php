...
final class WP_Agent_Bridge_V083_Post_Meta
{
    ...
    public static function get(array $params)
    {
        ...
        if (!metadata_exists('post', $context['post_id'], $key)) {
            return new WP_Error('wpab_post_meta_not_found', 'Post meta was not found.', ['status' => 404, 'key' => $key]);
        }
    ...
    private static function write(string $mode, array $params)
    {
        ...
        if (!array_key_exists('value', $params)) {
            return new WP_Error('wpab_post_meta_value_required', 'value is required.', ['status' => 400]);
        }
        if (self::contains_sensitive_nested_key($params['value'])) {
            return new WP_Error(
                'wpab_post_meta_nested_sensitive_write',
        ...
        if (!is_string($serialized) || strlen($serialized) > self::MAX_VALUE_BYTES) {
            return new WP_Error('wpab_post_meta_value_size', 'Post meta value is too large.', ['status' => 413]);
        }
        ...
        if ($result === false) {
            return new WP_Error('wpab_post_meta_write_failed', 'Post meta write failed or made no change.', ['status' => 409]);
        }
    ...
    private static function context(array $params)
    {
        ...
        if (!current_user_can('edit_post', $post_id)) {
            return new WP_Error('wpab_post_meta_forbidden', 'Connected user cannot read or change meta for this post.', ['status' => 403, 'post_id' => $post_id]);
        }
    ...
    private static function key(array $params)
    {
        if (!isset($params['key']) || !is_string($params['key'])) {
            return new WP_Error('wpab_post_meta_key_required', 'key is required.', ['status' => 400]);
        }
        $key = trim($params['key']);
        if ($key === '' || strlen($key) > 191 || strpos($key, "\0") !== false) {
            return new WP_Error('wpab_post_meta_key_invalid', 'Invalid post meta key.', ['status' => 400]);
        }
        return $key;
    }
    ...
    private static function protected_write_guard(string $key, array $params)
    {
        if (is_protected_meta($key, 'post') && empty($params['confirm_sensitive'])) {
            return new WP_Error(
                'wpab_post_meta_protected_confirmation_required',
    ...
    {
        return new WP_Error('wpab_confirmation_required', 'Post meta changes require confirm=true.', ['status' => 400]);
    }

    private static function sensitive_error(string $verb): WP_Error
    {
        return new WP_Error(
            'wpab_post_meta_sensitive',
