...
final class WP_Agent_Bridge_V07_Settings
{
    ...
    public static function theme_mod_get(array $params)
    {
        ...
        if ($value === $sentinel) return new WP_Error('wpab_theme_mod_not_found', 'Theme mod was not found.', ['status' => 404]);
    ...
    public static function theme_mod_set(array $params)
    {
        ...
        $sensitive = self::reject_sensitive_key($key);
        if (is_wp_error($sensitive)) return $sensitive;
        if ($key === 'nav_menu_locations') return new WP_Error('wpab_theme_mod_menu_locations_blocked', 'Use menu.locations.set for nav_menu_locations.', ['status' => 400]);
        if (!array_key_exists('value', $params)) return new WP_Error('wpab_theme_mod_value_required', 'value is required.', ['status' => 400]);
        if (self::contains_sensitive_nested_key($params['value'])) {
            return new WP_Error(
                'wpab_theme_mod_nested_sensitive_write',
        ...
        if (!is_string($serialized) || strlen($serialized) > 131072) return new WP_Error('wpab_theme_mod_value_size', 'Theme mod value is too large.', ['status' => 413]);
    ...
    public static function rewrite_set(array $params)
    {
        ...
        if (array_key_exists('structure', $params)) {
            if (!is_string($params['structure'])) return new WP_Error('wpab_rewrite_structure_type', 'structure must be a string.', ['status' => 400]);
        ...
        foreach (['category_base', 'tag_base'] as $key) {
            ...
            if (!is_string($params[$key])) return new WP_Error('wpab_rewrite_base_type', "{$key} must be a string.", ['status' => 400]);
            $value = trim($params[$key], " /\t\n\r\0\x0B");
            if ($value !== '' && !preg_match('/^[A-Za-z0-9._~%+\/-]+$/', $value)) return new WP_Error('wpab_rewrite_base_invalid', "{$key} contains unsupported characters.", ['status' => 400]);
            update_option($key, $value);
            $changed = true;
        }
        if (!$changed) return new WP_Error('wpab_rewrite_no_changes', 'No permalink fields were provided.', ['status' => 400]);
    ...
    {
        if (!isset($params['key']) || !is_string($params['key'])) return new WP_Error('wpab_theme_mod_key_required', 'key is required.', ['status' => 400]);
        $key = trim($params['key']);
        if ($key === '' || strlen($key) > 191 || strpos($key, "\0") !== false) return new WP_Error('wpab_theme_mod_key_invalid', 'Invalid theme mod key.', ['status' => 400]);
        return $key;
    }
    ...
    private static function validate_structure(string $structure)
    {
        ...
        if ($structure[0] !== '/' || substr($structure, -1) !== '/') return new WP_Error('wpab_rewrite_structure_shape', 'Non-empty permalink structure must begin and end with /.', ['status' => 400]);
        if (preg_match('/[<>"\'`\\]/', $structure)) return new WP_Error('wpab_rewrite_structure_chars', 'Permalink structure contains blocked characters.', ['status' => 400]);
        preg_match_all('/%([^%]+)%/', $structure, $matches);
        $allowed = ['year', 'monthnum', 'day', 'hour', 'minute', 'second', 'post_id', 'postname', 'category', 'author'];
        foreach ($matches[1] as $token) if (!in_array($token, $allowed, true)) return new WP_Error('wpab_rewrite_structure_token', 'Unsupported rewrite token.', ['status' => 400, 'token' => $token]);
        return true;
    }
    ...
    private static function confirm_error(string $message): WP_Error { return new WP_Error('wpab_confirmation_required', $message, ['status' => 400]); }
}
