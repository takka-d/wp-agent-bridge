...
final class WP_Agent_Bridge_V086
{
    private const VERSION = '0.8.6.1';
    private const INTERNAL_NAMESPACE = 'wpab-v086/v1';
    private const INTERNAL_ROUTE = '/wpab-v086/v1/manage';
    ...
    private const ACTIONS = [
        'v086.capabilities',
        'post.table.inspect',
        'post.table.patch.preview',
        'post.table.patch.apply',
    ];

    public static function init(): void
    {
        add_action('rest_api_init', [self::class, 'register_route']);
        add_filter('rest_pre_dispatch', [self::class, 'prepare_internal_call'], 49, 3);
        add_filter('rest_request_after_callbacks', [self::class, 'annotate_health'], 350, 3);
    }
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v086_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v086_json', 'JSON body is required.', ['status' => 400]);
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v086_unknown_action', 'Unknown or blocked v0.8.6 action.', ['status' => 400, 'action' => $action]);
        }

        try {
            switch ($action) {
                case 'v086.capabilities':
                    $response = rest_ensure_response(self::capabilities());
                    break;
                case 'post.table.inspect':
                    $response = WP_Agent_Bridge_V086_Table::inspect($params);
                    break;
                case 'post.table.patch.preview':
                    $response = WP_Agent_Bridge_V086_Table::preview($params);
                    break;
                case 'post.table.patch.apply':
                    $response = WP_Agent_Bridge_V086_Table::apply($params);
                    WP_Agent_Bridge_V07_Audit::record(self::$request_id, $action, self::audit_params($params), $response);
                    break;
                default:
                    $response = new WP_Error('wpab_v086_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v086_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return $response;
    }
    ...
    public static function annotate_health($response, array $handler, WP_REST_Request $request)
    {
        ...
        foreach (['semantic_table_row_editing', 'table_cell_preview_apply', 'table_edit_reuses_atomic_batch_guards', 'semantic_table_state_chain_fix'] as $feature) {
    ...
    private static function capabilities(): array
    {
        return [
            ...
            'table_selector' => 'zero_based_table_index',
            'row_selector' => 'exact_normalized_text_in_key_column',
            'operation_types' => ['set_cell', 'set_cells', 'insert_row_before', 'insert_row_after', 'delete_row'],
            'max_operations' => 50,
            'plain_text_cells_escaped' => true,
            'html_cells_sanitized_with_wp_kses_post' => true,
            'inserted_rows_clone_anchor_row_structure' => true,
            'semantic_operations_translate_to_atomic_v085_batch' => true,
            'apply_requires_expected_before_sha256' => true,
            'apply_requires_expected_plan_hash' => true,
            'optional_expected_table_sha256' => true,
            'non_draft_apply_requires_confirm_live' => true,
            'inspect_rows_are_opt_in_and_bounded' => true,
        ];
    }
    ...
    private static function audit_params(array $params): array
    {
        ...
        foreach ($out['operations'] as $index => $operation) {
            ...
            $item = $operation;
            if (isset($item['row_key']) && is_string($item['row_key'])) {
                $item['row_key'] = self::digest($item['row_key']);
            }
            if (isset($item['content']) && is_string($item['content'])) {
                $item['content'] = self::digest($item['content']);
            }
