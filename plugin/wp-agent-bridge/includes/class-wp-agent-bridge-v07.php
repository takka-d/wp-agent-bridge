...
final class WP_Agent_Bridge_V07
{
    private const VERSION = '0.7.0';
    private const INTERNAL_NAMESPACE = 'wpab-v07/v1';
    private const INTERNAL_ROUTE = '/wpab-v07/v1/manage';
    ...
    public static function prepare_internal_call($result, WP_REST_Server $server, WP_REST_Request $request)
    {
        ...
        if (in_array($action, ['option.get', 'option.update'], true)) {
            ...
            if (strpos($name, 'wpab_') === 0) {
                return new WP_REST_Response([
                    'code' => 'wpab_internal_option_protected',
                    'message' => 'WP Agent Bridge Bridge internal options require dedicated Bridge actions.',
                    'data' => ['status' => 403],
                ], 403);
            }
        }
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v07_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v07_json', 'JSON body is required.', ['status' => 400]);
        }
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v07_unknown_action', 'Unknown or blocked v0.7 action.', ['status' => 400, 'action' => $action]);
        }

        try {
            $response = self::run($action, $params);
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v07_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        if (in_array($action, self::AUDITED, true)) {
            WP_Agent_Bridge_V07_Audit::record(self::$request_id, $action, $params, $response);
        }
        return $response;
    }
    ...
    private static function run(string $action, array $params)
    {
        switch ($action) {
            ...
            case 'audit.list': return rest_ensure_response(WP_Agent_Bridge_V07_Audit::list($params));
            case 'term.list': return WP_Agent_Bridge_V07_Terms::list($params);
            case 'term.get': return WP_Agent_Bridge_V07_Terms::get($params);
            case 'term.create': return WP_Agent_Bridge_V07_Terms::create($params);
            case 'term.update': return WP_Agent_Bridge_V07_Terms::update($params);
            case 'term.delete.preview': return WP_Agent_Bridge_V07_Terms::delete_preview($params);
            case 'term.delete': return WP_Agent_Bridge_V07_Terms::delete($params);
            case 'theme_mod.list': return WP_Agent_Bridge_V07_Settings::theme_mod_list();
            case 'theme_mod.get': return WP_Agent_Bridge_V07_Settings::theme_mod_get($params);
            case 'theme_mod.set': return WP_Agent_Bridge_V07_Settings::theme_mod_set($params);
            case 'theme_mod.remove': return WP_Agent_Bridge_V07_Settings::theme_mod_remove($params);
            case 'rewrite.status': return rest_ensure_response(WP_Agent_Bridge_V07_Settings::rewrite_status());
            case 'rewrite.set': return WP_Agent_Bridge_V07_Settings::rewrite_set($params);
            case 'user.list': return rest_ensure_response(WP_Agent_Bridge_V07_Users::list($params));
            case 'user.get': return WP_Agent_Bridge_V07_Users::get($params);
            case 'user.create': return WP_Agent_Bridge_V07_Users::create($params);
            case 'user.update': return WP_Agent_Bridge_V07_Users::update($params);
            case 'user.set_role': return WP_Agent_Bridge_V07_Roles::set_role($params);
            case 'user.add_role': return WP_Agent_Bridge_V07_Roles::add_role($params);
            case 'user.remove_role': return WP_Agent_Bridge_V07_Roles::remove_role($params);
            case 'user.password.reset_email': return WP_Agent_Bridge_V07_Users::password_reset_email($params);
            case 'user.meta.list': return WP_Agent_Bridge_V07_User_Meta::list($params);
            case 'user.meta.get': return WP_Agent_Bridge_V07_User_Meta::get($params);
            case 'user.meta.add': return WP_Agent_Bridge_V07_User_Meta::add($params);
            case 'user.meta.update': return WP_Agent_Bridge_V07_User_Meta::update($params);
            case 'user.meta.delete': return WP_Agent_Bridge_V07_User_Meta::delete($params);
        }
        return new WP_Error('wpab_v07_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }

    public static function annotate_health($response, array $handler, WP_REST_Request $request)
    {
        if ($request->get_route() !== '/wp-agent-bridge/v1/health' || is_wp_error($response)) return $response;
