 ...
 */
final class WP_Agent_Bridge_Theme_Patch_Diagnostics
{
    private const V04_ROUTE = '/wp-agent-bridge/v1/manage';
    private const V099_ROUTE = '/wpab-v099/v1/operate';
    ...
    public static function pre_dispatch($result, WP_REST_Server $server, WP_REST_Request $request)
    {
        ...
        $authorized = WP_Agent_Bridge_V04::authorize_request($request);
        ...
        if ($dry_run) {
            $preview = WP_Agent_Bridge_Theme_Patch::execute($params);
        ...
        if ($expected_visible_plan !== '') {
            ...
            $preflight = WP_Agent_Bridge_Theme_Patch::execute($preflight_params);
            ...
            $applied = WP_Agent_Bridge_Theme_Patch::execute($apply_params);
        ...
        $response = WP_Agent_Bridge_Theme_Patch::execute($params);
    ...
    private static function read_theme_file(array $params)
    {
        ...
        $response = WP_Agent_Bridge::execute($request);
        if (is_wp_error($response)) {
            return $response;
        }
    ...
    private static function same_theme_candidates(array $anchors, string $scope, string $draft_id, string $current_path): array
    {
        ...
        $request = new WP_REST_Request('POST', '/wpab-v096/v1/theme-files');
        $request->set_header('content-type', 'application/json');
        $request->set_body((string) wp_json_encode([
            'action' => 'theme.files.search',
            'params' => $params,
        ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
        $response = WP_Agent_Bridge_V096_Theme_Files::dispatch($request);
        if (is_wp_error($response)) {
            return [[
