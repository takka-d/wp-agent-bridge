 ...
 */
final class WP_Agent_Bridge_V093_Public_Readiness
{
    private const VERSION = '0.9.3';
    private const MANAGE_NAMESPACE = 'wpab-v093/v1';
    private const MANAGE_ROUTE = '/wpab-v093/v1/manage';
    ...
    private const ENCRYPTION_CONTEXT = 'wp-agent-bridge-at-rest-v1';
    ...
    public static function init(): void
    {
        ...
        add_action('admin_post_wpab_rotate_secure_key', [self::class, 'handle_rotate_secure_key']);
        add_action('admin_post_wpab_generate_key', [self::class, 'block_database_key_rotation_when_constant'], 0);
    }

    public static function constant_secret_override($pre_option, string $option, $default_value)
    {
        if (!defined('WPAB_BRIDGE_SECRET')) {
            return $pre_option;
        }
        $secret = constant('WPAB_BRIDGE_SECRET');
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v093_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch_manage(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v093_json', 'JSON body is required.', ['status' => 400]);
        }
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v093_unknown_action', 'Unknown or blocked v0.9.3 action.', ['status' => 400, 'action' => $action]);
        }
        ...
        try {
            ...
            return new WP_Error('wpab_v093_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return new WP_Error('wpab_v093_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }
    ...
    private static function secure_key_status(): array
    {
        ...
        return [
            ...
            'rsa_bits' => $bits,
            'bridge_secret_source' => defined('WPAB_BRIDGE_SECRET') ? 'wp_config_constant' : self::storage_state($raw_secret),
    ...
    private static function secure_key_rotate(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Secure server key rotation requires confirm=true.', ['status' => 400]);
        }
        if (!function_exists('openssl_pkey_new') || !defined('OPENSSL_KEYTYPE_RSA')) {
            return new WP_Error('wpab_secure_crypto_unavailable', 'OpenSSL RSA support is unavailable.', ['status' => 501]);
        }
        ...
        if ($expected !== '' && (!is_string($old_key_id) || !hash_equals($old_key_id, $expected))) {
            return new WP_Error('wpab_secure_key_changed', 'Secure server key changed before rotation.', [
                'status' => 409,
        ...
        if ($resource === false || !openssl_pkey_export($resource, $private_pem)) {
            return new WP_Error('wpab_secure_keygen', 'Could not generate or export a new RSA secure transport key.', ['status' => 500]);
        }
        $details = openssl_pkey_get_details($resource);
        if (!is_array($details) || !isset($details['key'])) {
            self::memzero($private_pem);
            return new WP_Error('wpab_secure_keygen', 'Could not export the new RSA public key.', ['status' => 500]);
        }
        ...
        if (self::encryption_key() !== null && self::storage_state($stored_private) !== 'encrypted_wp_options') {
            return new WP_Error('wpab_secure_key_storage', 'New secure private key was generated but could not be protected at rest.', ['status' => 500]);
        }
    ...
    {
        add_management_page(
            'WP Agent Bridge WordPress Bridge Security',
            'WP Agent Bridge WP Bridge Security',
            'manage_options',
            'wp-agent-bridge-security',
    ...
    public static function render_security_page(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__('WP Agent Bridge WordPress Bridge Security', 'wp-agent-bridge'); ?></h1>
            <p><strong><?php echo esc_html__('Bridge version:', 'wp-agent-bridge'); ?></strong> <?php echo esc_html(self::VERSION); ?></p>
            <p><strong><?php echo esc_html__('Bridge secret source:', 'wp-agent-bridge'); ?></strong> <?php echo esc_html((string) $status['bridge_secret_source']); ?></p>
            <p><strong><?php echo esc_html__('Secure private key storage:', 'wp-agent-bridge'); ?></strong> <?php echo esc_html((string) $status['secure_private_key_storage']); ?></p>
            <p><strong><?php echo esc_html__('Secure server key ID:', 'wp-agent-bridge'); ?></strong> <code><?php echo esc_html((string) ($status['server_key_id'] ?? 'not generated')); ?></code></p>
            <p><strong><?php echo esc_html__('Secure key created (GMT):', 'wp-agent-bridge'); ?></strong> <?php echo esc_html((string) ($status['created_gmt'] ?? 'not generated')); ?></p>
            <p><?php echo esc_html__('Rotating the secure server key makes newly encrypted requests use a new RSA key. The previous private key is not retained by the plugin.', 'wp-agent-bridge'); ?></p>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <input type="hidden" name="action" value="wpab_rotate_secure_key">
                <input type="hidden" name="expected_key_id" value="<?php echo esc_attr((string) ($status['server_key_id'] ?? '')); ?>">
                <?php wp_nonce_field('wpab_rotate_secure_key'); ?>
                <?php submit_button(__('Rotate Secure Server Key', 'wp-agent-bridge'), 'secondary'); ?>
            </form>
        </div>
        <?php
    }
    ...
    public static function handle_rotate_secure_key(): void
    {
        ...
        check_admin_referer('wpab_rotate_secure_key');
        ...
        wp_safe_redirect(admin_url('tools.php?page=wp-agent-bridge-security&rotated=1'));
        exit;
    }
    ...
    public static function block_database_key_rotation_when_constant(): void
    {
        ...
        wp_die(
            esc_html__('WPAB_BRIDGE_SECRET is defined in wp-config.php. Remove or change that constant instead of rotating the database copy.', 'wp-agent-bridge'),
            '',
            ['response' => 409]
        );
    }
