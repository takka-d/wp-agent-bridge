 ...
 */
final class WP_Agent_Bridge_V06_Self_Update_Source
{
    ...
    public static function apply(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Bridge source self-update requires confirm=true.', ['status' => 400]);
        }
        ...
        if (!preg_match('/^[a-f0-9]{40}$/', $source_commit)) {
            return new WP_Error('wpab_self_update_source_commit', 'source_commit must be a full 40-character Git commit SHA.', ['status' => 400]);
        }
        ...
        if ($target_version === '' || strlen($target_version) > 64 || !preg_match('/^[0-9A-Za-z][0-9A-Za-z._+-]*$/', $target_version)) {
            return new WP_Error('wpab_self_update_source_version', 'target_version is missing or invalid.', ['status' => 400]);
        }
        ...
        if (!preg_match('/^[a-f0-9]{64}$/', $expected_manifest_sha)) {
            return new WP_Error('wpab_self_update_source_manifest_sha', 'manifest_sha256 must be a SHA-256 hex digest.', ['status' => 400]);
        }
        ...
        if (array_key_exists('expected_file_count', $params)) {
            ...
            if ($expected_file_count < 1 || $expected_file_count > self::MAX_PLUGIN_FILES) {
                return new WP_Error('wpab_self_update_source_file_count', 'expected_file_count is outside the supported range.', ['status' => 400]);
            }
        }
        ...
        try {
            ...
            if ($expected_file_count !== null && count($manifest['files']) !== $expected_file_count) {
                return new WP_Error(
                    'wpab_self_update_source_file_count_mismatch',
            ...
            if (!hash_equals($expected_manifest_sha, $manifest['manifest_sha256'])) {
                return new WP_Error(
                    'wpab_self_update_source_manifest_mismatch',
            ...
            if ($delete_paths && (($params['confirm_delete_paths'] ?? null) !== true)) {
                return new WP_Error(
                    'wpab_self_update_delete_confirmation_required',
            ...
            $result = WP_Agent_Bridge_V06_Self_Update_Safe::apply($expanded);
    ...
    private static function download_archive(string $source_commit)
    {
        ...
        if (is_wp_error($response)) {
            return new WP_Error('wpab_self_update_source_download', 'Could not download the pinned WP Agent Bridge source archive.', ['status' => 502, 'error' => $response->get_error_message()]);
        }
        $status = (int) wp_remote_retrieve_response_code($response);
        $body = wp_remote_retrieve_body($response);
        if ($status !== 200 || !is_string($body) || $body === '') {
            return new WP_Error('wpab_self_update_source_http', 'Pinned source archive returned an unexpected HTTP response.', ['status' => 502, 'http_status' => $status]);
        }
        if (strlen($body) > self::MAX_ARCHIVE_BYTES) {
            return new WP_Error('wpab_self_update_source_archive_size', 'Pinned source archive exceeds the byte limit.', ['status' => 413, 'max_bytes' => self::MAX_ARCHIVE_BYTES]);
        }
        ...
        if (!is_string($temp) || $temp === '' || file_put_contents($temp, $body) === false) {
            ...
            return new WP_Error('wpab_self_update_source_tempfile', 'Could not stage the pinned source archive.', ['status' => 500]);
        }
    ...
    private static function extract_package(string $archive_path)
    {
        ...
        if (!is_array($entries) || !$entries) {
            return new WP_Error('wpab_self_update_source_archive', 'Pinned source archive could not be inspected.', ['status' => 502]);
        }
        if (count($entries) > self::MAX_ARCHIVE_ENTRIES) {
            return new WP_Error('wpab_self_update_source_archive_entries', 'Pinned source archive contains too many entries.', ['status' => 413]);
        }
        ...
        foreach ($entries as $entry) {
            ...
            if ($name === '' || $name[0] === '/' || strpos($name, "\0") !== false || preg_match('#(^|/)\.\.(/|$)#', $name)) {
                return new WP_Error('wpab_self_update_source_archive_path', 'Pinned source archive contains an unsafe path.', ['status' => 400]);
            }
            $uncompressed += max(0, (int) ($entry['size'] ?? 0));
            if ($uncompressed > self::MAX_ARCHIVE_UNCOMPRESSED_BYTES) {
                return new WP_Error('wpab_self_update_source_archive_uncompressed', 'Pinned source archive expands beyond the extraction byte limit.', ['status' => 413]);
            }
            if (preg_match('#^([^/]+)/plugin/wp-agent-bridge/wp-agent-bridge\.php$#', $name, $match)) {
                $prefixes[$match[1]] = true;
            }
        }
        if (count($prefixes) !== 1) {
            return new WP_Error('wpab_self_update_source_package_root', 'Pinned source archive does not contain exactly one WP Agent Bridge plugin root.', ['status' => 409]);
        }
        ...
        if (!wp_mkdir_p($extract_root)) {
            return new WP_Error('wpab_self_update_source_extract_dir', 'Could not create a temporary source extraction directory.', ['status' => 500]);
        }
        $result = $archive->extract(PCLZIP_OPT_PATH, $extract_root, PCLZIP_OPT_REPLACE_NEWER);
        if (!is_array($result) || !$result) {
            self::remove_tree($extract_root);
            return new WP_Error('wpab_self_update_source_extract', 'Could not extract the pinned source archive.', ['status' => 502]);
        }
        ...
        try {
            ...
            foreach ($iterator as $info) {
                ...
                if (strpos($absolute, $prefix) !== 0) {
                    return new WP_Error('wpab_self_update_source_path_escape', 'Downloaded package escaped the expected plugin root.', ['status' => 409]);
                }
                $relative = self::normalize_path(substr($absolute, strlen($prefix)));
                if ($relative === null) {
                    return new WP_Error('wpab_self_update_source_path', 'Downloaded package contains an unsupported plugin path.', ['status' => 409]);
                }
                $data = file_get_contents($info->getPathname());
                if (!is_string($data)) {
                    return new WP_Error('wpab_self_update_source_read', 'Could not read a file from the downloaded package.', ['status' => 500, 'path' => $relative]);
                }
                $total += strlen($data);
                if ($total > self::MAX_PLUGIN_BYTES) {
                    return new WP_Error('wpab_self_update_source_decoded_size', 'Downloaded plugin tree exceeds the self-update decoded byte limit.', ['status' => 413]);
                }
            ...
            return new WP_Error('wpab_self_update_source_scan', 'Could not scan the downloaded plugin tree.', ['status' => 500, 'error' => $e->getMessage()]);
        }
        if (!$entries || count($entries) > self::MAX_PLUGIN_FILES) {
            return new WP_Error('wpab_self_update_source_files', 'Downloaded plugin tree contains an invalid number of files.', ['status' => 413, 'file_count' => count($entries)]);
        }
    ...
    private static function existing_files()
    {
        ...
        try {
            ...
            foreach ($iterator as $info) {
                ...
                $absolute = str_replace('\\', '/', $info->getPathname());
                if (strpos($absolute, $prefix) !== 0) {
                    return new WP_Error('wpab_self_update_source_current_path_escape', 'Current plugin file escaped the expected root.', ['status' => 500]);
                }
                $relative = self::normalize_path(substr($absolute, strlen($prefix)));
                if ($relative === null) {
                    return new WP_Error('wpab_self_update_source_current_path', 'Current plugin contains an unsupported path.', ['status' => 500]);
                }
                $files[] = $relative;
            }
        } catch (Throwable $e) {
            return new WP_Error('wpab_self_update_source_current_scan', 'Could not scan current plugin files.', ['status' => 500, 'error' => $e->getMessage()]);
        }
