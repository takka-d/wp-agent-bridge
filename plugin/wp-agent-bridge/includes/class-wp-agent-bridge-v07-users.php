...
final class WP_Agent_Bridge_V07_Users
{
    ...
    public static function create(array $params)
    {
        ...
        if ($login === '' || !is_email($email)) return new WP_Error('wpab_user_create_fields', 'user_login and valid user_email are required.', ['status' => 400]);
        if (username_exists($login) || email_exists($email)) return new WP_Error('wpab_user_exists', 'Login or email already exists.', ['status' => 409]);
        ...
        if (!isset(wp_roles()->roles[$role])) return new WP_Error('wpab_user_role_unknown', 'Unknown WordPress role.', ['status' => 400, 'role' => $role]);
        if ($role === 'administrator' && empty($params['confirm_sensitive'])) {
            return new WP_Error('wpab_sensitive_confirmation_required', 'Creating an administrator requires confirm_sensitive=true.', ['status' => 400]);
        }
    ...
    public static function update(array $params)
    {
        ...
        if (isset($params['user_email']) && is_string($params['user_email'])) {
            ...
            if (!is_email($email)) return new WP_Error('wpab_user_email_invalid', 'user_email is invalid.', ['status' => 400]);
            if (strcasecmp($email, (string) $user->user_email) !== 0) {
                $existing = email_exists($email);
                if ($existing && (int) $existing !== (int) $user->ID) return new WP_Error('wpab_user_email_exists', 'Email is used by another account.', ['status' => 409]);
        ...
        if (isset($params['user_url']) && is_string($params['user_url'])) $data['user_url'] = esc_url_raw($params['user_url']);
        if (count($data) === 1) return new WP_Error('wpab_user_no_changes', 'No supported user fields were provided.', ['status' => 400]);
        if ($sensitive && empty($params['confirm_sensitive'])) return new WP_Error('wpab_sensitive_confirmation_required', 'Changing email requires confirm_sensitive=true.', ['status' => 400]);
    ...
    public static function password_reset_email(array $params)
    {
        ...
        if (empty($params['confirm_sensitive'])) return new WP_Error('wpab_sensitive_confirmation_required', 'Password reset email requires confirm_sensitive=true.', ['status' => 400]);
        $user = self::resolve($params);
        if (is_wp_error($user)) return $user;
        $sent = retrieve_password($user->user_login);
        if ($sent !== true) return is_wp_error($sent) ? $sent : new WP_Error('wpab_password_reset_failed', 'WordPress could not send the password reset email.', ['status' => 500]);
    ...
    public static function resolve(array $params)
    {
        ...
        return $user instanceof WP_User ? $user : new WP_Error('wpab_user_not_found', 'User was not found.', ['status' => 404]);
    }
    ...
    {
        return new WP_Error('wpab_user_password_transport_blocked', 'Plaintext password values are blocked on the GitHub queue transport. Use generated passwords and user.password.reset_email.', ['status' => 400]);
    }
}
