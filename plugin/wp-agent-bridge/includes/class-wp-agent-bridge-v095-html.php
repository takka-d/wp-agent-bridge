...
final class WP_Agent_Bridge_V095_HTML
{
    ...
    public static function inspect(array $params)
    {
        ...
        for ($hop = 0; $hop <= self::MAX_REDIRECTS; $hop++) {
            ...
            if (is_wp_error($response)) {
                return new WP_Error('wpab_v095_html_http', $response->get_error_message(), [
                    'status' => 502,
                    'url' => $current,
            ...
            if ($status >= 300 && $status < 400 && $location !== '') {
                if ($hop >= self::MAX_REDIRECTS) {
                    return new WP_Error('wpab_v095_html_redirect_limit', 'Redirect limit reached.', ['status' => 508]);
                }
                $next = self::same_origin($location, $current);
                if (is_wp_error($next)) {
                    return new WP_Error('wpab_v095_html_redirect_blocked', 'Redirect left the WordPress origin.', [
                        'status' => 400,
                        'from' => $current,
                        'location' => $location,
                    ]);
        ...
        if (!is_array($response)) {
            return new WP_Error('wpab_v095_html_empty', 'HTML request returned no response.', ['status' => 502]);
        }
        ...
        if ($content_type !== '' && strpos($content_type, 'html') === false && strpos($content_type, 'xml') === false) {
            return new WP_Error('wpab_v095_not_html', 'Response is not HTML/XML content.', [
                'status' => 415,
    ...
    private static function select(string $html, string $selector, int $max_matches, int $max_chars)
    {
        if (!class_exists('DOMDocument') || !class_exists('DOMXPath')) {
            return new WP_Error('wpab_v095_dom_unavailable', 'DOM extension is not available.', ['status' => 501]);
        }
        ...
        if (!$loaded) {
            return new WP_Error('wpab_v095_dom_parse', 'HTML could not be parsed.', ['status' => 400]);
        }
        $nodes = @(new DOMXPath($dom))->query($xpath_expr);
        if ($nodes === false) {
            return new WP_Error('wpab_v095_selector_query', 'Selector could not be evaluated.', ['status' => 400]);
        }
        $matches = [];
        $used = 0;
    ...
    private static function css_to_xpath(string $selector)
    {
        if ($selector === '' || strlen($selector) > 1000 || preg_match('/[\r\n\0]/', $selector)) {
            return new WP_Error('wpab_v095_selector', 'Selector is empty or invalid.', ['status' => 400]);
        }
        if (strpos($selector, ',') !== false || strpos($selector, ':') !== false
            || strpos($selector, '+') !== false || strpos($selector, '~') !== false) {
            return new WP_Error(
                'wpab_v095_selector_subset',
        ...
        if (!is_array($parts) || !$parts) {
            return new WP_Error('wpab_v095_selector', 'Selector could not be parsed.', ['status' => 400]);
        }
        $xpath = '.';
        $axis = '//';
        ...
        while ($part !== '') {
            if ($part[0] === '#') {
                if (!preg_match('/^#([A-Za-z_][A-Za-z0-9_-]*)/', $part, $m)) {
                    return new WP_Error('wpab_v095_selector_id', 'Invalid id selector.', ['status' => 400]);
                }
            ...
            if ($part[0] === '.') {
                if (!preg_match('/^\.([A-Za-z_][A-Za-z0-9_-]*)/', $part, $m)) {
                    return new WP_Error('wpab_v095_selector_class', 'Invalid class selector.', ['status' => 400]);
                }
            ...
            if ($part[0] === '[') {
                if (!preg_match('/^\[([A-Za-z_:][A-Za-z0-9_:.\-]*)(?:=([\'\"]?)([^\]\'\"]*)\2)?\]/', $part, $m)) {
                    return new WP_Error('wpab_v095_selector_attr', 'Invalid attribute selector.', ['status' => 400]);
                }
                $predicates[] = isset($m[3]) && $m[3] !== '' ? '@' . $m[1] . '=' . self::literal($m[3]) : '@' . $m[1];
                $part = substr($part, strlen($m[0]));
                continue;
            }
            return new WP_Error('wpab_v095_selector_subset', 'Unsupported selector fragment.', [
                'status' => 400,
                'fragment' => $part,
            ]);
        }
    ...
    private static function same_origin(string $candidate, ?string $base)
    {
        ...
        if ($candidate === '' || strlen($candidate) > self::MAX_URL || preg_match('/[\r\n\0]/', $candidate)) {
            return new WP_Error('wpab_v095_url', 'Invalid URL.', ['status' => 400]);
        }
        $home = wp_parse_url(home_url('/'));
        if (!is_array($home) || empty($home['scheme']) || empty($home['host'])) {
            return new WP_Error('wpab_v095_home', 'Could not resolve WordPress origin.', ['status' => 500]);
        }
        ...
        if (strpos($candidate, '//') === 0) {
            ...
            if ($base === null) {
                ...
                if (!is_array($parts)) {
                    return new WP_Error('wpab_v095_base', 'Invalid base URL.', ['status' => 400]);
                }
        ...
        if (!is_array($parts) || empty($parts['scheme']) || empty($parts['host'])) {
            return new WP_Error('wpab_v095_url_parse', 'Could not parse URL.', ['status' => 400]);
        }
        ...
        if (!in_array($scheme, ['http', 'https'], true) || $scheme !== $home_scheme || $host !== $home_host || $port !== $home_port) {
            return new WP_Error('wpab_v095_cross_origin', 'HTML inspection is restricted to the WordPress site origin.', ['status' => 400]);
        }
