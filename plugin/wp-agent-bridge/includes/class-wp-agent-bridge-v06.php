...
final class WP_Agent_Bridge_V06
{
    ...
    public static function init(): void
    {
        ...
        WP_Agent_Bridge_V06_Idempotency::init();
    }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if ($action === '' || !in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v06_unknown_action', 'Unknown or blocked v0.6 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'bridge.self_update.status':
                    return rest_ensure_response(WP_Agent_Bridge_V06_Self_Update::status());
                case 'bridge.self_update.apply':
                    if (isset($params['source_commit']) && is_string($params['source_commit']) && trim($params['source_commit']) !== '') {
                        ...
                        return WP_Agent_Bridge_V06_Self_Update_Source::apply($params);
                    }
                    return WP_Agent_Bridge_V06_Self_Update_Safe::apply($params);
                case 'bridge.self_update.rollback':
                    return WP_Agent_Bridge_V06_Self_Update::rollback($params);
            }
        } catch (Throwable $e) {
            return new WP_Error('wpab_v06_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }

        return new WP_Error('wpab_v06_dispatch_error', 'v0.6 dispatch fell through unexpectedly.', ['status' => 500]);
    }
    ...
    private static function capabilities(): array
    {
        return [
            ...
            'self_update' => WP_Agent_Bridge_V06_Self_Update_Safe::capabilities(),
            'source_self_update' => WP_Agent_Bridge_V06_Self_Update_Source::capabilities(),
    ...
    private static function decode_envelope(WP_REST_Request $request)
    {
        ...
        if (!is_array($json) || !isset($json['payload_b64']) || !is_string($json['payload_b64'])) {
            return new WP_Error('wpab_v06_missing_envelope', 'Missing payload_b64.', ['status' => 400]);
        }
        $payload_b64 = trim($json['payload_b64']);
        if ($payload_b64 === '' || strlen($payload_b64) > self::MAX_ENVELOPE_BYTES) {
            return new WP_Error('wpab_v06_envelope_size', 'Envelope is empty or too large.', ['status' => 413]);
        }
        $decoded = base64_decode($payload_b64, true);
        if (!is_string($decoded)) {
            return new WP_Error('wpab_v06_invalid_base64', 'Envelope is not valid Base64.', ['status' => 400]);
        }
        $payload = json_decode($decoded, true);
        if (!is_array($payload) || json_last_error() !== JSON_ERROR_NONE) {
            return new WP_Error('wpab_v06_invalid_json', 'Decoded envelope is not valid JSON.', ['status' => 400]);
        }
        return $payload;
    }
}
