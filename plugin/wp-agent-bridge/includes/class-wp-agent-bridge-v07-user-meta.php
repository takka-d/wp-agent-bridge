...
final class WP_Agent_Bridge_V07_User_Meta
{
    ...
    public static function get(array $params)
    {
        ...
        if (!metadata_exists('user', (int) $user->ID, $key)) {
            return new WP_Error('wpab_user_meta_not_found', 'User meta was not found.', ['status' => 404]);
        }
    ...
    private static function write(string $mode, array $params)
    {
        ...
        if (!array_key_exists('value', $params)) return new WP_Error('wpab_user_meta_value_required', 'value is required.', ['status' => 400]);
        if (self::contains_sensitive_nested_key($params['value'])) {
            return new WP_Error(
                'wpab_user_meta_nested_sensitive_write',
        ...
        if (!is_string($serialized) || strlen($serialized) > self::MAX_VALUE_BYTES) return new WP_Error('wpab_user_meta_value_size', 'User meta value is too large.', ['status' => 413]);
        $result = $mode === 'add'
    ...
    {
        if (!isset($params['key']) || !is_string($params['key'])) return new WP_Error('wpab_user_meta_key_required', 'key is required.', ['status' => 400]);
        $key = trim($params['key']);
        if ($key === '' || strlen($key) > 191 || strpos($key, "\0") !== false) return new WP_Error('wpab_user_meta_key_invalid', 'Invalid user meta key.', ['status' => 400]);
        return $key;
    }
    ...
    {
        return new WP_Error('wpab_confirmation_required', 'User meta changes require confirm=true.', ['status' => 400]);
    }

    private static function sensitive_error(string $verb): WP_Error
    {
        return new WP_Error('wpab_user_meta_sensitive', "Sensitive capability/session/credential meta cannot be {$verb} through the Bridge.", ['status' => 403]);
    }
}
