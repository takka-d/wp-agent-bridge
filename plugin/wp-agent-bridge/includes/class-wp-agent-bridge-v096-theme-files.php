 ...
 */
final class WP_Agent_Bridge_V096_Theme_Files
{
    private const VERSION = '0.9.6';
    private const NS = 'wpab-v096/v1';
    private const ROUTE = '/wpab-v096/v1/theme-files';
    ...
    public static function permission()
    {
        if (!self::$allowed || !current_user_can('manage_options')) {
            return new WP_Error(
                'wpab_v096_theme_files_internal_only',
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v096_theme_files_json', 'JSON body is required.', ['status' => 400]);
        }
        $action = is_string($json['action'] ?? null) ? trim($json['action']) : '';
        $params = isset($json['params']) && is_array($json['params']) ? $json['params'] : [];
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v096_theme_files_action', 'Unknown or blocked v0.9.6 theme-files action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        try {
            ...
            return new WP_Error('wpab_v096_theme_files_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }
        return new WP_Error('wpab_v096_theme_files_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }
    ...
    private static function list_files(array $params)
    {
        ...
        if (strlen($pattern) > 1000 || strpos($pattern, "\0") !== false || preg_match('~(^|/)\.\.(/|$)~', $pattern)) {
            return new WP_Error('wpab_v096_theme_files_pattern', 'Invalid file pattern.', ['status' => 400]);
        }
    ...
    private static function search_files(array $params)
    {
        ...
        if ($query === '' || strlen($query) > 4096) {
            return new WP_Error('wpab_v096_theme_files_query', 'query must be a non-empty string within 4096 bytes.', ['status' => 400]);
        }
    ...
    private static function read_many(array $params)
    {
        ...
        if (!$items || count($items) > self::MAX_READ_ITEMS) {
            return new WP_Error('wpab_v096_theme_files_read_many', 'Provide 1-' . self::MAX_READ_ITEMS . ' files.', ['status' => 400]);
        }
        ...
        foreach ($items as $index => $item) {
            if (!is_array($item) || !isset($item['path']) || !is_string($item['path']) || trim($item['path']) === '') {
                return new WP_Error('wpab_v096_theme_files_read_item', 'Each files entry requires a path.', [
                    'status' => 400,
                    'index' => $index,
                ]);
            }
            ...
            $response = WP_Agent_Bridge_V095_Outline::read_range($read);
    ...
    private static function resolve_root(array $params)
    {
        ...
        if ($scope === 'active') {
            ...
            if ($draft_id === '') {
                return new WP_Error('wpab_v096_theme_files_draft', 'draft_id is required for draft scope.', ['status' => 400]);
            }
            $slug = '';
            $drafts = get_option('wpab_draft_themes', []);
            if (is_array($drafts) && isset($drafts[$draft_id])) {
                $slug = basename((string) ($drafts[$draft_id]['slug'] ?? ''));
            }
            if ($slug === '') {
                $classic = get_option('wpab_v095_classic_drafts', []);
                if (is_array($classic) && isset($classic[$draft_id])) {
                    $slug = basename((string) ($classic[$draft_id]['slug'] ?? ''));
                }
            }
            if ($slug === '') {
                return new WP_Error('wpab_v096_theme_files_draft_not_found', 'Theme draft was not found.', ['status' => 404]);
            }
            $root = realpath(rtrim(get_theme_root(), '/\\') . DIRECTORY_SEPARATOR . $slug);
        } else {
            return new WP_Error('wpab_v096_theme_files_scope', 'scope must be active or draft.', ['status' => 400]);
        }
        if ($root === false || !is_dir($root)) {
            return new WP_Error('wpab_v096_theme_files_root', 'Theme directory was not found.', ['status' => 404]);
        }
    ...
    private static function extensions(array $params)
    {
        ...
        if (!is_array($params['extensions']) || !$params['extensions']) {
            return new WP_Error('wpab_v096_theme_files_extensions', 'extensions must be a non-empty array.', ['status' => 400]);
        }
        $out = [];
        foreach ($params['extensions'] as $extension) {
            if (!is_string($extension)) {
                return new WP_Error('wpab_v096_theme_files_extension', 'Invalid file extension.', ['status' => 400]);
            }
            $extension = strtolower(ltrim(trim($extension), '.'));
            if (!in_array($extension, self::DEFAULT_EXTENSIONS, true)) {
                return new WP_Error('wpab_v096_theme_files_extension', 'Unsupported file extension.', [
                    'status' => 400,
                    'extension' => $extension,
                ]);
            }
