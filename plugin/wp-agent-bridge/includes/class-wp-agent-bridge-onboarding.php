 ...
 */
final class WP_Agent_Bridge_Onboarding
{
    private const OPTION_LEGACY_CONNECTION = 'wpab_github_connection_v1';
    private const OPTION_SECRET = 'wpab_secret';
    private const OPTION_USER_ID = 'wpab_user_id';
    private const OPTION_PENDING_SETUP = 'wpab_direct_onboarding_pending_v1';
    ...
    public static function init(): void
    {
        ...
        add_action('admin_post_wpab_connect_github', [self::class, 'handle_connect']);
    ...
    public static function admin_notice(): void
    {
        ...
        if (isset($_GET['page']) && sanitize_key((string) $_GET['page']) === 'wp-agent-bridge-connect') {
            return;
        }
        $url = admin_url('tools.php?page=wp-agent-bridge-connect');
        ?>
    ...
    private static function is_connected(): bool
    {
        ...
        $app = WP_Agent_Bridge_Direct_GitHub::app_config();
        return !empty($connection['installation_id'])
            && !empty($connection['repository_id'])
            && !empty($connection['repository'])
            && !empty($app['app_id'])
            && !empty($app['slug'])
            && WP_Agent_Bridge_Direct_GitHub::private_key() !== ''
            && WP_Agent_Bridge_Direct_GitHub::webhook_secret() !== '';
    }
    ...
    public static function render_page(): void
    {
        ...
        $status = isset($_GET['wpab_setup']) ? sanitize_key((string) $_GET['wpab_setup']) : '';
        ...
        <div class="wrap">
            ...
            <?php if ($connected) : ?>
                ...
                <textarea id="wpab-chat-start" class="large-text" rows="12" readonly><?php echo esc_textarea(WP_Agent_Bridge_Runtime_Guidance::client_prompt(
            ...
            <?php else : ?>
                ...
                <p><a id="wpab-direct-create-repo" class="button" target="_blank" rel="noopener noreferrer" href="<?php echo esc_url($repo_url); ?>">Create private repository on GitHub</a></p>
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    <input type="hidden" name="action" value="wpab_connect_github">
                    <input type="hidden" name="expected_repository_name" value="<?php echo esc_attr($repo_name); ?>">
                    <?php wp_nonce_field('wpab_connect_github'); ?>
    ...
    public static function handle_connect(): void
    {
        ...
        WP_Agent_Bridge_Direct_GitHub::clear_credentials();
        WP_Agent_Bridge_Direct_Runtime::clear_connection();
    ...
    public static function manifest_complete(WP_REST_Request $request)
    {
        ...
        if (!preg_match('/^[A-Za-z0-9_-]{10,200}$/', $code)) {
            return new WP_Error('wpab_direct_manifest_code', 'Invalid GitHub manifest code.', ['status' => 400]);
        }
        ...
        if ($status !== 201 || !is_array($data)) {
            ...
            return new WP_Error('wpab_direct_manifest_exchange', $message, ['status' => 502]);
        }
        ...
        if ($app_id < 1
            ...
            return new WP_Error('wpab_direct_manifest_response', 'GitHub returned an unexpected App configuration.', ['status' => 502]);
        }

        $requested_org = (string) ($pending['organization'] ?? '');
        if ($requested_org !== '' && strcasecmp($requested_org, $owner_login) !== 0) {
            return new WP_Error('wpab_direct_manifest_owner', 'The GitHub App was created under a different owner than requested.', ['status' => 403]);
        }

        if (!WP_Agent_Bridge_Direct_GitHub::store_app_credentials($app_id, $slug, $pem, $webhook_secret)) {
            return new WP_Error('wpab_direct_manifest_store', 'Could not securely store the GitHub App credentials on WordPress.', ['status' => 500]);
        }
    ...
    public static function installed(WP_REST_Request $request)
    {
        ...
        if ($installation_id < 1) {
            return new WP_Error('wpab_direct_installation_id', 'GitHub did not provide a valid installation ID.', ['status' => 400]);
        }
        ...
        $jwt = WP_Agent_Bridge_Direct_GitHub::app_jwt();
        if (is_wp_error($jwt)) {
            return $jwt;
        }
        $installation = WP_Agent_Bridge_Direct_GitHub::github_api(
            'GET',
        ...
        if ($account_login === ''
            ...
            return new WP_Error(
                'wpab_direct_installation_scope',
                'Install the site-specific GitHub App with Only select repositories, under the same account that owns the App.',
                ['status' => 403]
            );
        }

        $token = WP_Agent_Bridge_Direct_GitHub::installation_token($installation_id);
        if (is_wp_error($token)) {
            return $token;
        }
        $repositories = WP_Agent_Bridge_Direct_GitHub::installation_repositories($token);
        if (is_wp_error($repositories)) {
            return $repositories;
        }
        if (count($repositories) !== 1 || !is_array($repositories[0])) {
            return new WP_Error(
                'wpab_direct_repository_count',
        ...
        if ($repository_id < 1
            ...
            return new WP_Error(
                'wpab_direct_repository_mismatch',
                'The selected repository must be the private runtime repository created for this setup.',
                ['status' => 400]
            );
        }

        $scoped_token = WP_Agent_Bridge_Direct_GitHub::installation_token($installation_id, $repository_id);
        ...
        if (!WP_Agent_Bridge_Direct_Runtime::store_connection([
            'installation_id' => $installation_id,
            'repository_id' => $repository_id,
            'repository' => $full_name,
            'runtime_branch' => self::RUNTIME_BRANCH,
        ])) {
            return new WP_Error('wpab_direct_connection_store', 'Could not store the direct GitHub connection.', ['status' => 500]);
        }
        ...
        $return_url = add_query_arg('wpab_setup', 'complete', admin_url('tools.php?page=wp-agent-bridge-connect'));
    ...
    private static function manifest(string $state): array
    {
        ...
        return [
            ...
            'hook_attributes' => [
                'url' => rest_url(ltrim(WP_Agent_Bridge_Direct_Runtime::WEBHOOK_ROUTE, '/')),
                'active' => true,
            ],
    ...
    private static function pending(string $state, string $stage)
    {
        if (!preg_match('/^[a-f0-9]{64}$/', $state)) {
            return new WP_Error('wpab_direct_state', 'Invalid setup state.', ['status' => 400]);
        }
        ...
        if (!is_array($pending)
            ...
            return new WP_Error('wpab_direct_state_expired', 'The GitHub setup session is invalid or expired. Start Connect GitHub again.', ['status' => 410]);
        }
        return $pending;
    }
    ...
    private static function initialize_runtime_repository(string $token, string $full_name)
    {
        if (!preg_match('/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/', $full_name)) {
            return new WP_Error('wpab_direct_runtime_repo', 'Invalid runtime repository name.');
        }
        $repo_path = '/repos/' . self::encode_repository($full_name);
        $repo_response = WP_Agent_Bridge_Direct_GitHub::github_api('GET', $repo_path, $token);
        ...
        if (empty($repo['private'])) {
            return new WP_Error('wpab_direct_runtime_private', 'Runtime repository must remain private.');
        }
        ...
        if ($runtime_ref === null) {
            ...
            if ($default_ref === null) {
                ...
                $init = WP_Agent_Bridge_Direct_GitHub::github_api(
                    'PUT',
                ...
                if (is_wp_error($default_ref) || $default_ref === null) {
                    return is_wp_error($default_ref)
                        ? $default_ref
                        : new WP_Error('wpab_direct_default_branch', 'GitHub did not create the default branch.');
                }
            }
            $sha = isset($default_ref['object']['sha']) ? (string) $default_ref['object']['sha'] : '';
            if (!preg_match('/^[a-f0-9]{40,64}$/', strtolower($sha))) {
                return new WP_Error('wpab_direct_default_sha', 'Could not determine the default branch commit.');
            }
            $created = WP_Agent_Bridge_Direct_GitHub::github_api(
                'POST',
                $repo_path . '/git/refs',
                $token,
    ...
    private static function get_ref_optional(string $token, string $full_name, string $branch)
    {
        ...
        $response = WP_Agent_Bridge_Direct_GitHub::github_api('GET', $endpoint, $token);
        if (is_wp_error($response)) {
    ...
    private static function put_if_missing(string $token, string $full_name, string $path, string $content)
    {
        ...
        $lookup = WP_Agent_Bridge_Direct_GitHub::github_api(
            'GET',
        ...
        return WP_Agent_Bridge_Direct_GitHub::github_api('PUT', $endpoint, $token, [
    ...
    private static function redirect_response(string $url)
    {
        ...
        if (!is_array($parts) || !isset($parts['scheme'], $parts['host'])) {
            return new WP_Error('wpab_direct_redirect', 'Invalid setup redirect.', ['status' => 500]);
        }
        $host = strtolower((string) $parts['host']);
        if ($host !== 'github.com' && $host !== strtolower((string) wp_parse_url(home_url('/'), PHP_URL_HOST))) {
            return new WP_Error('wpab_direct_redirect_host', 'Unexpected setup redirect host.', ['status' => 500]);
        }
