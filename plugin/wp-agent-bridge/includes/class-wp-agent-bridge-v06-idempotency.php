...
final class WP_Agent_Bridge_V06_Idempotency
{
    ...
    private const PREFIX = 'wpab_idem_';
    private const INDEX = 'wpab_idem_index';
    ...
    public static function before($result, WP_REST_Server $server, WP_REST_Request $request)
    {
        ...
        if (is_array($existing)) {
            if (!isset($existing['payload_hash']) || !hash_equals((string) $existing['payload_hash'], $context['payload_hash'])) {
                return self::error('wpab_idempotency_payload_conflict', 'request_id was already used for a different payload.', 409, [
                    'request_id' => $context['request_id'],
                ]);
            }
            if (($existing['state'] ?? '') === 'completed') {
                $headers = isset($existing['headers']) && is_array($existing['headers']) ? $existing['headers'] : [];
                unset($headers['X-WPAB-Idempotency-Internal'], $headers['X-WPAB-Idempotent-Replay']);
                ...
                $cached->header('X-WPAB-Idempotency-Internal', '1');
                return $cached;
            }
            $created_at = isset($existing['created_at']) ? (int) $existing['created_at'] : 0;
            if ($created_at > 0 && (time() - $created_at) < self::IN_PROGRESS_TTL) {
                return self::error('wpab_idempotency_in_progress', 'The same request_id is already in progress.', 409, [
                    'request_id' => $context['request_id'],
                ]);
            }
        ...
        if (!add_option($option, self::lock($context), '', false)) {
            return self::error('wpab_idempotency_lock_race', 'Could not acquire idempotency lock.', 409, [
                'request_id' => $context['request_id'],
            ]);
        }
        self::touch($option);
    ...
    public static function after($response, array $handler, WP_REST_Request $request)
    {
        ...
        if (isset($headers['X-WPAB-Idempotency-Internal'])) {
            return $response;
        }
