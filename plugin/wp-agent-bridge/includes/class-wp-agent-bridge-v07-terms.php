...
final class WP_Agent_Bridge_V07_Terms
{
    ...
    public static function create(array $params)
    {
        ...
        if ($name === '') return new WP_Error('wpab_term_name_required', 'name is required.', ['status' => 400]);
        $args = [];
    ...
    public static function update(array $params)
    {
        ...
        if ($taxonomy->hierarchical && isset($params['parent'])) {
            ...
            if ($parent === (int) $term->term_id) return new WP_Error('wpab_term_parent_self', 'A term cannot be its own parent.', ['status' => 400]);
            $args['parent'] = $parent;
        }
        if (!$args) return new WP_Error('wpab_term_no_changes', 'No supported term fields were provided.', ['status' => 400]);
    ...
    public static function delete(array $params)
    {
        ...
        if (!empty($params['expected_impact_hash']) && is_string($params['expected_impact_hash']) && !hash_equals($impact['impact_hash'], strtolower(trim($params['expected_impact_hash'])))) {
            return new WP_Error('wpab_term_delete_impact_changed', 'Term deletion impact changed after preview.', ['status' => 409, 'current_impact' => $impact]);
        }
    ...
    private static function taxonomy(array $params)
    {
        ...
        if ($name === '') return new WP_Error('wpab_taxonomy_required', 'taxonomy is required.', ['status' => 400]);
        $taxonomy = get_taxonomy($name);
        if (!$taxonomy) return new WP_Error('wpab_taxonomy_not_found', 'Taxonomy was not found.', ['status' => 404]);
        if (empty($taxonomy->show_ui) && empty($taxonomy->public)) return new WP_Error('wpab_taxonomy_internal_blocked', 'Internal/private taxonomies are blocked.', ['status' => 403]);
        return $taxonomy;
    }
    ...
    private static function resolve(string $taxonomy, array $params)
    {
        ...
        if (!$term || is_wp_error($term)) return is_wp_error($term) ? $term : new WP_Error('wpab_term_not_found', 'Term was not found.', ['status' => 404]);
        return $term;
    }
    ...
    private static function forbidden(): WP_Error { return new WP_Error('wpab_term_forbidden', 'Bridge user cannot manage this taxonomy.', ['status' => 403]); }
}
