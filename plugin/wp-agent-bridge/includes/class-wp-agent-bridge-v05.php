 ...
 */
final class WP_Agent_Bridge_V05
{
    private const VERSION = '0.5.0';
    private const ROUTE_NAMESPACE = 'wp-agent-bridge/v1';
    private const ROUTE = '/wp-agent-bridge/v1/v05';
    private const OPTION_SECRET = 'wpab_secret';
    private const OPTION_USER_ID = 'wpab_user_id';
    private const IDEMPOTENCY_PREFIX = 'wpab_idem_';
    private const IDEMPOTENCY_INDEX = 'wpab_idem_index';
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if ($action === '' || !in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v05_unknown_action', 'Unknown or blocked v0.5 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'db.search_replace.plan':
                    return WP_Agent_Bridge_V05_Search_Replace::plan($params);
                case 'db.search_replace.execute':
                    return WP_Agent_Bridge_V05_Search_Replace::execute($params);
                case 'menu.list':
                    return rest_ensure_response(WP_Agent_Bridge_V05_Admin::menu_list());
                case 'menu.get':
                    return WP_Agent_Bridge_V05_Admin::menu_get($params);
                case 'menu.create':
                    return WP_Agent_Bridge_V05_Admin::menu_create($params);
                case 'menu.update':
                    return WP_Agent_Bridge_V05_Admin::menu_update($params);
                case 'menu.delete':
                    return WP_Agent_Bridge_V05_Admin::menu_delete($params);
                case 'menu.item.upsert':
                    return WP_Agent_Bridge_V05_Admin::menu_item_upsert($params);
                case 'menu.item.delete':
                    return WP_Agent_Bridge_V05_Admin::menu_item_delete($params);
                case 'menu.locations.set':
                    return WP_Agent_Bridge_V05_Admin::menu_locations_set($params);
                case 'updates.status':
                    return rest_ensure_response(WP_Agent_Bridge_V05_Admin::updates_status(!empty($params['refresh'])));
            }
        } catch (Throwable $e) {
            return new WP_Error('wpab_v05_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }

        return new WP_Error('wpab_v05_dispatch_error', 'v0.5 dispatch fell through unexpectedly.', ['status' => 500]);
    }
    ...
    public static function idempotency_before($response, array $handler, WP_REST_Request $request)
    {
        ...
        if (!in_array($request->get_route(), [
            '/wp-agent-bridge/v1/execute',
            '/wp-agent-bridge/v1/manage',
            self::ROUTE,
        ], true)) {
            return $response;
        }
        ...
        if (is_array($existing)) {
            if (!isset($existing['payload_hash']) || !hash_equals((string) $existing['payload_hash'], $context['payload_hash'])) {
                return self::idempotency_short_circuit_response(
                    'wpab_idempotency_payload_conflict',
    ...
    public static function idempotency_after($response, array $handler, WP_REST_Request $request)
    {
        ...
        if (isset($response_headers['X-WPAB-Idempotency-Internal'])) {
            return $response;
        }
    ...
    private static function idempotency_short_circuit_response(string $code, string $message, int $status, array $extra = []): WP_REST_Response
    {
        ...
        $response->header('X-WPAB-Idempotency-Internal', '1');
        return $response;
    }
    ...
    private static function extract_request_context(WP_REST_Request $request)
    {
        ...
        $decoded = null;
        if ($request->get_route() === '/wp-agent-bridge/v1/execute') {
        ...
        if (strlen($request_id) > 120 || !preg_match('/^[A-Za-z0-9._:-]+$/', $request_id)) {
            return new WP_Error('wpab_invalid_request_id', 'request_id contains invalid characters or is too long.', ['status' => 400]);
        }
        return [
    ...
    private static function idempotency_status(array $params)
    {
        ...
        if (!is_array($record)) {
            return new WP_Error('wpab_idempotency_not_found', 'No idempotency record exists for request_id.', ['status' => 404]);
        }
    ...
    private static function decode_envelope(WP_REST_Request $request)
    {
        ...
        if (!is_array($json) || !isset($json['payload_b64']) || !is_string($json['payload_b64'])) {
            return new WP_Error('wpab_v05_missing_envelope', 'Missing payload_b64.', ['status' => 400]);
        }
        $payload_b64 = trim($json['payload_b64']);
        if ($payload_b64 === '' || strlen($payload_b64) > self::MAX_ENVELOPE_BYTES) {
            return new WP_Error('wpab_v05_envelope_size', 'Envelope is empty or too large.', ['status' => 413]);
        }
        $decoded = base64_decode($payload_b64, true);
        if (!is_string($decoded)) {
            return new WP_Error('wpab_v05_invalid_base64', 'Envelope is not valid Base64.', ['status' => 400]);
        }
        $payload = json_decode($decoded, true);
        if (!is_array($payload) || json_last_error() !== JSON_ERROR_NONE) {
            return new WP_Error('wpab_v05_invalid_json', 'Decoded envelope is not valid JSON.', ['status' => 400]);
        }
        return $payload;
    }
    ...
    private static function required_string(array $params, string $key, bool $trim = true)
    {
        if (!array_key_exists($key, $params) || !is_string($params[$key])) {
            return new WP_Error('wpab_required_string', "{$key} must be a string.", ['status' => 400]);
        }
        $value = $trim ? trim($params[$key]) : $params[$key];
        if ($trim && $value === '') {
            return new WP_Error('wpab_required_string_empty', "{$key} must not be empty.", ['status' => 400]);
        }
        return $value;
    }
}
