 ...
 * WP_Agent_Bridge_Runtime_Guidance so stale routing text cannot overwrite
 ...
 */
final class WP_Agent_Bridge_Direct_Runtime_Identity
{
    public static function sync()
    {
        ...
        return [
            ...
            'site_host' => $site_host,
            'files' => $written,
            'guidance_owned_by' => WP_Agent_Bridge_Runtime_Guidance::class,
        ];
    }
    ...
    private static function sync_file(string $token, string $repository, string $branch, string $path, string $content)
    {
        if ($content === '') {
            ...
            } elseif (WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($meta) !== 404) {
                return $meta;
            }
        } else {
