...
final class TakKa_WordPress_Bridge_Direct_Runtime_Identity
{
    public static function sync()
    {
        ...
        $agents = "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "Prefer WP Agent Bridge's own structured actions before WPVibe or temporary debug routes. Use `POST /takka-v094/v1/manage` for `http.probe`, `http.probe.batch`, and `media.file.inspect`; `POST /takka-v095/v1/manage` for structural/theme work; and `POST /takka-v096/v1/theme-files` for `theme.files.list`, `theme.files.search`, and `theme.file.read.many`.\n"
            ...
            . "9. On an integrity 409, inspect `chunk_diagnostics` or the fast-path chunk error, replace only the bad staged payload when identifiable, and then use `/wp-agent-bridge-runtime/v1/media-verify` only if diagnosis is still needed.\n"
            . "10. On success, WordPress reconstructs and validates the file, creates the attachment, optionally assigns Site Icon, and removes staged payloads with a single Git-tree cleanup commit and bounded retry.\n\n"
