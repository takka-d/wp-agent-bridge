...
final class TakKa_WordPress_Bridge_Direct_Runtime_Identity
{
    public static function sync()
    {
        ...
        $agents = "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "2. Split the binary into ordered chunks. The Bridge supports up to 1,200,000 decoded bytes per chunk, 32 chunks, and 6 MiB total; use smaller chunks if connector/tool payload limits require it.\n"
            . "3. For each chunk compute `chunk_bytes` and `chunk_sha256`, Base64-encode that chunk only, and send one command containing `upload_id`, `chunk_index`, `chunk_count`, `filename`, whole-file integrity fields, chunk integrity fields, and `data_b64`.\n"
            . "4. Send chunks sequentially and wait for the matching pending command to disappear or completed result to appear before sending the next chunk.\n"
            . "5. The final chunk automatically reassembles, verifies, creates the attachment, and cleans WordPress-side staging files.\n"
