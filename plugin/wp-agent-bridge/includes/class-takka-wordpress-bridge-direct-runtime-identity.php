...
final class TakKa_WordPress_Bridge_Direct_Runtime_Identity
{
    public static function sync()
    {
        ...
        $agents = "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "When `wordpress-bridge/RUNTIME_CAPABILITIES.json` advertises `atomic_command_bookkeeping=true`, result creation, completed-command storage, and pending deletion are one Git commit. Once the matching result appears, that command's bookkeeping is complete and the next pending command may be created immediately. Only use the older post-result wait rule on runtimes that do not advertise atomic bookkeeping.\n\n"
            . "## Bridge self-update\n\n"
            . "When the installed Bridge advertises `bridge.self_update.apply` source mode, prefer that small-command path for normal Bridge upgrades. Pin the official `takka-d/wp-agent-bridge` source with a full 40-character commit SHA and include `target_version`, aggregate `manifest_sha256`, `expected_file_count`, `expected_current_version`, and `confirm=true`. WordPress downloads that exact official source archive, reconstructs the complete plugin manifest locally, verifies integrity, and then reuses the full-manifest/PHP-parse/backup/rollback safety gate.\n"
            . "Do not generate or transport a 1+ MiB inline Base64 full-manifest command once source mode is available. Inline full-manifest `bridge.self_update.apply` remains a bootstrap/backward-compatibility fallback for older installed versions that do not yet support the source action.\n"
            . "After any self-update apply, verify the installed version with a separate `bridge.self_update.status` request because the next request loads the new code. Do not route Bridge self-update through WPVibe, a temporary PHP installer, an old GitHub Actions worker, an old Bridge Key, `takka-d/chatgpt-data`, or another legacy command queue.\n\n"
            ...
            . "2. split the ORIGINAL BINARY into ordered chunks before Base64 encoding. Keep total decoded size within 6 MiB and at most 32 chunks. Prefer about 64-128 KiB decoded chunks when connector limits are unknown; use larger chunks only when the connector is known to accept them. Smaller files should normally use one chunk.\n"
            ...
            . "5. The upload fast path automatically resolves all ordered `data_paths` from one current Git tree snapshot and then reads those Git blobs directly. `data_blob_shas` is optional; include it only when the staging tool already returned the payload blob SHAs and you want an additional path-to-blob pin.\n"
            . "6. If `create_tree` accepts inline UTF-8 `content`, prefer one tree creation containing every `.b64` payload plus the pending command, followed by one commit/ref update. If inline tree content is unavailable, use `create_blob` for the payloads/command and still publish them in ONE tree/commit/ref update. The command-bearing push should be the only WordPress-triggering push needed for the upload.\n"
            . "7. Send ONE upload command with `type=rest`, `method=POST`, `route=/wp-agent-bridge-runtime/v1/media-upload`, `filename`, ordered `data_paths`, whole-file integrity, `chunk_integrity`, and optional `data_blob_shas`. Do not call media-verify first in the normal path.\n"
        ...
        $runtime = "# Runtime\n\n"
            ...
            . "Command bookkeeping: when `atomic_command_bookkeeping` is advertised, result creation, completed-command storage, and pending deletion use one Git ref update; a visible matching result is the completion barrier for chaining the next command.\n"
            . "Bridge self-update: when source self-update capability is advertised, use `bridge.self_update.apply` with a small pinned-source command with the official source repository, a full 40-character commit SHA, `target_version`, aggregate `manifest_sha256`, `expected_file_count`, `expected_current_version`, and `confirm=true`; use inline full-manifest transport only as bootstrap/backward compatibility. Verify every apply with a separate `bridge.self_update.status` request. Do not use WPVibe, temporary installers, old Actions workers, old Bridge Keys, or retired command queues for self-update.\n"
            . "Media default: batched staged upload. Normal uploads do not run verify-first; integrity is checked by upload itself. When Git Data staging is available, publish payloads plus command in one commit; prefer inline tree content when supported. WordPress resolves staged paths from one tree snapshot and reads the resulting Git blobs directly. `data_blob_shas` is optional.\n"
