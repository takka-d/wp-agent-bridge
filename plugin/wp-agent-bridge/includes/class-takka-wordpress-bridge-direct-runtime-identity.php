...
final class TakKa_WordPress_Bridge_Direct_Runtime_Identity
{
    public static function sync()
    {
        ...
        $agents = "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "Before switching to WPVibe for a WordPress-server task, check WP Agent Bridge's structured actions and REST proxy. For the remaining inspection/theme-creation cases use `POST /takka-v095/v1/manage`.\n"
            ...
            . "4. Build ONE normal runtime REST command with `type=rest`, `method=POST`, `route=/wp-agent-bridge-runtime/v1/media-upload`, `filename`, whole-file `expected_bytes`, whole-file `expected_sha256`, and an ordered `data_paths` array containing all staged chunk paths. For upload-and-favicon in one operation, also add `set_site_icon=true` and `confirm_site_icon=true`.\n"
            ...
            . "7. Wait once for the media-upload result. WordPress reads all ordered `data_paths`, reconstructs and validates the file, creates the attachment, optionally assigns it as Site Icon when explicitly requested, then removes the staged payloads with its bounded single-tree cleanup flow.\n"
            . "This batched staged-media path is the default for ChatGPT-local and connector-downloaded files because it removes the previous `chunk command -> Webhook -> result -> next chunk` loop.\n\n"
        ...
        $runtime = "# Runtime\n\n"
            ...
            . "WPVibe compatibility / server-side inspection: `POST /takka-v095/v1/manage` with `theme.file.outline`, `theme.file.read.range`, `page.html.inspect`, or `classic_theme.create/list/info/preview_url/publish/discard`. Prefer these and existing WP Agent Bridge structured/REST actions over WPVibe for WordPress-server work; JavaScript-executed DOM/layout/browser events remain browser-side.\n"
            ...
            . "ChatGPT-local or connector-downloaded media default: split the original binary into bounded chunks, Base64 each chunk independently, stage the Base64 text under `wordpress-bridge/media/pending/*.b64`, then submit ONE `/wp-agent-bridge-runtime/v1/media-upload` command with ordered `data_paths`, whole-file bytes, and SHA-256.\n"
