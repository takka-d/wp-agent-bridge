 ...
 */
final class WP_Agent_Bridge_V06_Self_Update_Safe
{
    public static function capabilities(): array
    {
        $caps = WP_Agent_Bridge_V06_Self_Update::capabilities();
    ...
    public static function apply(array $params)
    {
        ...
        return WP_Agent_Bridge_V06_Self_Update::apply($params);
    }
    ...
    private static function validate_complete_manifest(array $params)
    {
        if (($params['full_manifest'] ?? null) !== true) {
            return new WP_Error(
                'wpab_self_update_full_manifest_required',
        ...
        foreach ($params['files'] as $entry) {
            if (!is_array($entry) || !isset($entry['path']) || !is_string($entry['path'])) {
                return new WP_Error('wpab_self_update_manifest_entry', 'Each manifest file entry must contain a path.', ['status' => 400]);
            }
            ...
            if (strtolower(pathinfo($path, PATHINFO_EXTENSION)) === 'php') {
                $raw = isset($entry['data_b64']) && is_string($entry['data_b64'])
                    ...
                    : false;
                if (!is_string($raw)) {
                    return new WP_Error('wpab_self_update_base64', 'Manifest PHP content is not valid Base64.', ['status' => 400, 'path' => $path]);
                }
        ...
        if ($missing_current !== $delete_paths) {
            return new WP_Error(
                'wpab_self_update_manifest_incomplete',
    ...
    private static function normalize_delete_paths($value)
    {
        ...
        if (!is_array($value)) {
            return new WP_Error('wpab_self_update_delete_paths', 'delete_paths must be an array.', ['status' => 400]);
        }
        $paths = [];
        foreach ($value as $path) {
            if (!is_string($path)) {
                return new WP_Error('wpab_self_update_delete_paths', 'delete_paths contains a non-string path.', ['status' => 400]);
            }
            $normalized = self::normalize_path($path);
            if ($normalized === null) {
                return new WP_Error('wpab_self_update_delete_paths', 'delete_paths contains an invalid path.', ['status' => 400, 'path' => $path]);
            }
    ...
    private static function validate_required_php_dependencies(array $decoded_php, array $submitted)
    {
        foreach ($decoded_php as $path => $code) {
            ...
            foreach ($matches[1] as $relative) {
                ...
                if ($candidate === null) {
                    return new WP_Error(
                        'wpab_self_update_required_path_invalid',
                ...
                if (!isset($submitted[$candidate])) {
                    return new WP_Error(
                        'wpab_self_update_required_file_missing',
