...
final class WP_Agent_Bridge_V08_Post_Terms
{
    ...
    {
        if (empty($params['confirm'])) return new WP_Error('wpab_confirmation_required', 'Post term changes require confirm=true.', ['status' => 400]);
        $plan = self::plan($params);
        if (is_wp_error($plan)) return $plan;
        ...
        if ($expected === '' || !hash_equals((string) $plan['impact_hash'], $expected)) {
            return new WP_Error('wpab_post_term_impact_changed', 'Post term impact changed or expected_impact_hash is missing.', ['status' => 409, 'current_impact_hash' => $plan['impact_hash']]);
        }
        ...
        if ($after !== $plan['after_term_ids']) {
            return new WP_Error('wpab_post_term_verify_failed', 'Assigned terms did not match the planned result.', ['status' => 500, 'actual_term_ids' => $after]);
        }
    ...
    private static function plan(array $params)
    {
        ...
        if (!in_array($operation, ['set', 'add', 'remove'], true)) {
            return new WP_Error('wpab_post_term_operation', 'operation must be set, add, or remove.', ['status' => 400]);
        }
        $targets = self::resolve_targets($params, $context['taxonomy']);
        if (is_wp_error($targets)) return $targets;
        if ($operation !== 'set' && !$targets) return new WP_Error('wpab_post_term_targets_required', 'add/remove requires at least one existing term.', ['status' => 400]);
    ...
    private static function context(array $params)
    {
        ...
        if (!current_user_can('edit_post', $post_id)) return new WP_Error('wpab_post_edit_forbidden', 'Connected user cannot edit this post.', ['status' => 403, 'post_id' => $post_id]);
        $taxonomy = isset($params['taxonomy']) && is_string($params['taxonomy']) ? sanitize_key($params['taxonomy']) : '';
        if ($taxonomy === '' || !taxonomy_exists($taxonomy)) return new WP_Error('wpab_taxonomy_not_found', 'Taxonomy was not found.', ['status' => 404, 'taxonomy' => $taxonomy]);
        $tax = get_taxonomy($taxonomy);
        if (!$tax || !in_array($post->post_type, (array) $tax->object_type, true)) return new WP_Error('wpab_taxonomy_post_type', 'Taxonomy is not registered for this post type.', ['status' => 409]);
        $assign_cap = isset($tax->cap->assign_terms) ? $tax->cap->assign_terms : 'edit_posts';
        if (!current_user_can($assign_cap)) return new WP_Error('wpab_taxonomy_assign_forbidden', 'Connected user cannot assign terms in this taxonomy.', ['status' => 403, 'capability' => $assign_cap]);
    ...
    private static function resolve_targets(array $params, string $taxonomy)
    {
        ...
        $terms = $params['terms'];
        if (!is_array($terms)) return new WP_Error('wpab_post_terms_array', 'terms must be an array.', ['status' => 400]);
        if (count($terms) > 200) return new WP_Error('wpab_post_terms_limit', 'Too many terms in one operation.', ['status' => 413]);
        $by = isset($params['by']) && is_string($params['by']) ? strtolower(trim($params['by'])) : 'slug';
        if (!in_array($by, ['slug', 'id'], true)) return new WP_Error('wpab_post_terms_by', 'by must be slug or id.', ['status' => 400]);
        $out = [];
        foreach ($terms as $selector) {
            if ($by === 'id') {
                ...
                if (!is_string($selector) || trim($selector) === '') return new WP_Error('wpab_post_term_slug', 'Term slugs must be non-empty strings.', ['status' => 400]);
                $term = get_term_by('slug', sanitize_title($selector), $taxonomy);
            }
            if (!$term || is_wp_error($term)) return new WP_Error('wpab_post_term_not_found', 'A requested term does not exist. Terms are never created implicitly.', ['status' => 404, 'selector' => $selector, 'by' => $by, 'taxonomy' => $taxonomy]);
