...
final class WP_Agent_Bridge_V08_Option_Delete
{
    ...
    {
        if (empty($params['confirm'])) return new WP_Error('wpab_confirmation_required', 'Option deletion requires confirm=true.', ['status' => 400]);
        ...
        if ($expected === '' || !hash_equals((string) $data['impact_hash'], $expected)) {
            return new WP_Error('wpab_option_delete_impact_changed', 'Option deletion impact changed or expected_impact_hash is missing.', ['status' => 409, 'current_impact_hash' => $data['impact_hash']]);
        }
        $deleted = delete_option((string) $data['name']);
        if (self::exists((string) $data['name'])) return new WP_Error('wpab_option_delete_failed', 'Option still exists after deletion.', ['status' => 500]);
    ...
    private static function context(array $params)
    {
        ...
        if (!self::deletable($name)) return new WP_Error('wpab_option_delete_protected', 'This option is protected from Bridge deletion.', ['status' => 403, 'name' => $name]);
        $row = $wpdb->get_row($wpdb->prepare("SELECT option_value, autoload FROM {$wpdb->options} WHERE option_name = %s LIMIT 1", $name), ARRAY_A);
        if (!is_array($row)) return new WP_Error('wpab_option_not_found', 'Option was not found.', ['status' => 404, 'name' => $name]);
