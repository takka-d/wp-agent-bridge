...
final class WP_Agent_Bridge_V06_Self_Update
{
    private const OPTION_BACKUPS = 'wpab_self_update_backups';
    private const LEGACY_BOOTSTRAP = 'wp-agent-bridge.php';
    ...
    public static function apply(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Bridge self-update requires confirm=true.', ['status' => 400]);
        }
        ...
        if ($expected !== '' && $expected !== $current) {
            return new WP_Error('wpab_self_update_version_conflict', 'Current Bridge version does not match expected_current_version.', [
                'status' => 409,
        ...
        if (empty($params['allow_same_version']) && version_compare($target, $current, '<=')) {
            return new WP_Error('wpab_self_update_not_newer', 'Target Bridge version must be newer than the installed version.', [
                'status' => 409,
                'current_version' => $current,
        ...
        if (is_wp_error($apply)) {
            ...
            if (is_wp_error($restore)) {
                return new WP_Error('wpab_self_update_and_rollback_failed', 'Self-update failed and automatic rollback also failed.', [
                    'status' => 500,
            ...
            return new WP_Error('wpab_self_update_failed_rolled_back', 'Self-update failed; the previous plugin files were restored.', [
                'status' => 500,
                'update_error' => $apply->get_error_message(),
                'backup_id' => $backup['id'],
            ]);
        ...
        if (is_wp_error($migration)) {
            $restore = self::restore_record($backup);
            return new WP_Error(
                'wpab_self_update_identity_migration_failed',
    ...
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Bridge self-update rollback requires confirm=true.', ['status' => 400]);
        }
        $backups = self::load_backups();
        if (!$backups) {
            return new WP_Error('wpab_self_update_no_backups', 'No self-update backups are available.', ['status' => 404]);
        }
        ...
        if (!is_array($selected)) {
            return new WP_Error('wpab_self_update_backup_not_found', 'Requested self-update backup was not found.', ['status' => 404]);
        }
    ...
    private static function validate_manifest(array $params)
    {
        if (!isset($params['files']) || !is_array($params['files']) || !$params['files']) {
            return new WP_Error('wpab_self_update_manifest_files', 'files must be a non-empty array.', ['status' => 400]);
        }
        if (count($params['files']) > self::MAX_FILES) {
            return new WP_Error('wpab_self_update_manifest_files', 'Manifest contains too many files.', ['status' => 413]);
        }
        ...
        if (!preg_match('/^[a-f0-9]{64}$/', $declared_manifest_sha)) {
            return new WP_Error('wpab_self_update_manifest_sha', 'manifest_sha256 must be a SHA-256 hex digest.', ['status' => 400]);
        }
        ...
        foreach ($params['files'] as $entry) {
            if (!is_array($entry)) {
                return new WP_Error('wpab_self_update_manifest_entry', 'Each manifest file entry must be an object.', ['status' => 400]);
            }
            $path = isset($entry['path']) && is_string($entry['path']) ? self::normalize_path($entry['path']) : null;
            if ($path === null) {
                return new WP_Error('wpab_self_update_path', 'Manifest contains an invalid file path.', ['status' => 400]);
            }
            if (isset($files[$path])) {
                return new WP_Error('wpab_self_update_duplicate_path', 'Manifest contains a duplicate file path.', ['status' => 400, 'path' => $path]);
            }
            if (!in_array(strtolower(pathinfo($path, PATHINFO_EXTENSION)), self::ALLOWED_EXTENSIONS, true)) {
                return new WP_Error('wpab_self_update_extension', 'Manifest contains a blocked file extension.', ['status' => 400, 'path' => $path]);
            }
            ...
            $data = isset($entry['data_b64']) && is_string($entry['data_b64'])
                ...
                : false;
            if (!is_string($data)) {
                return new WP_Error('wpab_self_update_base64', 'Manifest file content is not valid Base64.', ['status' => 400, 'path' => $path]);
            }
            $total += strlen($data);
            if ($total > self::MAX_BYTES) {
                return new WP_Error('wpab_self_update_size', 'Decoded self-update manifest exceeds the byte limit.', ['status' => 413]);
            }
            ...
            if (!preg_match('/^[a-f0-9]{64}$/', $declared_sha) || !hash_equals($declared_sha, $actual_sha)) {
                return new WP_Error('wpab_self_update_file_sha', 'Manifest file SHA-256 does not match decoded content.', [
                    'status' => 400,
                    'path' => $path,
                ]);
            }
        ...
        if (!hash_equals($declared_manifest_sha, $actual_manifest_sha)) {
            return new WP_Error('wpab_self_update_manifest_sha_mismatch', 'manifest_sha256 does not match the validated file set.', [
                'status' => 400,
        ...
        if ($version === '') {
            return new WP_Error('wpab_self_update_version_missing', 'Could not read Version from plugin bootstrap.', ['status' => 400]);
        }
        ...
        if ($declared_version !== '' && $declared_version !== $version) {
            return new WP_Error('wpab_self_update_version_mismatch', 'target_version does not match the plugin bootstrap Version header.', [
                'status' => 400,
    ...
    private static function validate_php(string $code, string $path)
    {
        if (strpos($code, '<?php') === false && strpos($code, '<?=') === false) {
            return new WP_Error('wpab_self_update_php_open_tag', 'PHP manifest file does not contain a PHP opening tag.', [
                'status' => 400,
                'path' => $path,
            ]);
        }
        try {
            ...
            return new WP_Error('wpab_self_update_php_syntax', 'PHP syntax validation failed before self-update.', [
                'status' => 400,
                'path' => $path,
                'error' => $e->getMessage(),
            ]);
    ...
    private static function capture_backup(string $reason)
    {
        ...
        foreach ($paths as $relative) {
            ...
            if (!is_string($data)) {
                return new WP_Error('wpab_self_update_backup_read', 'Could not read a plugin file for backup.', ['status' => 500, 'path' => $relative]);
            }
            $total += strlen($data);
            if ($total > self::MAX_BYTES) {
                return new WP_Error('wpab_self_update_backup_size', 'Current plugin is too large for the self-update backup limit.', ['status' => 500]);
            }
    ...
    private static function restore_record(array $backup)
    {
        if (!isset($backup['files']) || !is_array($backup['files']) || !$backup['files']) {
            return new WP_Error('wpab_self_update_backup_invalid', 'Backup record is invalid.', ['status' => 500]);
        }
        $files = [];
        foreach ($backup['files'] as $path => $data_b64) {
            ...
            if ($path === null || !is_string($data)) {
                return new WP_Error('wpab_self_update_backup_invalid', 'Backup record contains an invalid file.', ['status' => 500]);
            }
            $files[$path] = [
    ...
    private static function replace_files(array $files)
    {
        ...
        foreach ($files as $relative => $entry) {
            ...
            if (!is_dir(dirname($destination)) && !wp_mkdir_p(dirname($destination))) {
                return new WP_Error('wpab_self_update_mkdir', 'Could not create plugin directory during self-update.', ['status' => 500, 'path' => dirname($relative)]);
            }
            try {
            ...
            $temporary = $destination . '.wpab-new-' . $suffix;
            $written = @file_put_contents($temporary, $entry['data'], LOCK_EX);
            if ($written !== $entry['bytes']) {
                @unlink($temporary);
                return new WP_Error('wpab_self_update_write', 'Could not write a plugin file during self-update.', ['status' => 500, 'path' => $relative]);
            }
            @chmod($temporary, 0644);
            if (!@rename($temporary, $destination)) {
                @unlink($temporary);
                return new WP_Error('wpab_self_update_rename', 'Could not atomically replace a plugin file during self-update.', ['status' => 500, 'path' => $relative]);
            }
            clearstatcache(true, $destination);
            $verify = @hash_file('sha256', $destination);
            if (!is_string($verify) || !hash_equals($entry['sha256'], $verify)) {
                return new WP_Error('wpab_self_update_verify', 'A written plugin file failed SHA-256 verification.', ['status' => 500, 'path' => $relative]);
            }
        }
        ...
        foreach ($existing as $relative) {
            if (!isset($wanted[$relative]) && !@unlink($root . '/' . $relative)) {
                return new WP_Error('wpab_self_update_delete_stale', 'Could not delete a stale plugin file during self-update.', ['status' => 500, 'path' => $relative]);
            }
        }
        ...
        try {
            ...
            foreach ($iterator as $info) {
                ...
                if (strpos($absolute, $prefix) !== 0) {
                    return new WP_Error('wpab_self_update_path_escape', 'Plugin file escaped the expected root.', ['status' => 500]);
                }
                $relative = self::normalize_path(substr($absolute, strlen($prefix)));
                if ($relative === null) {
                    return new WP_Error('wpab_self_update_existing_path', 'Current plugin contains an unsupported path.', ['status' => 500]);
                }
                $files[] = $relative;
            }
        } catch (Throwable $e) {
            return new WP_Error('wpab_self_update_scan', 'Could not scan current plugin files.', ['status' => 500, 'error' => $e->getMessage()]);
        }
    ...
    private static function migrate_generic_identity_if_needed(array $files)
    {
        ...
        if (!isset($wpdb) || !is_object($wpdb) || !isset($wpdb->options)) {
            return new WP_Error('wpab_self_update_identity_db', 'WordPress options table is unavailable.', ['status' => 500]);
        }

        $like = method_exists($wpdb, 'esc_like') ? $wpdb->esc_like('wpab_') . '%' : 'wp-agent-bridge\_%';
        ...
        if (is_array($names)) {
            ...
            foreach ($names as $old_name) {
                if (!is_string($old_name) || strpos($old_name, 'wpab_') !== 0) {
                    continue;
                }
                if (strpos($old_name, 'wpab_') === 0) {
                    $new_name = 'wpab_' . substr($old_name, strlen('wpab_'));
                } elseif (strpos($old_name, 'wpab_direct_') === 0) {
                    $new_name = 'wpab_direct_' . substr($old_name, strlen('wpab_direct_'));
                } else {
                    $new_name = 'wpab_' . substr($old_name, strlen('wpab_'));
                }
