...
final class WP_Agent_Bridge_V05_Admin
{
    ...
    public static function menu_update(array $params)
    {
        ...
        if (!$args) {
            return new WP_Error('wpab_menu_no_changes', 'No menu fields were provided.', ['status' => 400]);
        }
    ...
    public static function menu_item_upsert(array $params)
    {
        ...
        if ($item_id > 0 && !self::menu_contains_item((int) $menu->term_id, $item_id)) {
            return new WP_Error('wpab_menu_item_not_in_menu', 'item_id does not belong to the selected menu.', ['status' => 409]);
        }
        ...
        if (!in_array($type, ['custom', 'post_type', 'taxonomy'], true)) {
            return new WP_Error('wpab_menu_item_type', 'Unsupported menu item type.', ['status' => 400]);
        }
        $args = [
        ...
        if ($type === 'custom') {
            if (!isset($params['url']) || !is_string($params['url']) || trim($params['url']) === '') {
                return new WP_Error('wpab_menu_item_url', 'Custom menu items require url.', ['status' => 400]);
            }
            $args['menu-item-url'] = esc_url_raw($params['url']);
        } else {
            if (!isset($params['object']) || !is_string($params['object']) || !isset($params['object_id'])) {
                return new WP_Error('wpab_menu_item_object', 'post_type/taxonomy menu items require object and object_id.', ['status' => 400]);
            }
    ...
    public static function menu_item_delete(array $params)
    {
        ...
        if ($item_id < 1 || !self::menu_contains_item((int) $menu->term_id, $item_id)) {
            return new WP_Error('wpab_menu_item_not_found', 'Menu item was not found in the selected menu.', ['status' => 404]);
        }
    ...
    public static function menu_locations_set(array $params)
    {
        ...
        if (!isset($params['locations']) || !is_array($params['locations'])) {
            return new WP_Error('wpab_menu_locations', 'locations must be an object/map.', ['status' => 400]);
        }
        $registered = get_registered_nav_menus();
        ...
        foreach ($params['locations'] as $location => $menu_id) {
            if (!is_string($location) || !array_key_exists($location, $registered)) {
                return new WP_Error('wpab_menu_location_unknown', 'Unknown registered menu location.', ['status' => 400, 'location' => $location]);
            }
            $menu_id = absint($menu_id);
            if ($menu_id > 0 && !wp_get_nav_menu_object($menu_id)) {
                return new WP_Error('wpab_menu_location_menu', 'Assigned menu does not exist.', ['status' => 404, 'menu_id' => $menu_id]);
            }
    ...
    private static function resolve_menu(array $params)
    {
        ...
        if ($selector === null || $selector === '' || $selector === 0) {
            return new WP_Error('wpab_menu_required', 'menu_id or menu is required.', ['status' => 400]);
        }
        $menu = wp_get_nav_menu_object($selector);
        if (!$menu) {
            return new WP_Error('wpab_menu_not_found', 'Menu was not found.', ['status' => 404]);
        }
        return $menu;
    }
