 ...
 */
final class WP_Agent_Bridge_V096_Site
{
    private const VERSION = '0.9.6';
    private const NS = 'wpab-v096/v1';
    private const ROUTE = '/wpab-v096/v1/manage';
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v096_json', 'JSON body is required.', ['status' => 400]);
        }
        $action = is_string($json['action'] ?? null) ? trim($json['action']) : '';
        $params = isset($json['params']) && is_array($json['params']) ? $json['params'] : [];
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v096_action', 'Unknown or blocked v0.9.6 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'site.icon.clear':
                    ...
                    break;
                default:
                    $response = new WP_Error('wpab_v096_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v096_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }
        ...
        if (in_array($action, ['site.icon.set', 'site.icon.clear'], true)
            && class_exists('WP_Agent_Bridge_V07_Audit')) {
    ...
    public static function set_icon(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_v096_confirmation_required', 'site.icon.set requires confirm=true.', ['status' => 400]);
        }
        $attachment_id = isset($params['attachment_id']) ? absint($params['attachment_id']) : 0;
        if ($attachment_id < 1) {
            return new WP_Error('wpab_v096_attachment_required', 'attachment_id is required.', ['status' => 400]);
        }
        ...
        if ($expected !== null && $expected !== $current) {
            return new WP_Error('wpab_v096_site_icon_changed', 'Site Icon changed after it was inspected.', [
                'status' => 409,
        ...
        if ($actual !== $attachment_id) {
            return new WP_Error('wpab_v096_site_icon_verify_failed', 'WordPress did not retain the requested Site Icon attachment ID.', [
                'status' => 500,
    ...
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_v096_confirmation_required', 'site.icon.clear requires confirm=true.', ['status' => 400]);
        }
        $current = (int) get_option('site_icon', 0);
        $expected = isset($params['expected_current_id']) ? (int) $params['expected_current_id'] : null;
        if ($expected !== null && $expected !== $current) {
        ...
        $actual = (int) get_option('site_icon', 0);
        if ($actual !== 0) {
            return new WP_Error('wpab_v096_site_icon_clear_verify_failed', 'WordPress did not clear the Site Icon option.', [
                'status' => 500,
                'actual_id' => $actual,
            ]);
        }
    ...
    private static function attachment_state(int $attachment_id)
    {
        ...
        if (!$post || $post->post_type !== 'attachment') {
            return new WP_Error('wpab_v096_attachment_not_found', 'Attachment was not found.', [
                'status' => 404,
                'attachment_id' => $attachment_id,
            ]);
        }
        if (!wp_attachment_is_image($attachment_id)) {
            return new WP_Error('wpab_v096_attachment_not_image', 'Site Icon must reference an image attachment.', [
                'status' => 400,
                'attachment_id' => $attachment_id,
        ...
        if (!is_string($file) || $file === '' || !is_file($file)) {
            return new WP_Error('wpab_v096_attachment_file_missing', 'Image attachment file is missing.', [
                'status' => 410,
                'attachment_id' => $attachment_id,
            ]);
        }
