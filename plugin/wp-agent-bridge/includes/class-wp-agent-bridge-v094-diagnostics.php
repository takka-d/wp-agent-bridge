...
final class WP_Agent_Bridge_V094_Diagnostics
{
    private const VERSION='0.9.4';
    private const NS='wpab-v094/v1';
    private const ROUTE='/wpab-v094/v1/manage';
    ...
    public static function permission(){
        if(!self::$allowed||!current_user_can('manage_options')) return new WP_Error('wpab_v094_internal_only','This route is only callable through the signed Bridge REST proxy.',['status'=>403]);
        self::$allowed=false; return true;
    }
    public static function dispatch(WP_REST_Request $request){
        $j=$request->get_json_params();
        if(!is_array($j)) return new WP_Error('wpab_v094_json','JSON body is required.',['status'=>400]);
        $a=is_string($j['action']??null)?trim($j['action']):''; $p=isset($j['params'])&&is_array($j['params'])?$j['params']:[];
        if(!in_array($a,self::ACTIONS,true)) return new WP_Error('wpab_v094_action','Unknown or blocked v0.9.4 action.',['status'=>400,'action'=>$a]);
        try{
        ...
        }catch(Throwable $e){return new WP_Error('wpab_v094_exception',$e->getMessage(),['status'=>500,'type'=>get_class($e)]);}
        return new WP_Error('wpab_v094_dispatch','Dispatch fell through.',['status'=>500]);
    }
    ...
    private static function batch(array $p){
        ...
        if(!$q) return new WP_Error('wpab_v094_batch','requests must be a non-empty array.',['status'=>400]);
        if(count($q)>self::MAX_BATCH) return new WP_Error('wpab_v094_batch_limit','Too many probes.',['status'=>413,'max'=>self::MAX_BATCH]);
    ...
    private static function probe(array $p){
        $raw=is_string($p['url']??null)?trim($p['url']):''; if($raw===''||strlen($raw)>self::MAX_URL) return new WP_Error('wpab_v094_url','url is required and must be within the size limit.',['status'=>400]);
        $url=self::same_origin($raw,null); if(is_wp_error($url)) return $url;
        $method=strtoupper((string)($p['method']??'GET')); if(!in_array($method,['GET','HEAD'],true)) return new WP_Error('wpab_v094_method','Only GET and HEAD are allowed.',['status'=>400]);
        ...
        if(isset($p['headers'])){if(!is_array($p['headers'])) return new WP_Error('wpab_v094_headers','headers must be an object.',['status'=>400]); $custom=self::headers($p['headers']); if(is_wp_error($custom)) return $custom; foreach($custom as $k=>$v)$headers[$k]=$v;}
        ...
        for($hop=0;$hop<=$maxr;$hop++){
            ...
            $resp=wp_safe_remote_request($current,$args); if(is_wp_error($resp)) return new WP_Error('wpab_v094_http',$resp->get_error_message(),['status'=>502,'url'=>$current,'inner_code'=>$resp->get_error_code()]);
            ...
            if($follow&&$status>=300&&$status<400&&$loc!==''){
                if($hop>=$maxr) return new WP_Error('wpab_v094_redirect_limit','Redirect limit reached.',['status'=>508]);
                $next=self::same_origin($loc,$current); if(is_wp_error($next)) return new WP_Error('wpab_v094_redirect_blocked','Redirect left the WordPress origin.',['status'=>400,'from'=>$current,'location'=>$loc]);
                $chain[]=['url'=>$current,'status'=>$status,'location'=>$next];$current=$next;continue;
            } break;
        }
        if(!is_array($resp)) return new WP_Error('wpab_v094_empty','HTTP probe returned no response.',['status'=>502]);
        ...
        $search=null;if(is_string($p['search']??null)&&$p['search']!==''){$needle=$p['search'];if(strlen($needle)>4096)return new WP_Error('wpab_v094_search','search exceeds 4096 bytes.',['status'=>413]);$search=self::search($body,$needle);}
    ...
    private static function media(array $p){
        ...
        if($id<1&&$url==='') return new WP_Error('wpab_v094_media_selector','attachment_id or url is required.',['status'=>400]);
        if($id>0){$post=get_post($id);if(!$post||$post->post_type!=='attachment')return new WP_Error('wpab_v094_attachment','Attachment was not found.',['status'=>404]);$path=(string)get_attached_file($id,true);if($url===''){$u=wp_get_attachment_url($id);$url=is_string($u)?$u:'';}}
        ...
        $uploads=wp_get_upload_dir();$base=(string)($uploads['basedir']??'');$safe=self::safe_file($path,$base);if(is_wp_error($safe))return $safe;$size=filesize($safe);if($size===false)return new WP_Error('wpab_v094_stat','Could not stat media file.',['status'=>500]);
    ...
    private static function profile(string $p){$ref=home_url('/');$uaA='Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Mobile Safari/537.36';$uaD='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Safari/537.36';$img='image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8';$doc='text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8';$all=['default'=>['User-Agent'=>'WP-Agent-Bridge/'.self::VERSION.'; '.home_url('/'),'Accept'=>'*/*'],'android_chrome_image'=>['User-Agent'=>$uaA,'Accept'=>$img,'Referer'=>$ref,'Sec-Fetch-Dest'=>'image','Sec-Fetch-Mode'=>'no-cors','Sec-Fetch-Site'=>'same-origin'],'android_chrome_document'=>['User-Agent'=>$uaA,'Accept'=>$doc,'Sec-Fetch-Dest'=>'document','Sec-Fetch-Mode'=>'navigate','Sec-Fetch-Site'=>'none','Sec-Fetch-User'=>'?1'],'desktop_chrome_image'=>['User-Agent'=>$uaD,'Accept'=>$img,'Referer'=>$ref,'Sec-Fetch-Dest'=>'image','Sec-Fetch-Mode'=>'no-cors','Sec-Fetch-Site'=>'same-origin'],'desktop_chrome_document'=>['User-Agent'=>$uaD,'Accept'=>$doc,'Sec-Fetch-Dest'=>'document','Sec-Fetch-Mode'=>'navigate','Sec-Fetch-Site'=>'none','Sec-Fetch-User'=>'?1']];return isset($all[$p])?$all[$p]:new WP_Error('wpab_v094_profile','Unknown HTTP profile.',['status'=>400,'profile'=>$p]);}
    private static function headers(array $in){if(count($in)>20)return new WP_Error('wpab_v094_header_count','Too many headers.',['status'=>413]);$out=[];foreach($in as $k=>$v){if(!is_string($k)||(!is_string($v)&&!is_numeric($v)))return new WP_Error('wpab_v094_header_type','Header names/values must be strings.',['status'=>400]);$l=strtolower(trim($k));if(!in_array($l,self::HEADER_ALLOW,true))return new WP_Error('wpab_v094_header_blocked','Header is not allowlisted.',['status'=>400,'header'=>$k]);$s=trim((string)$v);if(strlen($s)>2048||preg_match('/[\r\n]/',$s))return new WP_Error('wpab_v094_header_value','Invalid header value.',['status'=>400]);if($l==='referer'){$u=self::same_origin($s,null);if(is_wp_error($u))return new WP_Error('wpab_v094_referer','Referer must be same-origin.',['status'=>400]);$s=$u;}$out[implode('-',array_map('ucfirst',explode('-',$l)))]=$s;}return $out;}
    private static function same_origin(string $c,?string $base){$c=trim($c);if($c===''||strlen($c)>self::MAX_URL||preg_match('/[\r\n\0]/',$c))return new WP_Error('wpab_v094_url_invalid','Invalid URL.',['status'=>400]);$home=wp_parse_url(home_url('/'));if(!is_array($home)||empty($home['scheme'])||empty($home['host']))return new WP_Error('wpab_v094_home','Could not resolve home origin.',['status'=>500]);$origin=$home['scheme'].'://'.$home['host'].(isset($home['port'])?':'.(int)$home['port']:'');if(strpos($c,'//')===0)$c=$home['scheme'].':'.$c;elseif(strpos($c,'/')===0)$c=$origin.$c;elseif(!preg_match('#^https?://#i',$c)){if($base===null)$c=home_url('/'.ltrim($c,'/'));else{$b=wp_parse_url($base);if(!is_array($b))return new WP_Error('wpab_v094_base','Invalid base URL.',['status'=>400]);$bo=$b['scheme'].'://'.$b['host'].(isset($b['port'])?':'.(int)$b['port']:'');$dir=preg_replace('#/[^/]*$#','/',(string)($b['path']??'/'));$c=$bo.$dir.$c;}}$p=wp_parse_url($c);if(!is_array($p)||empty($p['scheme'])||empty($p['host']))return new WP_Error('wpab_v094_parse','Could not parse URL.',['status'=>400]);$scheme=strtolower((string)$p['scheme']);$hs=strtolower((string)$home['scheme']);$host=strtolower(rtrim((string)$p['host'],'.'));$hh=strtolower(rtrim((string)$home['host'],'.'));$port=isset($p['port'])?(int)$p['port']:($scheme==='https'?443:80);$hp=isset($home['port'])?(int)$home['port']:($hs==='https'?443:80);if(!in_array($scheme,['http','https'],true)||$scheme!==$hs||$host!==$hh||$port!==$hp)return new WP_Error('wpab_v094_cross_origin','Diagnostics are restricted to the WordPress site origin.',['status'=>400]);return esc_url_raw($c);}
    ...
    private static function upload_path(string $url){$u=wp_get_upload_dir();$bu=(string)($u['baseurl']??'');$bd=(string)($u['basedir']??'');$t=wp_parse_url($url);$b=wp_parse_url($bu);if($bu===''||$bd===''||!is_array($t)||!is_array($b))return new WP_Error('wpab_v094_uploads','Uploads mapping unavailable.',['status'=>500]);$tp=rawurldecode((string)($t['path']??''));$bp=rtrim(rawurldecode((string)($b['path']??'')),'/');if($bp===''||strpos($tp,$bp.'/')!==0)return new WP_Error('wpab_v094_upload_scope','URL is outside WordPress uploads.',['status'=>400]);$r=ltrim(substr($tp,strlen($bp)),'/');if($r===''||preg_match('#(^|/)\.\.(/|$)#',$r)||strpos($r,"\0")!==false)return new WP_Error('wpab_v094_upload_path','Unsafe uploads path.',['status'=>400]);return rtrim($bd,'/\\').DIRECTORY_SEPARATOR.str_replace('/',DIRECTORY_SEPARATOR,$r);}
    private static function safe_file(string $p,string $base){$br=realpath($base);$pr=realpath($p);if($br===false||$pr===false||!is_file($pr))return new WP_Error('wpab_v094_file_missing','File was not found.',['status'=>404]);$bn=rtrim(str_replace('\\','/',$br),'/').'/';$pn=str_replace('\\','/',$pr);if(strpos($pn,$bn)!==0)return new WP_Error('wpab_v094_file_scope','File is outside allowed uploads directory.',['status'=>403]);return$pr;}
