...
final class WP_Agent_Bridge_V085_Content_Batch
{
    ...
    public static function apply(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Batch post content changes require confirm=true.', ['status' => 400]);
        }
        ...
        if ($expected_before === '' || !hash_equals((string) $plan['before_sha256'], $expected_before)) {
            return new WP_Error('wpab_post_content_changed', 'Post content changed after batch preview or expected_before_sha256 is missing.', [
                'status' => 409,
        ...
        if ($expected_plan === '' || !hash_equals((string) $plan['plan_hash'], $expected_plan)) {
            return new WP_Error('wpab_post_content_batch_plan_changed', 'Batch patch plan changed after preview or expected_plan_hash is missing.', [
                'status' => 409,
        ...
        if (!$plan['changed']) {
            return new WP_Error('wpab_post_content_no_change', 'Batch patch would not change post content.', ['status' => 409]);
        }

        if (!in_array($plan['status'], ['draft', 'pending', 'auto-draft'], true) && empty($params['confirm_live'])) {
            return new WP_Error('wpab_post_content_live_confirmation_required', 'Changing non-draft post content requires confirm_live=true.', [
                'status' => 400,
        ...
        if (!$post) {
            return new WP_Error('wpab_post_not_found_after_update', 'Post disappeared after batch update.', ['status' => 500]);
        }
        ...
        if (!hash_equals((string) $plan['after_sha256'], $actual_sha)) {
            return new WP_Error('wpab_post_content_verify_failed', 'Post content did not match the planned batch result after update.', [
                'status' => 500,
    ...
    private static function plan(array $params)
    {
        ...
        if (strlen($before) > self::MAX_CONTENT_BYTES) {
            return new WP_Error('wpab_post_content_size', 'Post content is too large for Bridge batch tooling.', ['status' => 413]);
        }

        $operations = isset($params['operations']) ? $params['operations'] : null;
        if (!is_array($operations) || !$operations) {
            return new WP_Error('wpab_batch_operations_required', 'operations must be a non-empty array.', ['status' => 400]);
        }
        if (count($operations) > self::MAX_OPERATIONS) {
            return new WP_Error('wpab_batch_operations_limit', 'Too many batch operations.', ['status' => 413, 'max_operations' => self::MAX_OPERATIONS]);
        }
        ...
        foreach (array_values($operations) as $index => $operation) {
            if (!is_array($operation)) {
                return new WP_Error('wpab_batch_operation_object', 'Each batch operation must be an object.', ['status' => 400, 'index' => $index]);
            }
    ...
    private static function apply_operation(string $content, array $operation, int $index)
    {
        ...
        if (!in_array($type, ['replace', 'insert_before', 'insert_after', 'delete'], true)) {
            return new WP_Error('wpab_batch_operation_type', 'Batch operation type must be replace, insert_before, insert_after, or delete.', [
                'status' => 400,
                'index' => $index,
            ]);
        }
        ...
        if ($selector === '') {
            return new WP_Error('wpab_batch_selector_required', $selector_key . ' must be a non-empty string.', ['status' => 400, 'index' => $index]);
        }
        if (strlen($selector) > self::MAX_FRAGMENT_BYTES) {
            return new WP_Error('wpab_batch_selector_size', 'Batch selector exceeds the fragment size limit.', ['status' => 413, 'index' => $index]);
        }
        ...
        if (strlen($payload) > self::MAX_FRAGMENT_BYTES) {
            return new WP_Error('wpab_batch_payload_size', 'Batch payload exceeds the fragment size limit.', ['status' => 413, 'index' => $index]);
        }
        if (in_array($type, ['insert_before', 'insert_after'], true) && $payload === '') {
            return new WP_Error('wpab_batch_insert_empty', 'Insert content must be non-empty.', ['status' => 400, 'index' => $index]);
        }
        ...
        if (!$replace_all && $expected_matches !== 1) {
            return new WP_Error('wpab_batch_unique_required', 'Single-operation mode requires expected_matches=1. Use a more specific selector or replace_all=true.', [
                'status' => 400,
                'index' => $index,
            ]);
        }
        ...
        if ($actual_matches !== $expected_matches) {
            return new WP_Error('wpab_batch_match_count', 'Current exact match count differs from expected_matches.', [
                'status' => 409,
                'index' => $index,
                'type' => $type,
        ...
        if ($after === $content) {
            return new WP_Error('wpab_batch_operation_no_change', 'A batch operation would not change content.', ['status' => 409, 'index' => $index]);
        }
        if (strlen($after) > self::MAX_CONTENT_BYTES) {
            return new WP_Error('wpab_post_content_size', 'Batch operation would exceed the content size limit.', ['status' => 413, 'index' => $index]);
        }
