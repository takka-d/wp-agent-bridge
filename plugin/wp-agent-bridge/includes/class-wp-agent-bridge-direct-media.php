 ...
 */
final class WP_Agent_Bridge_Direct_Media
{
    ...
    public static function upload(WP_REST_Request $request)
    {
        ...
        if ($installation_id < 1 || $repository_id < 1 || $repository === '' || $branch !== WP_Agent_Bridge_Direct_Runtime::RUNTIME_BRANCH) {
    ...
    private static function cleanup_sources_atomic(string $token, string $repository, string $branch, array $sources)
    {
        ...
        for ($attempt = 1; $attempt <= self::CLEANUP_RETRIES; $attempt++) {
            ...
            foreach ($sources as $source) {
                ...
                $current = WP_Agent_Bridge_Direct_GitHub::get_content_metadata($token, $repository, $head, $path);
            ...
            $new_commit = WP_Agent_Bridge_Direct_GitHub::github_api(
                'POST',
            ...
            $update = WP_Agent_Bridge_Direct_GitHub::github_api(
                'PATCH',
    ...
    private static function read_runtime_file(string $token, string $repository, string $ref, string $path)
    {
        ...
        if ($encoded === '') {
            $blob = WP_Agent_Bridge_Direct_GitHub::github_api('GET', '/repos/' . $repository . '/git/blobs/' . $sha, $token);
            if (is_wp_error($blob)) {
                return $blob;
            }
