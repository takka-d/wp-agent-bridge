...
final class WP_Agent_Bridge_V087_Table_Locator
{
    ...
    public static function locate(array $params)
    {
        ...
        if ($query === '') {
            return new WP_Error('wpab_table_locator_query', 'query must be a non-empty string.', ['status' => 400]);
        }
        if (self::text_length($query) > self::MAX_QUERY_CHARS) {
            return new WP_Error('wpab_table_locator_query_size', 'query exceeds the locator size limit.', ['status' => 413]);
        }
        ...
        if (!in_array($match_mode, ['exact', 'contains'], true)) {
            return new WP_Error('wpab_table_locator_match_mode', 'match_mode must be exact or contains.', ['status' => 400]);
        }
        ...
        if ($column_filter !== null && ($column_filter < 0 || $column_filter >= self::MAX_CELLS)) {
            return new WP_Error('wpab_table_locator_column', 'column is out of range.', ['status' => 400]);
        }
    ...
    public static function smart_preview(array $params)
    {
        ...
        $response = WP_Agent_Bridge_V086_Table::preview($resolved['params']);
    ...
    public static function smart_apply(array $params)
    {
        $resolved = self::resolve_smart_params($params);
        if (is_wp_error($resolved)) return $resolved;
        $response = WP_Agent_Bridge_V086_Table::apply($resolved['params']);
        return self::decorate_smart_response($response, $resolved);
    }
    ...
    private static function resolve_smart_params(array $params)
    {
        ...
        if ($first_key === '') {
            return new WP_Error('wpab_table_row_key', 'The first operation must contain a non-empty row_key.', ['status' => 400, 'index' => 0]);
        }
    ...
    {
        return new WP_Error('wpab_table_smart_target_ambiguous', $message, [
            'status' => 409,
            'index' => $index,
