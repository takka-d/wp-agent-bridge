...
{
    private const OPTION_SYNCED_VERSION = 'wpab_runtime_guidance_synced_version';
    private const TRANSIENT_LOCK = 'wpab_runtime_guidance_sync_lock';
    ...
    public static function sync()
    {
        ...
        foreach ([
            ...
            $current = WP_Agent_Bridge_Direct_GitHub::get_text_file($token, $repository, $branch, $path);
            ...
            if (is_wp_error($current)
                && WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($current) !== 404) {
                return $current;
            }
    ...
    public static function client_prompt(string $repository, string $branch, string $host): string
    {
        return "WordPress操作には、GitHubプラグイン経由でサイト {$host} のWP Agent Bridgeを使用してください。\n"
            ...
            . "このrepository名とbranchは確定済みです。repository検索や別repositoryの推測は不要です。GitHubのfile read/writeはref/branchを省略するとdefault branchを見る場合があるため、runtimeの読み書きでは必ず branch/ref={$branch} を明示してください。\n"
            . "repositoryとbranchはこの指示で確定済みです。通常のWordPress依頼では wordpress-bridge/RUNTIME_CONNECTION.json の事前readを開始条件にしないでください。GitHubのwrite action(create_file等)が利用可能なら、依頼内容に応じたコマンドを branch={$branch} の wordpress-bridge/commands/pending/<id>.json へ直接作成してください。AGENTS.md、wordpress-bridge/RUNTIME_CAPABILITIES.json、RUNTIME_CONNECTION.json の再読は、接続変更の兆候、既知仕様との矛盾、または実際のcommand publication失敗がある場合に限定してください。\n"
            . "RUNTIME_CONNECTION.jsonやbootstrap fileのreadが404でも、それだけでBridge使用不能、変更不能、変更未適用とは判断しないでください。read失敗とwrite可否は別です。具体的なWordPress依頼がありwrite actionが見えている場合は、既知のrepository/branchを使って wordpress-bridge/commands/pending/<id>.json へのrequested command publicationを優先してください。実際のpending writeまで失敗した場合にだけrepository/path/refや接続状態を再確認してください。default branchへcommandを書いてはいけません。\n"
            ...
            . "具体的なWordPress操作がこの指示と併記されている場合は、接続確認だけで止まらず、その依頼を同じ応答で実行してください。接続確認だけを明示的に求められた場合に限り変更を行わないでください。";
    }
    ...
    public static function agents(string $repository, string $branch, string $host, string $version): string
    {
        return "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "## Mandatory fast path\n\n"
            . "1. Every runtime GitHub read/write must use repository `{$repository}` and explicitly set branch/ref `{$branch}`. File readers commonly fall back to the repository default branch when ref is omitted. Never write pending commands to the default branch.\n"
            . "2. The repository/branch binding above is sufficient for normal WordPress work. Do not make `wordpress-bridge/RUNTIME_CONNECTION.json` a startup prerequisite or mandatory preflight read. For a concrete user request, use an available GitHub write action to publish the requested command to `wordpress-bridge/commands/pending/<id>.json` directly.\n"
            . "3. Read the canonical marker only after an explicit reconnect/migration signal, a mapping mismatch, an actual pending-command publication failure, or another concrete contradiction. A marker/bootstrap read 404 alone is not evidence that the Bridge is unavailable or that a requested change is unapplied.\n"
            . "4. Read `wordpress-bridge/RUNTIME_CAPABILITIES.json` before probing Bridge capabilities or searching source code for known routes/actions. Read it once per verified connection/version and reuse its operation catalog. Refresh after a version/connection change or a contradictory response, not before every command.\n"
            . "5. For advertised common operations use `type=operation` with `operation` and `params`. The runtime selects `/wpab-v099/v1/operate` internally. Do not reconstruct versioned routes when an operation is listed in the catalog.\n"
            . "6. Never put `?query=...` in a REST `route` field. Put query parameters in the `query` object or in the deterministic operation params.\n"
            . "7. Do not ask the user to identify repositories, files, routes, branches, or installed state that connected tools can inspect directly.\n"
            . "8. Do not switch normal WordPress work to WPVibe, `legacy operator-owned runtime repository`, old Bridge Key/queue, an operator relay, temporary PHP installers, or a per-command GitHub Actions worker. GitHub Actions is for source CI/package/release, not the Direct Runtime command path.\n\n"
            ...
            . "- Local/conversation media: always use `wordpress-bridge/prepare-media.py` connector-safe chunk commands. The preparer splits the original bytes into 8192-byte decoded chunks so each pending-command JSON stays below 16384 bytes. Every chunk command carries whole-file `expected_bytes`/`expected_sha256` plus per-chunk byte/SHA-256 integrity. Do not use one `media.upload.inline` command for a conversation attachment, even below 1 MiB, because client/tool payload truncation can corrupt Base64 before WordPress receives it.\n"
            ...
            . "## Failure rules\n\n"
            . "- Bootstrap/marker 404 is not a startup blocker. Do not infer Bridge unavailability, write failure, or an unapplied change from that read alone. If a concrete user-requested WordPress operation is pending and a GitHub write action is available, publish that requested command to `wordpress-bridge/commands/pending/<id>.json` on `{$branch}` first. Re-check marker/bootstrap only after an actual command-publication failure or another reconnect/migration signal; do not create a no-op command merely to test transport.\n"            . "- Keep one command ID and identical payload when checking/recovering an uncertain outcome. A new ID means a new operation; do not generate a fresh ID to retry an unconfirmed mutation.\n"
    ...
    public static function runtime(string $repository, string $branch, string $host, string $version): string
    {
        return "# WP Agent Bridge Direct Runtime\n\n"
            ...
            . "Repository: `{$repository}`\n\n"
            . "Branch: `{$branch}`\n\n"
            . "Runtime clients must explicitly pass this branch/ref on every GitHub read and write. Omitting ref may read the repository default branch, whose mirrored bootstrap files are read-only discovery aids. The repository/branch binding in this document is authoritative for normal work: do not require a RUNTIME_CONNECTION.json preflight read before publishing a requested pending command.\n\n"
            ...
            . "Machine-readable routing contract: `wordpress-bridge/RUNTIME_CAPABILITIES.json`. Common operations use `/wpab-v099/v1/operate`; its policy layer bounds post reads, blocks unguarded content replacement, validates the small-media path, and normalizes read-only batch aliases.\n\n"
