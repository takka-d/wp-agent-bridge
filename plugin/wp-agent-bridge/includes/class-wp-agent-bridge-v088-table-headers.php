...
final class WP_Agent_Bridge_V088_Table_Headers
{
    ...
    public static function preview(array $params)
    {
        ...
        $response = WP_Agent_Bridge_V087_Table_Locator::smart_preview($resolved['params']);
    ...
    public static function apply(array $params)
    {
        $resolved = self::resolve($params);
        if (is_wp_error($resolved)) return $resolved;
        $response = WP_Agent_Bridge_V087_Table_Locator::smart_apply($resolved['params']);
        return self::decorate($response, $resolved);
    }
    ...
    private static function resolve(array $params)
    {
        ...
        if (!is_array($first)) {
            return new WP_Error('wpab_table_operation_object', 'Each table operation must be an object.', ['status' => 400, 'index' => 0]);
        }
        ...
        foreach (array_values($operations) as $index => $operation) {
            ...
            if (isset($copy['key_header'])) {
                if (!is_string($copy['key_header']) || trim($copy['key_header']) === '') {
                    return new WP_Error('wpab_table_key_header', 'key_header must be a non-empty string.', ['status' => 400, 'index' => $index]);
                }
                ...
                if (array_key_exists('key_column', $copy) && (int) $copy['key_column'] !== $key_column) {
                    return new WP_Error('wpab_table_header_column_conflict', 'key_header and key_column resolve to different columns.', ['status' => 409, 'index' => $index]);
                }
            ...
            if ($type === 'set_cell') {
                if (isset($copy['column_header'])) {
                    if (!is_string($copy['column_header']) || trim($copy['column_header']) === '') {
                        return new WP_Error('wpab_table_column_header', 'column_header must be a non-empty string.', ['status' => 400, 'index' => $index]);
                    }
                    ...
                    if (array_key_exists('column', $copy) && (int) $copy['column'] !== $column) {
                        return new WP_Error('wpab_table_header_column_conflict', 'column_header and column resolve to different columns.', ['status' => 409, 'index' => $index]);
                    }
                    $copy['column'] = $column;
                ...
                if (isset($copy['cells_by_header'])) {
                    if (isset($copy['cells'])) {
                        return new WP_Error('wpab_table_header_cells_conflict', 'Use either cells or cells_by_header, not both.', ['status' => 400, 'index' => $index]);
                    }
                    if (!is_array($copy['cells_by_header']) || !$copy['cells_by_header']) {
                        return new WP_Error('wpab_table_cells', 'cells_by_header must be a non-empty object.', ['status' => 400, 'index' => $index]);
                    }
                    $numeric = [];
                    foreach ($copy['cells_by_header'] as $name => $spec) {
                        if (!is_string($name) || trim($name) === '') {
                            return new WP_Error('wpab_table_column_header', 'cells_by_header keys must be non-empty header names.', ['status' => 400, 'index' => $index]);
                        }
                ...
                if (isset($copy['cells_by_header'])) {
                    ...
                    if (count($filled) !== count($header['headers'])) {
                        ...
                        return new WP_Error('wpab_table_insert_headers_incomplete', 'Inserted row must provide every header column.', [
                            'status' => 409,
                            'index' => $index,
    ...
    private static function resolve_table_index(array $params, array $tables, array $first)
    {
        ...
        if ($row_key === '') {
            return new WP_Error('wpab_table_row_key', 'The first operation must contain a non-empty row_key when table_index is omitted.', ['status' => 400, 'index' => 0]);
        }
        $case_sensitive = array_key_exists('case_sensitive', $first) ? !empty($first['case_sensitive']) : true;

        $located = WP_Agent_Bridge_V087_Table_Locator::locate([
        ...
        if (count($matches) !== 1) {
            ...
            return new WP_Error('wpab_table_header_target_ambiguous', 'Could not resolve a unique table from the first row_key.', [
                'status' => 409,
    ...
    private static function detect_header(array $rows, array $params, bool $required)
    {
        if (array_key_exists('header_row_index', $params)) {
            ...
            if ($row_index < 0 || !isset($rows[$row_index])) {
                return new WP_Error('wpab_table_header_row', 'header_row_index is out of range.', ['status' => 404, 'row_count' => count($rows)]);
            }
        } else {
            $row_index = null;
            ...
            if ($row_index === null) {
                ...
                return new WP_Error('wpab_table_header_not_found', 'No table header row containing <th> cells was found. Supply header_row_index explicitly for non-standard tables.', ['status' => 409]);
            }
        }
        ...
        if (!$headers) {
            return new WP_Error('wpab_table_header_empty', 'Resolved header row has no cells.', ['status' => 409]);
        }
        return [
            'row_index' => $row_index,
            'headers' => $headers,
    ...
    private static function resolve_header_column(array $headers, string $name, bool $case_sensitive, int $operation_index, string $field, bool $strict = true)
    {
        ...
        if (count($matches) !== 1) {
            return new WP_Error('wpab_table_header_match', 'Header name must identify exactly one column.', [
