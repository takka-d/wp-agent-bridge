 ...
 */
final class WP_Agent_Bridge_V098_Read_Batch
{
    private const VERSION = '0.9.8.1';
    private const NS = 'wpab-v098/v1';
    private const ROUTE = '/wpab-v098/v1/manage';
    ...
    private const DIRECT_ACTION_ROUTES = [
        'v04.capabilities' => '/wp-agent-bridge/v1/manage',
        'plugin.list' => '/wp-agent-bridge/v1/manage',
        'theme.manage.list' => '/wp-agent-bridge/v1/manage',
        'cron.schedules' => '/wp-agent-bridge/v1/manage',
        'admin.capabilities' => '/wp-agent-bridge/v1/manage',
        'v05.capabilities' => '/wp-agent-bridge/v1/v05',
        'idempotency.status' => '/wp-agent-bridge/v1/v05',
        'menu.list' => '/wp-agent-bridge/v1/v05',
        'menu.get' => '/wp-agent-bridge/v1/v05',
        'updates.status' => '/wp-agent-bridge/v1/v05',
        'v06.capabilities' => '/wp-agent-bridge/v1/v06',
        'bridge.self_update.status' => '/wp-agent-bridge/v1/v06',
    ];

    /** Internal REST surfaces callable only through the signed Bridge proxy. */
    private const REST_ACTION_ROUTES = [
        'post.content.inspect' => '/wpab-v084/v1/manage',
        'post.content.search' => '/wpab-v084/v1/manage',
        'post.content.read.range' => '/wpab-v084/v1/manage',
        'v097.capabilities' => '/wpab-v097/v1/manage',
        'workspace.list' => '/wpab-v097/v1/manage',
        'workspace.file.get' => '/wpab-v097/v1/manage',
        'workspace.file.read.range' => '/wpab-v097/v1/manage',
        'workspace.file.search' => '/wpab-v097/v1/manage',
        'workspace.file.diff' => '/wpab-v097/v1/manage',
        'workspace.snapshot.list' => '/wpab-v097/v1/manage',
        'theme.files.list' => '/wpab-v096/v1/theme-files',
        'theme.files.search' => '/wpab-v096/v1/theme-files',
        'theme.file.read.many' => '/wpab-v096/v1/theme-files',
        'theme.file.outline' => '/wpab-v095/v1/manage',
        'theme.file.read.range' => '/wpab-v095/v1/manage',
        'page.html.inspect' => '/wpab-v095/v1/manage',
        'media.file.inspect' => '/wpab-v094/v1/manage',
        'site.icon.get' => '/wpab-v096/v1/manage',
        'media.upload.capabilities' => '/wpab-v096/v1/manage',
    ];
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v098_json', 'JSON body is required.', ['status' => 400]);
        }
        $action = is_string($json['action'] ?? null) ? trim($json['action']) : '';
        $params = isset($json['params']) && is_array($json['params']) ? $json['params'] : [];
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v098_action', 'Unknown or blocked v0.9.8 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
    ...
    private static function readonly_batch(array $params)
    {
        ...
        if (!$operations || count($operations) > self::MAX_OPERATIONS) {
            return new WP_Error('wpab_read_batch_operations', 'operations must contain between 1 and ' . self::MAX_OPERATIONS . ' items.', [
                'status' => 400,
                'max_operations' => self::MAX_OPERATIONS,
            ]);
        }
    ...
    private static function validate_operation($operation, int $index)
    {
        if (!is_array($operation)) {
            return new WP_Error('wpab_read_batch_operation', 'Each operation must be an object.', [
                'status' => 400,
                'index' => $index,
            ]);
        }
        ...
        if ($action === ''
            ...
            return new WP_Error('wpab_read_batch_blocked_action', 'Operation action is not on the strict read-only allowlist.', [
                'status' => 400,
                'index' => $index,
                'action' => $action,
            ]);
        ...
        if (!is_string($params_json) || strlen($params_json) > self::MAX_OPERATION_PARAMS_BYTES) {
            return new WP_Error('wpab_read_batch_params', 'Operation params exceed the per-operation size limit.', [
                'status' => 413,
                'index' => $index,
        ...
        if (array_key_exists('label', $operation)) {
            if (!is_string($operation['label']) || strlen($operation['label']) > 80 || preg_match('/[\x00-\x1F\x7F]/', $operation['label'])) {
                return new WP_Error('wpab_read_batch_label', 'Operation label is invalid.', [
                    'status' => 400,
                    'index' => $index,
                ]);
            }
