 ...
 */
final class WP_Agent_Bridge_Runtime_Bootstrap
{
    private const OPTION_SYNCED_SIGNATURE = 'wpab_runtime_bootstrap_synced_signature';
    private const TRANSIENT_LOCK = 'wpab_runtime_bootstrap_sync_lock';
    ...
    public static function sync()
    {
        ...
        $repo = WP_Agent_Bridge_Direct_GitHub::github_api('GET', '/repos/' . $repository, $token);
        if (is_wp_error($repo)) {
            return $repo;
        }
        ...
        if (class_exists('WP_Agent_Bridge_Post_Concurrency_Runtime_Guidance')) {
            $agents = WP_Agent_Bridge_Post_Concurrency_Runtime_Guidance::enrich_agents($agents);
        }
        if (class_exists('WP_Agent_Bridge_Post_Reliability_Runtime_Guidance')) {
            $agents = WP_Agent_Bridge_Post_Reliability_Runtime_Guidance::enrich_agents($agents);
        }
        ...
        if (class_exists('WP_Agent_Bridge_Post_Concurrency_Runtime_Guidance')) {
            $catalog = WP_Agent_Bridge_Post_Concurrency_Runtime_Guidance::enrich_capabilities($catalog);
        }
        if (class_exists('WP_Agent_Bridge_Post_Reliability_Runtime_Guidance')) {
            $catalog = WP_Agent_Bridge_Post_Reliability_Runtime_Guidance::enrich_capabilities($catalog);
        }
        ...
        $marker = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            $runtime_branch,
    ...
    private static function sync_file(
        ...
        } elseif (WP_Agent_Bridge_Direct_GitHub_Recovery::error_status($current) !== 404) {
            return $current;
        }
