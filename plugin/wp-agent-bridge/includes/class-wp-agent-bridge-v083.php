...
final class WP_Agent_Bridge_V083
{
    private const VERSION = '0.8.3';
    private const INTERNAL_NAMESPACE = 'wpab-v083/v1';
    private const INTERNAL_ROUTE = '/wpab-v083/v1/manage';
    ...
    public static function block_legacy_post_meta_actions($result, WP_REST_Server $server, WP_REST_Request $request)
    {
        ...
        return new WP_REST_Response([
            'code' => 'wpab_legacy_post_meta_action_disabled',
            'message' => 'Legacy post_meta.* actions are disabled. Use the protected post.meta.* actions through /wpab-v083/v1/manage.',
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v083_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v083_json', 'JSON body is required.', ['status' => 400]);
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v083_unknown_action', 'Unknown or blocked v0.8.3 action.', ['status' => 400, 'action' => $action]);
        }

        try {
            $response = self::run($action, $params);
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v083_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
    ...
    private static function run(string $action, array $params)
    {
        switch ($action) {
            ...
            case 'post.meta.list': return WP_Agent_Bridge_V083_Post_Meta::list($params);
            case 'post.meta.get': return WP_Agent_Bridge_V083_Post_Meta::get($params);
            case 'post.meta.add': return WP_Agent_Bridge_V083_Post_Meta::add($params);
            case 'post.meta.update': return WP_Agent_Bridge_V083_Post_Meta::update($params);
            case 'post.meta.delete': return WP_Agent_Bridge_V083_Post_Meta::delete($params);
        }
        return new WP_Error('wpab_v083_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }
