...
final class WP_Agent_Bridge_Direct_Runtime
{
    ...
    private const OPTION_CONNECTION = 'wpab_direct_connection_v1';
    private const OPTION_BRIDGE_SECRET = 'wpab_secret';
    private const MAX_COMMANDS_PER_PUSH = 20;
    private const MAX_COMMAND_BYTES = 2097152;
    private const DELIVERY_PREFIX = 'wpab_direct_delivery_';
    private const COMMAND_INFLIGHT_PREFIX = 'wpab_direct_command_inflight_';
    private const COMMAND_INFLIGHT_STALE_SECONDS = 600;
    private const COMMAND_JOURNAL_PREFIX = 'wpab_direct_command_journal_';
    ...
    public static function webhook(WP_REST_Request $request)
    {
        ...
        if (!WP_Agent_Bridge_Direct_GitHub::verify_webhook($raw, $signature)) {
            return new WP_Error('wpab_direct_webhook_signature', 'Invalid GitHub webhook signature.', ['status' => 401]);
        }
        ...
        try {
            ...
            return new WP_Error('wpab_direct_webhook_exception', $e->getMessage(), ['status' => 500]);
        }
    }
    ...
    private static function process_push(string $raw)
    {
        ...
        if (!is_array($payload)) {
            return new WP_Error('wpab_direct_webhook_json', 'Invalid webhook JSON.', ['status' => 400]);
        }
        ...
        if (!$private
            ...
            return new WP_Error('wpab_direct_webhook_mapping', 'Webhook repository does not match this WordPress connection.', ['status' => 404]);
        }
        ...
        if (count($paths) > self::MAX_COMMANDS_PER_PUSH) {
            return new WP_Error('wpab_direct_webhook_too_many', 'Too many pending commands in one push.', ['status' => 413]);
        }
    ...
    private static function process_command(string $token, string $repository, string $ref, string $path): array
    {
        ...
        $meta = WP_Agent_Bridge_Direct_GitHub::get_content_metadata($token, $repository, $ref, $path);
        if (is_wp_error($meta)) {
    ...
    private static function finalize_command_atomic(
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $expected_pending_sha)) {
            return new WP_Error('wpab_direct_bookkeeping_pending_sha', 'Pending command SHA is invalid.', ['status' => 409]);
        }
        ...
        for ($attempt = 1; $attempt <= self::BOOKKEEPING_MAX_ATTEMPTS; $attempt++) {
            ...
            if (!preg_match('/^[a-f0-9]{40,64}$/', $head_sha)) {
                return new WP_Error('wpab_direct_bookkeeping_head', 'GitHub did not return a valid runtime branch head.', ['status' => 502]);
            }

            $base_commit = WP_Agent_Bridge_Direct_GitHub::github_api(
                'GET',
            ...
            if (!preg_match('/^[a-f0-9]{40,64}$/', $base_tree_sha)) {
                return new WP_Error('wpab_direct_bookkeeping_tree', 'GitHub did not return a valid runtime base tree.', ['status' => 502]);
            }
            ...
            $pending_meta = WP_Agent_Bridge_Direct_GitHub::get_content_metadata(
                $token,
                $repository,
                $head_sha,
                $pending_path
            ...
            if (!preg_match('/^[a-f0-9]{40,64}$/', $current_pending_sha)
                ...
                return new WP_Error(
                    'wpab_direct_bookkeeping_pending_conflict',
            ...
            if (!preg_match('/^[a-f0-9]{40,64}$/', $tree_sha)) {
                return new WP_Error('wpab_direct_bookkeeping_tree', 'GitHub did not return a valid bookkeeping tree SHA.', ['status' => 502]);
            }
            ...
            if (!preg_match('/^[a-f0-9]{40,64}$/', $commit_sha)) {
                return new WP_Error('wpab_direct_bookkeeping_commit', 'GitHub did not return a valid bookkeeping commit SHA.', ['status' => 502]);
            }

            $updated = WP_Agent_Bridge_Direct_GitHub::github_api(
                'PATCH',
        ...
        return new WP_Error(
            'wpab_direct_bookkeeping_conflict',
    ...
    private static function create_git_blob(string $token, string $repository, string $content)
    {
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $sha)) {
            return new WP_Error('wpab_direct_bookkeeping_blob', 'GitHub did not return a valid bookkeeping blob SHA.', ['status' => 502]);
        }
        return $sha;
    }
    ...
    private static function verify_atomic_finalization(
        ...
        $stored_result = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            self::RUNTIME_BRANCH,
            $result_path
        ...
        $stored_completed = WP_Agent_Bridge_Direct_GitHub::get_text_file(
            $token,
            $repository,
            self::RUNTIME_BRANCH,
            $completed_path
        ...
        $pending = WP_Agent_Bridge_Direct_GitHub::get_content_metadata(
            $token,
            $repository,
            self::RUNTIME_BRANCH,
            $pending_path
    ...
    private static function execute_command(array $command, string $request_id): array
    {
        ...
        if ($type === 'health') {
            return self::local_bridge_request('GET', '/wp-agent-bridge/v1/health', null, $request_id, false);
        }
        if ($type === 'rest') {
            $method = strtoupper((string) ($command['method'] ?? 'GET'));
            $route = isset($command['route']) ? (string) $command['route'] : '';
            if ($route === '/wp-agent-bridge/v1/execute' || strpos($route, '?') !== false || strpos($route, '#') !== false) {
                $message = $route === '/wp-agent-bridge/v1/execute'
            ...
            $result = self::local_bridge_request('POST', '/wp-agent-bridge/v1/execute', [
                'action' => 'rest.call',
                'params' => $params,
            ], $request_id, true);
            if ($route === '/wpab-v099/v1/operate') {
                $wrapper = $result['data'] ?? [];
        ...
        if ($type === 'bridge') {
            ...
            if (in_array($action, self::V06_ACTIONS, true)) {
                return self::action_request('/wp-agent-bridge/v1/v06', $action, $params, $request_id);
            }
            if (in_array($action, self::V05_ACTIONS, true)) {
                return self::action_request('/wp-agent-bridge/v1/v05', $action, $params, $request_id);
            }
            if (in_array($action, self::V04_ACTIONS, true)) {
                return self::action_request('/wp-agent-bridge/v1/manage', $action, $params, $request_id);
            }
            return self::local_bridge_request('POST', '/wp-agent-bridge/v1/execute', [
                'action' => $action,
                'params' => $params,
            ], $request_id, true);
        }
    ...
    private static function load_command_journal(string $request_id, string $id, string $command_sha256)
    {
        ...
        if (!preg_match('/^[a-f0-9]{64}$/', $stored_sha) || !hash_equals($stored_sha, $command_sha256)) {
            return new WP_Error(
                'wpab_direct_command_journal_conflict',
        ...
        if (!hash_equals((string) ($current['id'] ?? ''), $id)) {
            return new WP_Error(
                'wpab_direct_command_journal_id_conflict',
        ...
        if ($result_json === '' || strlen($result_json) > self::MAX_COMMAND_JOURNAL_BYTES) {
            return new WP_Error('wpab_direct_command_journal_invalid', 'Stored Direct Runtime execution journal is invalid.', ['status' => 500]);
        }
        $output = json_decode($result_json, true);
        if (!is_array($output)
            || !hash_equals((string) ($output['id'] ?? ''), $id)
            || !hash_equals((string) ($output['request_id'] ?? ''), $request_id)) {
            return new WP_Error('wpab_direct_command_journal_invalid', 'Stored Direct Runtime execution journal does not match this command.', ['status' => 500]);
        }
    ...
    private static function store_command_journal(string $request_id, string $id, string $command_sha256, string $result_json)
    {
        if (strlen($result_json) < 1 || strlen($result_json) > self::MAX_COMMAND_JOURNAL_BYTES) {
            return new WP_Error('wpab_direct_command_journal_size', 'Direct Runtime result is too large for local recovery journaling.', ['status' => 500]);
        }
        $option = self::command_journal_option($request_id);
        $value = [
            'id' => $id,
        ...
        if (!is_array($verify)
            ...
            return new WP_Error('wpab_direct_command_journal_store', 'Could not persist Direct Runtime execution journal before GitHub bookkeeping.', ['status' => 500]);
        }
        return true;
    }
