...
final class WP_Agent_Bridge_V08
{
    private const VERSION = '0.8.0';
    private const INTERNAL_NAMESPACE = 'wpab-v08/v1';
    private const INTERNAL_ROUTE = '/wpab-v08/v1/manage';
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v08_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v08_json', 'JSON body is required.', ['status' => 400]);
        $action = isset($json['action']) && is_string($json['action']) ? trim($json['action']) : '';
        $params = isset($json['params']) && is_array($json['params']) ? $json['params'] : [];
        if (!in_array($action, self::ACTIONS, true)) return new WP_Error('wpab_v08_unknown_action', 'Unknown or blocked v0.8 action.', ['status' => 400, 'action' => $action]);
        try {
            $response = self::run($action, $params);
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v08_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        if (in_array($action, self::AUDITED, true)) WP_Agent_Bridge_V07_Audit::record(self::$request_id, $action, $params, $response);
        return $response;
    }
    ...
    private static function run(string $action, array $params)
    {
        switch ($action) {
            ...
            case 'role.list': return rest_ensure_response(WP_Agent_Bridge_V08_Roles::role_list());
            case 'role.get': return WP_Agent_Bridge_V08_Roles::role_get($params);
            case 'role.create': return WP_Agent_Bridge_V08_Roles::role_create($params);
            case 'role.delete.preview': return WP_Agent_Bridge_V08_Roles::role_delete_preview($params);
            case 'role.delete': return WP_Agent_Bridge_V08_Roles::role_delete($params);
            case 'role.cap.add': return WP_Agent_Bridge_V08_Roles::role_cap_add($params);
            case 'role.cap.remove': return WP_Agent_Bridge_V08_Roles::role_cap_remove($params);
            case 'user.cap.list': return WP_Agent_Bridge_V08_Roles::user_cap_list($params);
            case 'user.cap.add': return WP_Agent_Bridge_V08_Roles::user_cap_add($params);
            case 'user.cap.remove': return WP_Agent_Bridge_V08_Roles::user_cap_remove($params);
            case 'option.safe.list': return rest_ensure_response(WP_Agent_Bridge_V08_Options::list($params));
            case 'option.safe.get': return WP_Agent_Bridge_V08_Options::get($params);
            case 'option.safe.pluck': return WP_Agent_Bridge_V08_Options::pluck($params);
            case 'option.safe.patch.preview': return WP_Agent_Bridge_V08_Options::patch_preview($params);
            case 'option.safe.patch': return WP_Agent_Bridge_V08_Options::patch($params);
            case 'option.safe.delete.preview': return WP_Agent_Bridge_V08_Option_Delete::preview($params);
            case 'option.safe.delete': return WP_Agent_Bridge_V08_Option_Delete::delete($params);
            case 'post.term.list': return WP_Agent_Bridge_V08_Post_Terms::list($params);
            case 'post.term.preview': return WP_Agent_Bridge_V08_Post_Terms::preview($params);
            case 'post.term.apply': return WP_Agent_Bridge_V08_Post_Terms::apply($params);
        }
        return new WP_Error('wpab_v08_dispatch', 'Dispatch fell through.', ['status' => 500]);
    }
