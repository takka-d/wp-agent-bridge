        ...
        return [
            ...
            'command_contract' => [
                ...
                'templates' => [
                    ...
                    'media_chunk_upload' => [
                        'type' => 'rest',
                        'method' => 'POST',
                        'route' => '/wp-agent-bridge-media/v1/upload-chunk',
                        'body' => [
                            'upload_id' => '<stable upload id>',
                            'chunk_index' => '<0-based index>',
                            'chunk_count' => '<total chunks>',
                            'filename' => '<name>',
                            'expected_bytes' => '<original byte count>',
                            'expected_sha256' => '<original sha256>',
                            'chunk_bytes' => '<decoded chunk byte count>',
                            'chunk_sha256' => '<decoded chunk sha256>',
                            'data_b64' => '<base64 of at most 8192 decoded bytes>',
                        ],
                    ],
                    'workspace_search' => [
            ...
            'task_recipes' => [
                ...
                'featured_image_small_file' => [
                    'steps' => ['prepare connector-safe 8192-byte chunks', 'publish chunk commands in ascending order', 'post.update with fields.featured_media after the upload result returns attachment id'],
                    'rule' => 'Conversation/local media uses the chunk command route even below 1 MiB; do not place the whole Base64 file in one GitHub tool argument.',
                ],
                'featured_image_large_file' => [
                    'steps' => ['prepare connector-safe 8192-byte chunks', 'publish chunk commands in ascending order with at most 20 commands per Git push', 'post.update with fields.featured_media after the upload result returns attachment id'],
                    'rule' => 'Use the same connector-safe chunk stream up to the 6 MiB Bridge limit. Integrity is checked per chunk and for the reassembled original.',
                ],
            ...
            'media_routing' => [
                'client_preparer' => [
                    ...
                    'returns' => ['manifest', 'commands', 'tree'],
                    'publication' => 'Publish generated pending chunk commands in ascending chunk_index order. Each command is intentionally kept below the connector-safe JSON budget; never concatenate their Base64 strings into one tool argument.',
                ...
                'conversation_upload_route' => '/wp-agent-bridge-media/v1/upload-chunk',
                'conversation_upload_mode' => 'connector-safe-chunk-commands',
                'recommended_decoded_chunk_bytes' => 8192,
                'recommended_command_json_bytes' => 16384,
                'max_chunks' => 768,
                'max_decoded_bytes' => 6291456,
                'max_commands_per_git_push' => 20,
                'inline_action_available_but_not_canonical_for_conversation_files' => 'media.upload_base64',
                'rule' => 'For local or conversation-uploaded media, always use prepare-media.py connector-safe chunk commands. Do not send the whole Base64 file in one pending command, even when the file is below 1 MiB.',
            ],
            'connector_policy' => [
            ...
            'fast_path' => [
                ...
                'pending_recovery' => 'Automatic recovery never re-executes a pending command when its age cannot be established. It determines age from command created_at, a timestamp-bearing ID, or the pending file\'s latest GitHub commit. Only when all evidence is unavailable does it skip without execution. Commands older than 86400 seconds are recorded as expired, copied to commands/expired, and removed from commands/pending without executing the WordPress action.',
                ...
                'media_upload' => 'For every local or conversation-uploaded media file, run prepare-media.py and publish its connector-safe 8192-byte chunk commands in ascending order. Never send the whole file as one Base64 tool argument. The server reassembles and verifies the original bytes before sideloading.',
