 ...
 */
final class WP_Agent_Bridge_V097_Workspace
{
    private const VERSION = '0.9.7';
    private const NS = 'wpab-v097/v1';
    private const ROUTE = '/wpab-v097/v1/manage';
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) {
            return new WP_Error('wpab_v097_json', 'JSON body is required.', ['status' => 400]);
        }
        $action = is_string($json['action'] ?? null) ? trim($json['action']) : '';
        $params = isset($json['params']) && is_array($json['params']) ? $json['params'] : [];
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v097_action', 'Unknown or blocked v0.9.7 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'workspace.snapshot.rollback':
                    ...
                    break;
                default:
                    $response = new WP_Error('wpab_v097_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v097_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }
        ...
        if (in_array($action, [
            ...
            'workspace.snapshot.create',
            'workspace.snapshot.rollback',
        ], true) && class_exists('WP_Agent_Bridge_V07_Audit')) {
            WP_Agent_Bridge_V07_Audit::record(
                self::$request_id,
                $action,
    ...
    private static function file_get(array $params)
    {
        ...
        if ($file['bytes'] > self::MAX_FULL_READ_BYTES && empty($params['allow_large_full_read'])) {
            return new WP_Error('wpab_workspace_read_too_large', 'File exceeds the normal full-read limit. Use workspace.file.read.range.', [
                'status' => 413,
                'path' => $path,
                'bytes' => $file['bytes'],
                'max_full_read_bytes' => self::MAX_FULL_READ_BYTES,
    ...
    private static function file_read_range(array $params)
    {
        ...
        if ($start < 1 || $end < $start || ($end - $start + 1) > self::MAX_RANGE_LINES) {
            return new WP_Error('wpab_workspace_range', 'Invalid or oversized line range.', [
                'status' => 400,
                'max_range_lines' => self::MAX_RANGE_LINES,
            ]);
        }
    ...
    private static function file_search(array $params)
    {
        ...
        if ($query === '' || strlen($query) > 4096) {
            return new WP_Error('wpab_workspace_search_query', 'A literal query up to 4096 bytes is required.', ['status' => 400]);
        }
    ...
    private static function file_write(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_workspace_confirmation_required', 'workspace.file.write requires confirm=true.', ['status' => 400]);
        }
        if (!isset($params['content']) || !is_string($params['content'])) {
            return new WP_Error('wpab_workspace_content', 'content must be a UTF-8 text string.', ['status' => 400]);
        }
        $content = $params['content'];
        if (strlen($content) > self::MAX_FILE_BYTES) {
            return new WP_Error('wpab_workspace_file_too_large', 'Workspace file exceeds the maximum size.', [
                'status' => 413,
                'max_file_bytes' => self::MAX_FILE_BYTES,
            ]);
        }
        if (!self::valid_utf8($content)) {
            return new WP_Error('wpab_workspace_utf8', 'Workspace text must be valid UTF-8.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
    ...
    private static function file_patch(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_workspace_confirmation_required', 'workspace.file.patch requires confirm=true.', ['status' => 400]);
        }
        ...
        if ($search === '') {
            return new WP_Error('wpab_workspace_patch_search', 'search must be a non-empty exact text fragment.', ['status' => 400]);
        }
        $expected = isset($params['expected_current_sha256']) ? strtolower(trim((string) $params['expected_current_sha256'])) : '';
        if (!preg_match('/^[a-f0-9]{64}$/', $expected)) {
            return new WP_Error('wpab_workspace_expected_sha_required', 'workspace.file.patch requires expected_current_sha256.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
        ...
        if (!hash_equals($expected, $file['sha256'])) {
            return new WP_Error('wpab_workspace_stale', 'Workspace file changed after it was inspected.', [
                'status' => 409,
        ...
        if ($count < 1) {
            return new WP_Error('wpab_workspace_patch_not_found', 'Exact search text was not found.', ['status' => 409]);
        }
        ...
        if ($replace_all) {
            ...
            if ($occurrence > $count) {
                return new WP_Error('wpab_workspace_patch_occurrence', 'Requested occurrence exceeds the number of exact matches.', [
                    'status' => 409,
            ...
            if ($count !== 1) {
                return new WP_Error('wpab_workspace_patch_ambiguous', 'Exact search text matched more than once; specify occurrence or replace_all.', [
                    'status' => 409,
                    'match_count' => $count,
                ]);
            }
        ...
        if (strlen($new_content) > self::MAX_FILE_BYTES || !self::valid_utf8($new_content)) {
            return new WP_Error('wpab_workspace_patch_result', 'Patched content is invalid or exceeds the file-size limit.', ['status' => 413]);
        }
    ...
    private static function file_delete(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_workspace_confirmation_required', 'workspace.file.delete requires confirm=true.', ['status' => 400]);
        }
        $expected = isset($params['expected_current_sha256']) ? strtolower(trim((string) $params['expected_current_sha256'])) : '';
        if (!preg_match('/^[a-f0-9]{64}$/', $expected)) {
            return new WP_Error('wpab_workspace_expected_sha_required', 'workspace.file.delete requires expected_current_sha256.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
        ...
        $deleted = WP_Agent_Bridge_Direct_GitHub::delete_file(
    ...
    private static function snapshot_create(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_workspace_confirmation_required', 'workspace.snapshot.create requires confirm=true.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
        ...
        if (!is_string($json)) {
            return new WP_Error('wpab_workspace_snapshot_json', 'Could not encode workspace snapshot.', ['status' => 500]);
        }
    ...
    private static function file_diff(array $params)
    {
        ...
        if (($snapshot_id === '' && $blob_sha === '') || ($snapshot_id !== '' && $blob_sha !== '')) {
            return new WP_Error('wpab_workspace_diff_source', 'Provide exactly one of snapshot_id or blob_sha.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
        ...
        if ($snapshot_id !== '') {
            ...
            if ($snapshot['path'] !== $path) {
                return new WP_Error('wpab_workspace_snapshot_path', 'Snapshot belongs to a different workspace path.', [
                    'status' => 409,
            ...
            return new WP_Error('wpab_workspace_blob_sha', 'Invalid blob_sha.', ['status' => 400]);
        }
    ...
    private static function snapshot_rollback(array $params)
    {
        ...
        if (!self::valid_snapshot_id($snapshot_id)) {
            return new WP_Error('wpab_workspace_snapshot_id', 'A valid snapshot_id is required.', ['status' => 400]);
        }
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_workspace_confirmation_required', 'workspace.snapshot.rollback requires confirm=true.', ['status' => 400]);
        }
        $expected = isset($params['expected_current_sha256']) ? strtolower(trim((string) $params['expected_current_sha256'])) : '';
        if (!preg_match('/^[a-f0-9]{64}$/', $expected)) {
            return new WP_Error('wpab_workspace_expected_sha_required', 'workspace.snapshot.rollback requires expected_current_sha256.', ['status' => 400]);
        }
        $ctx = self::context();
        if (is_wp_error($ctx)) {
            return $ctx;
        ...
        if (!hash_equals($snapshot['sha256'], hash('sha256', $content))) {
            return new WP_Error('wpab_workspace_snapshot_integrity', 'Snapshot blob content does not match the stored SHA-256.', ['status' => 409]);
        }
    ...
    private static function guarded_write(array $ctx, string $path, string $content, array $params, string $message)
    {
        ...
        if ($current === null) {
            if (empty($params['expected_current_absent'])) {
                return new WP_Error('wpab_workspace_absent_guard_required', 'New workspace files require expected_current_absent=true.', [
                    'status' => 409,
                    'path' => $path,
                ]);
            }
            ...
            $expected = isset($params['expected_current_sha256']) ? strtolower(trim((string) $params['expected_current_sha256'])) : '';
            if (!preg_match('/^[a-f0-9]{64}$/', $expected)) {
                return new WP_Error('wpab_workspace_expected_sha_required', 'Existing workspace files require expected_current_sha256.', [
                    'status' => 409,
                    'path' => $path,
        ...
        if (!hash_equals($expected_new, $verified['sha256'])) {
            return new WP_Error('wpab_workspace_write_verify', 'Workspace write verification failed.', [
                'status' => 500,
    ...
    private static function context()
    {
        ...
        if ($installation_id < 1 || $repository_id < 1
            ...
            return new WP_Error('wpab_workspace_connection', 'Canonical Direct Runtime connection is incomplete.', ['status' => 503]);
        }
    ...
    {
        $ref = WP_Agent_Bridge_Direct_GitHub::github_api(
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $head)) {
            return new WP_Error('wpab_workspace_head', 'Could not resolve runtime branch head.', ['status' => 502]);
        }
        $commit = WP_Agent_Bridge_Direct_GitHub::github_api(
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $tree_sha)) {
            return new WP_Error('wpab_workspace_tree', 'Could not resolve runtime Git tree.', ['status' => 502]);
        }
        $tree = WP_Agent_Bridge_Direct_GitHub::github_api(
        ...
        if (!empty($tree['data']['truncated'])) {
            return new WP_Error('wpab_workspace_tree_truncated', 'Runtime Git tree is too large for safe workspace enumeration.', ['status' => 413]);
        }
        return [
            'head' => $head,
            'tree_sha' => $tree_sha,
    ...
    {
        $meta = WP_Agent_Bridge_Direct_GitHub::get_content_metadata(
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $blob_sha)) {
            return new WP_Error('wpab_workspace_blob_sha', 'GitHub did not return a valid blob SHA.', ['status' => 502]);
        }
        ...
        if (strlen($content) > self::MAX_FILE_BYTES) {
            return new WP_Error('wpab_workspace_file_too_large', 'Workspace file exceeds the maximum supported size.', [
                'status' => 413,
                'path' => $relative,
            ]);
        }
        if (!self::valid_utf8($content)) {
            return new WP_Error('wpab_workspace_utf8', 'Workspace file is not valid UTF-8 text.', [
                'status' => 415,
                'path' => $relative,
            ]);
        }
    ...
    private static function read_blob(array $ctx, string $blob_sha)
    {
        if (!preg_match('/^[a-f0-9]{40,64}$/', strtolower($blob_sha))) {
            return new WP_Error('wpab_workspace_blob_sha', 'Invalid Git blob SHA.', ['status' => 400]);
        }
        $response = WP_Agent_Bridge_Direct_GitHub::github_api(
        ...
        if ($encoding !== 'base64' || $encoded === '') {
            return new WP_Error('wpab_workspace_blob_content', 'Git blob response did not contain Base64 content.', ['status' => 502]);
        }
        $content = base64_decode($encoded, true);
        if (!is_string($content)) {
            return new WP_Error('wpab_workspace_blob_decode', 'Could not decode Git blob content.', ['status' => 502]);
        }
        if (strlen($content) > self::MAX_FILE_BYTES || !self::valid_utf8($content)) {
            return new WP_Error('wpab_workspace_blob_type', 'Git blob is not a supported bounded UTF-8 workspace artifact.', ['status' => 415]);
        }
        return $content;
    }
    ...
    {
        if (!self::valid_snapshot_id($snapshot_id)) {
            return new WP_Error('wpab_workspace_snapshot_id', 'Invalid snapshot_id.', ['status' => 400]);
        }
        $meta = WP_Agent_Bridge_Direct_GitHub::get_content_metadata(
        ...
        if (!is_array($snapshot)
            ...
            return new WP_Error('wpab_workspace_snapshot_manifest', 'Snapshot manifest is invalid.', ['status' => 409]);
        }
        ...
        if (!preg_match('/^[a-f0-9]{40,64}$/', $blob_sha) || !preg_match('/^[a-f0-9]{64}$/', $sha256)) {
            return new WP_Error('wpab_workspace_snapshot_manifest', 'Snapshot hashes are invalid.', ['status' => 409]);
        }
        return [
            'snapshot_id' => $snapshot_id,
            'path' => $path,
    ...
    private static function normalize_prefix(string $prefix)
    {
        ...
        if (strpos($prefix, '..') !== false || strpos($prefix, "\0") !== false || strlen($prefix) > 400) {
            return new WP_Error('wpab_workspace_prefix', 'Invalid workspace path_prefix.', ['status' => 400]);
        }
    ...
    private static function normalize_relative_path(string $path)
    {
        ...
        if ($path === '' || strlen($path) > 400 || strpos($path, '..') !== false || strpos($path, "\0") !== false) {
            return new WP_Error('wpab_workspace_path', 'Invalid workspace relative path.', ['status' => 400]);
        }
        ...
        if ($extension === '' || !in_array($extension, self::EXTENSIONS, true)) {
            return new WP_Error('wpab_workspace_extension', 'Workspace file extension is not allowlisted.', [
                'status' => 415,
                'allowed_extensions' => self::EXTENSIONS,
            ]);
        }
    ...
    private static function decode_contents_meta(array $meta)
    {
        ...
        $encoded = isset($meta['content']) && is_string($meta['content'])
            ...
            : '';
        if ($encoding !== 'base64' || $encoded === '') {
            return new WP_Error('wpab_workspace_content', 'GitHub contents response did not contain Base64 file content.', ['status' => 502]);
        }
        $decoded = base64_decode($encoded, true);
        if (!is_string($decoded)) {
            return new WP_Error('wpab_workspace_content_decode', 'Could not decode workspace file content.', ['status' => 502]);
        }
        return $decoded;
    }
