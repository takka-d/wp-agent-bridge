...
final class WP_Agent_Bridge_V095_Classic
{
    private const OPTION = 'wpab_v095_classic_drafts';
    private const PREVIEW_PREFIX = 'wpab_v095_preview_';
    ...
    public static function create(array $params)
    {
        ...
        if ($name === '') {
            return new WP_Error('wpab_v095_theme_name', 'theme_name is required.', ['status' => 400]);
        }
        ...
        if ($theme_root === false || !is_dir($theme_root)) {
            return new WP_Error('wpab_v095_theme_root', 'Theme root was not found.', ['status' => 500]);
        }
        $target = $theme_root . DIRECTORY_SEPARATOR . $slug;
        if (file_exists($target) || !wp_mkdir_p($target)) {
            return new WP_Error('wpab_v095_theme_dir', 'Could not create classic theme draft directory.', ['status' => 500]);
        }
        ...
        foreach ($files as $relative => $content) {
            ...
            if (file_put_contents($path, $content, LOCK_EX) !== strlen($content)) {
                ...
                return new WP_Error('wpab_v095_theme_write', 'Could not write classic theme draft file.', [
                    'status' => 500,
                    'path' => $relative,
                ]);
            }
        ...
        if (!$theme->exists() || $theme->errors()) {
            ...
            self::remove_tree($target);
            wp_clean_themes_cache(true);
            return new WP_Error('wpab_v095_theme_invalid', 'Generated classic theme was not recognized by WordPress.', [
                'status' => 500,
    ...
    public static function info(array $params)
    {
        ...
        if (!is_dir($root)) {
            return new WP_Error('wpab_v095_classic_root', 'Classic theme draft directory is missing.', ['status' => 410]);
        }
    ...
    public static function preview_url(array $params)
    {
        ...
        if ($path === '' || $path[0] !== '/' || strpos($path, '://') !== false || preg_match('/[\r\n\0]/', $path)) {
            return new WP_Error('wpab_v095_preview_path', 'Preview path must be a local path beginning with /.', ['status' => 400]);
        }
        return rest_ensure_response([
            'draft_id' => $draft['id'],
    ...
    public static function publish(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_v095_publish_confirm', 'classic_theme.publish requires confirm=true.', ['status' => 400]);
        }
        $root = self::root($draft);
        $lint = self::lint_tree($root);
        if (is_wp_error($lint)) {
        ...
        if (!$theme->exists() || $theme->errors()) {
            return new WP_Error('wpab_v095_theme_invalid', 'Classic theme draft is not a valid WordPress theme.', ['status' => 400]);
        }
        $previous = [
        ...
        if (get_stylesheet() !== $draft['slug']) {
            return new WP_Error('wpab_v095_theme_activate', 'WordPress did not activate the generated classic theme.', ['status' => 500]);
        }
        $drafts = self::drafts();
    ...
    public static function discard(array $params)
    {
        ...
        if (get_stylesheet() === $draft['slug']) {
            return new WP_Error('wpab_v095_discard_active', 'Cannot discard a currently active classic theme.', ['status' => 409]);
        }
        $root = self::root($draft);
        if (!self::remove_tree($root) && is_dir($root)) {
            return new WP_Error('wpab_v095_discard', 'Could not remove classic theme draft directory.', ['status' => 500]);
        }
        $drafts = self::drafts();
        unset($drafts[$draft['id']]);
        self::save($drafts);
    ...
    private static function require_draft(array $params)
    {
        ...
        if ($id === '') {
            return new WP_Error('wpab_v095_classic_id', 'draft_id is required.', ['status' => 400]);
        }
        $drafts = self::drafts();
        if (!isset($drafts[$id]) || !is_array($drafts[$id])) {
            return new WP_Error('wpab_v095_classic_missing', 'Classic theme draft was not found.', ['status' => 404]);
        }
        return $drafts[$id];
    }
    ...
    private static function lint_php(string $content)
    {
        try {
            ...
            return new WP_Error('wpab_v095_theme_php', $e->getMessage(), ['status' => 400]);
        }
        return true;
    }
    ...
    private static function lint_tree(string $root)
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (!is_string($content)) {
                return new WP_Error('wpab_v095_theme_read', 'Could not read PHP file.', ['status' => 500]);
            }
            $lint = self::lint_php($content);
            if (is_wp_error($lint)) {
    ...
    private static function collect_files(string $root)
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (count($files) > self::MAX_FILES || $bytes > self::MAX_BYTES) {
                return new WP_Error('wpab_v095_classic_size', 'Classic theme draft exceeds inspection limits.', ['status' => 413]);
            }
        }
