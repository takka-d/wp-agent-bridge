...
final class WP_Agent_Bridge_Direct_GitHub
{
    ...
    private const OPTION_APP = 'wpab_direct_github_app_v1';
    private const OPTION_PRIVATE_KEY = 'wpab_direct_github_private_key_v1';
    private const OPTION_WEBHOOK_SECRET = 'wpab_direct_github_webhook_secret_v1';
    private const ENCRYPTED_PREFIX = 'twbdg1:';
    private const ENCRYPTION_CONTEXT = 'wp-agent-bridge-direct-github-v1';
    private const TOKEN_TRANSIENT_PREFIX = 'wpab_direct_iat_';
    ...
    public static function app_jwt()
    {
        ...
        if ($app_id < 1 || $private_key === '') {
            return new WP_Error('wpab_direct_github_credentials', 'Direct GitHub App credentials are incomplete.');
        }
        ...
        if ($header === '' || $payload === '') {
            return new WP_Error('wpab_direct_github_jwt_encode', 'Could not encode GitHub App JWT.');
        }
        ...
        if ($key === false || !openssl_sign($unsigned, $signature, $key, OPENSSL_ALGO_SHA256)) {
            return new WP_Error('wpab_direct_github_jwt_sign', 'Could not sign GitHub App JWT.');
        }
    ...
    public static function installation_token(int $installation_id, int $repository_id = 0)
    {
        if ($installation_id < 1) {
            return new WP_Error('wpab_direct_github_installation', 'Invalid GitHub App installation ID.');
        }
        ...
        if ($token === '' || $expires_at === false) {
            return new WP_Error('wpab_direct_github_token_response', 'GitHub did not return a valid installation token.');
        }
    ...
    public static function get_text_file(string $token, string $repository, string $ref, string $path)
    {
        ...
        if ($encoded === '') {
            return new WP_Error('wpab_direct_github_content', 'GitHub file content is missing.');
        }
        $decoded = base64_decode($encoded, true);
        if (!is_string($decoded)) {
            return new WP_Error('wpab_direct_github_content_decode', 'Could not decode GitHub file content.');
        }
        return $decoded;
    }
    ...
    public static function get_content_metadata(string $token, string $repository, string $ref, string $path)
    {
        if (!self::valid_repository($repository) || !self::valid_path($path)) {
            return new WP_Error('wpab_direct_github_path', 'Invalid GitHub repository or path.');
        }
    ...
    public static function delete_file(string $token, string $repository, string $branch, string $path, string $sha, string $message)
    {
        if (!self::valid_repository($repository) || !self::valid_path($path) || !preg_match('/^[a-f0-9]{40,64}$/', strtolower($sha))) {
            return new WP_Error('wpab_direct_github_delete', 'Invalid GitHub delete request.');
        }
        $endpoint = '/repos/' . $repository . '/contents/' . self::encode_path($path);
    ...
    private static function request(string $method, string $endpoint, string $token, $body, bool $jwt_auth, array $allowed_statuses = [])
    {
        if (strpos($endpoint, '/') !== 0 || strpos($endpoint, '://') !== false) {
            return new WP_Error('wpab_direct_github_endpoint', 'Invalid GitHub API endpoint.');
        }
        ...
        if ($body !== null) {
            ...
            if (!is_string($encoded)) {
                return new WP_Error('wpab_direct_github_json', 'Could not encode GitHub API request.');
            }
        ...
        if (($status < 200 || $status >= 300) && !in_array($status, $allowed_statuses, true)) {
            ...
            return new WP_Error('wpab_direct_github_http_' . $status, $message, [
                'status' => $status,
