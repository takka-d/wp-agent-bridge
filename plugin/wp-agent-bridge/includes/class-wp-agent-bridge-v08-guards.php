...
final class WP_Agent_Bridge_V08_Guards
{
    ...
    {
        if ($response !== null || $request->get_route() !== '/wpab-v08/v1/manage') return $response;
        ...
        if (!is_string($params['autoload']) || !in_array(strtolower(trim($params['autoload'])), ['on', 'off'], true)) {
            return new WP_Error('wpab_option_autoload_invalid', 'autoload must be on or off.', ['status' => 400]);
        }
        return $response;
    }
}
