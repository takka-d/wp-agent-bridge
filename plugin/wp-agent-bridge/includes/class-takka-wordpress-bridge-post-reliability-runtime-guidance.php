...
final class TakKa_WordPress_Bridge_Post_Reliability_Runtime_Guidance
{
    ...
    private const OPTION_SIGNATURE = 'takka_bridge_post_reliability_guidance_signature_v1';
    private const TRANSIENT_LOCK = 'takka_bridge_post_reliability_guidance_lock_v1';
    ...
    public static function sync()
    {
        ...
        $capabilities = TakKa_WordPress_Bridge_Direct_GitHub::get_text_file($token, $repository, $branch, self::CAPABILITIES_PATH);
    ...
    public static function enrich_capabilities(array $catalog): array
    {
        ...
        $operations = isset($catalog['routes']['operations']['operations']) && is_array($catalog['routes']['operations']['operations'])
            ...
            : [];
        foreach (TakKa_WordPress_Bridge_Post_Revisions::contract()['operations'] as $operation) {
            if (!in_array($operation, $operations, true)) {
                $operations[] = $operation;
            }
        }
        $catalog['routes']['operations']['operations'] = $operations;
        $catalog['routes']['operations']['post_revisions'] = TakKa_WordPress_Bridge_Post_Revisions::contract();
        $catalog['routes']['operations']['post_content_patch_diagnostics'] = TakKa_WordPress_Bridge_Post_Content_Diagnostics::contract();
        $catalog['routes']['operations']['response_contract'] = TakKa_WordPress_Bridge_V099_Response_Contract::contract();

        if (!isset($catalog['command_contract']) || !is_array($catalog['command_contract'])) {
            $catalog['command_contract'] = [];
        }
        $catalog['command_contract']['response_contract'] = TakKa_WordPress_Bridge_V099_Response_Contract::contract();
    ...
    {
        return [
            'response' => TakKa_WordPress_Bridge_V099_Response_Contract::contract(),
            'content_diagnostics' => TakKa_WordPress_Bridge_Post_Content_Diagnostics::contract(),
            'revisions' => TakKa_WordPress_Bridge_Post_Revisions::contract(),
        ];
    }
