...
final class WP_Agent_Bridge_V086_Table
{
    ...
    public static function inspect(array $params)
    {
        ...
        if (strlen($content) > self::MAX_CONTENT_BYTES) {
            return new WP_Error('wpab_table_content_size', 'Post content is too large for Bridge table tooling.', ['status' => 413]);
        }
        ...
        if ($selected !== null && ($selected < 0 || $selected >= count($tables))) {
            return new WP_Error('wpab_table_index', 'table_index is out of range.', ['status' => 404, 'table_count' => count($tables)]);
        }
    ...
    public static function preview(array $params)
    {
        ...
        $response = WP_Agent_Bridge_V085_Content_Batch::preview($translation['batch_params']);
    ...
    public static function apply(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Table changes require confirm=true.', ['status' => 400]);
        }
        ...
        $response = WP_Agent_Bridge_V085_Content_Batch::apply($batch);
        if (is_wp_error($response)) return $response;
        $rest = rest_ensure_response($response);
        $data = $rest->get_data();
        if (!is_array($data)) return $response;
    ...
    private static function translate(array $params)
    {
        ...
        if ($expected_table !== '' && !hash_equals($table_before_sha, $expected_table)) {
            return new WP_Error('wpab_table_changed', 'Selected table changed after inspection or preview.', [
                'status' => 409,
        ...
        if (substr_count($content, $table_before) !== 1) {
            return new WP_Error('wpab_table_not_unique', 'Selected table markup is not unique in the post; refusing ambiguous replacement.', ['status' => 409]);
        }

        $operations = isset($params['operations']) ? $params['operations'] : null;
        if (!is_array($operations) || !$operations) {
            return new WP_Error('wpab_table_operations_required', 'operations must be a non-empty array.', ['status' => 400]);
        }
        if (count($operations) > self::MAX_OPERATIONS) {
            return new WP_Error('wpab_table_operations_limit', 'Too many table operations.', ['status' => 413, 'max_operations' => self::MAX_OPERATIONS]);
        }
        ...
        foreach (array_values($operations) as $index => $operation) {
            if (!is_array($operation)) {
                return new WP_Error('wpab_table_operation_object', 'Each table operation must be an object.', ['status' => 400, 'index' => $index]);
            }
        ...
        if ($working === $table_before) {
            return new WP_Error('wpab_table_no_change', 'Table operations would not change the selected table.', ['status' => 409]);
        }
    ...
    private static function apply_semantic_operation(string $table, array $operation, int $index)
    {
        ...
        if (!in_array($type, ['set_cell', 'set_cells', 'insert_row_before', 'insert_row_after', 'delete_row'], true)) {
            return new WP_Error('wpab_table_operation_type', 'Table operation type must be set_cell, set_cells, insert_row_before, insert_row_after, or delete_row.', ['status' => 400, 'index' => $index]);
        }

        $row_key = isset($operation['row_key']) && is_string($operation['row_key']) ? self::normalize_text($operation['row_key']) : '';
        if ($row_key === '') {
            return new WP_Error('wpab_table_row_key', 'row_key must be a non-empty string.', ['status' => 400, 'index' => $index]);
        }
        $key_column = isset($operation['key_column']) ? (int) $operation['key_column'] : 0;
        if ($key_column < 0 || $key_column >= self::MAX_CELLS) {
            return new WP_Error('wpab_table_key_column', 'key_column is out of range.', ['status' => 400, 'index' => $index]);
        }
        ...
        if (count($matches) !== 1) {
            return new WP_Error('wpab_table_row_match', 'row_key must identify exactly one row in the selected table.', [
                'status' => 409,
                'index' => $index,
        ...
        if ($type === 'set_cell') {
            ...
            if ($column < 0 || !isset($row['cells'][$column])) {
                return new WP_Error('wpab_table_column', 'column is out of range for the matched row.', ['status' => 400, 'index' => $index, 'column' => $column]);
            }
            $spec = [
            ...
            if (!is_array($cells) || !$cells) {
                return new WP_Error('wpab_table_cells', 'set_cells requires a non-empty cells object or array.', ['status' => 400, 'index' => $index]);
            }
            $map = [];
            foreach ($cells as $column => $spec) {
                if (is_int($column)) {
                    ...
                    return new WP_Error('wpab_table_column', 'set_cells keys must be numeric column indexes.', ['status' => 400, 'index' => $index]);
                }
                if ($column_index < 0 || !isset($row['cells'][$column_index])) {
                    return new WP_Error('wpab_table_column', 'A set_cells column is out of range for the matched row.', ['status' => 400, 'index' => $index, 'column' => $column_index]);
                }
            ...
            $cells = isset($operation['cells']) ? $operation['cells'] : null;
            if (!is_array($cells) || !$cells) {
                return new WP_Error('wpab_table_cells', 'Row insertion requires a non-empty cells array.', ['status' => 400, 'index' => $index]);
            }
            if (count($cells) !== count($row['cells'])) {
                return new WP_Error('wpab_table_insert_columns', 'Inserted row must have the same number of cells as the anchor row.', [
                    'status' => 409,
                    'index' => $index,
            ...
            if ($new_row === $before_row) {
                return new WP_Error('wpab_table_insert_no_change', 'Inserted row must differ from the anchor row.', ['status' => 409, 'index' => $index]);
            }
        ...
        if (is_string($after_row) && strlen($after_row) > self::MAX_CELL_BYTES * self::MAX_CELLS) {
            return new WP_Error('wpab_table_row_size', 'Updated row is too large.', ['status' => 413, 'index' => $index]);
        }
        if ($after_row === $before_row) {
            return new WP_Error('wpab_table_operation_no_change', 'Table operation would not change the matched row.', ['status' => 409, 'index' => $index]);
        }
    ...
    private static function replace_row_cells(string $row_html, array $map)
    {
        ...
        foreach ($map as $column => $spec) {
            ...
            if (!isset($cells[$column])) {
                return new WP_Error('wpab_table_column', 'Column is out of range while building row.', ['status' => 400, 'column' => $column]);
            }
            ...
            if (strlen($replacement) > self::MAX_CELL_BYTES) {
                return new WP_Error('wpab_table_cell_size', 'Cell content exceeds the size limit.', ['status' => 413, 'column' => $column]);
            }
            $replacements[] = [
    ...
    private static function render_cell_content($spec)
    {
        ...
        if (!is_array($spec)) {
            return new WP_Error('wpab_table_cell_spec', 'Cell value must be a string/number or an object with content and format.', ['status' => 400]);
        }
        ...
        return new WP_Error('wpab_table_cell_format', 'Cell format must be text or html.', ['status' => 400]);
    }
    ...
    private static function parse_tables(string $content)
    {
        ...
        if ($count === false) {
            return new WP_Error('wpab_table_parse', 'Could not parse table markup.', ['status' => 500]);
        }
        if ($count > self::MAX_TABLES) {
            return new WP_Error('wpab_table_limit', 'Too many tables in post content.', ['status' => 413, 'max_tables' => self::MAX_TABLES]);
        }
        $tables = [];
    ...
    private static function parse_rows(string $table)
    {
        ...
        if ($count === false) {
            return new WP_Error('wpab_table_row_parse', 'Could not parse table rows.', ['status' => 500]);
        }
        if ($count > self::MAX_ROWS) {
            return new WP_Error('wpab_table_row_limit', 'Too many rows in selected table.', ['status' => 413, 'max_rows' => self::MAX_ROWS]);
        }
        $rows = [];
        foreach ($matches[0] as $match) {
    ...
    private static function parse_cells(string $row)
    {
        ...
        if ($count === false) {
            return new WP_Error('wpab_table_cell_parse', 'Could not parse table cells.', ['status' => 500]);
        }
        if ($count > self::MAX_CELLS) {
            return new WP_Error('wpab_table_cell_limit', 'Too many cells in a table row.', ['status' => 413, 'max_cells' => self::MAX_CELLS]);
        }
        $cells = [];
    ...
    private static function context(array $params)
    {
        ...
        if ($post_id < 1) return new WP_Error('wpab_post_id_required', 'post_id is required.', ['status' => 400]);
        $post = get_post($post_id);
        if (!$post) return new WP_Error('wpab_post_not_found', 'Post was not found.', ['status' => 404]);
        if (!current_user_can('edit_post', $post_id)) {
            return new WP_Error('wpab_post_content_forbidden', 'Connected user cannot inspect or change this post.', ['status' => 403, 'post_id' => $post_id]);
        }
        return ['post' => $post];
    }
