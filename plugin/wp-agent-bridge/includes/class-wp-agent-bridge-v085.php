...
final class WP_Agent_Bridge_V085
{
    private const VERSION = '0.8.5';
    private const INTERNAL_NAMESPACE = 'wpab-v085/v1';
    private const INTERNAL_ROUTE = '/wpab-v085/v1/manage';
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('wpab_v085_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('wpab_v085_json', 'JSON body is required.', ['status' => 400]);
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_v085_unknown_action', 'Unknown or blocked v0.8.5 action.', ['status' => 400, 'action' => $action]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'post.content.batch.preview':
                    $response = WP_Agent_Bridge_V085_Content_Batch::preview($params);
                    break;
                case 'post.content.batch.apply':
                    $response = WP_Agent_Bridge_V085_Content_Batch::apply($params);
                    WP_Agent_Bridge_V07_Audit::record(self::$request_id, $action, self::audit_params($params), $response);
                    break;
                default:
                    $response = new WP_Error('wpab_v085_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('wpab_v085_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return $response;
    }
