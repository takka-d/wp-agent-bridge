...
final class TakKa_WordPress_Bridge_Runtime_Guidance
{
    ...
    public static function client_prompt(string $repository, string $branch, string $host): string
    {
        return "WordPress操作には、GitHubプラグイン経由でサイト {$host} のWP Agent Bridgeを使用してください。\n"
            ...
            . "最初に repository={$repository}, branch/ref={$branch}, path=wordpress-bridge/RUNTIME_CONNECTION.json の完全一致で読み、site_hostとrepositoryが上記と一致し、status=canonical、transport=direct-github-webhook、ownership=user-owned、operator_relay=false を確認してください。rootのRUNTIME_CONNECTION.jsonはbootstrap aliasです。続いて同じbranch/refで AGENTS.md と wordpress-bridge/RUNTIME_CAPABILITIES.json を読み、今回の依頼に必要な操作を選んでください。確認済みの接続と機能一覧は同じ会話内で再利用してください。\n"
            . "404が出た場合、同じ読み取りを繰り返す前にrepository/path/refを確認してください。ref省略・default branch・root/nested path違いによる404はrepositoryへのアクセス拒否を意味しません。完全一致のrepository/path/refで1回だけ補正し、それでも404なら初めてruntime bootstrap不整合として扱ってください。コマンドは必ず branch={$branch} の wordpress-bridge/commands/pending/<id>.json に作成し、default branchへは書き込まないでください。\n"
    ...
    public static function agents(string $repository, string $branch, string $host, string $version): string
    {
        return "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "## Mandatory fast path\n\n"
            . "1. Every runtime GitHub read/write must use repository `{$repository}` and explicitly set branch/ref `{$branch}`. File readers commonly fall back to the repository default branch when ref is omitted. The canonical marker is `wordpress-bridge/RUNTIME_CONNECTION.json`; root `RUNTIME_CONNECTION.json` is only a bootstrap alias. Never write pending commands to the default branch.\n"
            . "2. Reuse this repository/branch when already verified in the current task/session. Re-resolve only after an explicit reconnect/migration signal, a mapping mismatch, an inaccessible repository/branch, a non-canonical marker, or a connected GitHub account/tool-context change.\n"
            . "3. Read `wordpress-bridge/RUNTIME_CAPABILITIES.json` before probing Bridge capabilities or searching source code for known routes/actions. Read it once per verified connection/version and reuse its operation catalog. Refresh after a version/connection change or a contradictory response, not before every command.\n"
            . "4. For advertised common operations use `type=operation` with `operation` and `params`. The runtime selects `/takka-v099/v1/operate` internally. Do not reconstruct versioned routes when an operation is listed in the catalog.\n"
            . "5. Never put `?query=...` in a REST `route` field. Put query parameters in the `query` object or in the deterministic operation params.\n"
            . "6. Do not ask the user to identify repositories, files, routes, branches, or installed state that connected tools can inspect directly.\n"
            . "7. Do not switch normal WordPress work to WPVibe, `takka-d/chatgpt-data`, old Bridge Key/queue, an operator relay, temporary PHP installers, or a per-command GitHub Actions worker. GitHub Actions is for source CI/package/release, not the Direct Runtime command path.\n\n"
            ...
            . "## Failure rules\n\n"
            . "- Bootstrap/marker 404: verify the exact repository, path and branch/ref before concluding access failed. An omitted ref commonly means the default branch was read. Correct that tuple once using `{$repository}` + `{$branch}`; do not repeat the same wrong-branch read. Only an exact-tuple failure is evidence of a runtime bootstrap problem.\n"            . "- Keep one command ID and identical payload when checking/recovering an uncertain outcome. A new ID means a new operation; do not generate a fresh ID to retry an unconfirmed mutation.\n"
    ...
    public static function runtime(string $repository, string $branch, string $host, string $version): string
    {
        return "# WP Agent Bridge Direct Runtime\n\n"
            ...
            . "Repository: `{$repository}`\n\n"
            . "Branch: `{$branch}`\n\n"
            . "Runtime clients must explicitly pass this branch/ref on every GitHub read and write. Omitting ref may read the repository default branch, whose mirrored bootstrap files are read-only discovery aids.\n\n"
