...
final class TakKa_WordPress_Bridge_Runtime_Guidance
{
    ...
    public static function agents(string $repository, string $branch, string $host, string $version): string
    {
        return "# WP Agent Bridge runtime — CANONICAL\n\n"
            ...
            . "2. Read `wordpress-bridge/RUNTIME_CAPABILITIES.json` before probing Bridge capabilities or searching source code for known routes/actions.\n"
            . "3. For advertised common operations use the single deterministic route `/takka-v099/v1/operate`. Pending command shape: `type=rest`, `method=POST`, `route=/takka-v099/v1/operate`, body `{operation, params}`. Do not reconstruct versioned routes when an operation is listed in the catalog.\n"
            ...
            . "## Routing rules\n\n"
            . "- Preferred pending JSON: `{\"id\":\"<unique-id>\",\"type\":\"operation\",\"operation\":\"post.get\",\"params\":{\"post_id\":123}}`. The filename is `<unique-id>.json`. This reaches `/takka-v099/v1/operate` without reconstructing a route or method; the REST shape above remains compatible.\n"
            ...
            . "- Post metadata/status/featured image: `post.get` and `post.update`. `post.get` is metadata-oriented and bounded by default; do not request `content` through it. `post.update` must not replace `content`.\n"
            ...
            . "- Two or more independent reads: one `readonly.batch`. Nested items may use the same high-level operation names (`post.content.read_range`, `workspace.file.read_range`, etc.); the router normalizes them to the strict read-only action map. Do not hand-derive another action spelling when a documented alias exists.\n"
            ...
            . "- Fetch the exact `wordpress-bridge/results/<id>.json` path. A missing result is pending/unknown, not proof of failure. Record submission-to-result time separately from the existing `duration_ms` execution field; never describe that field as total user wait time.\n"
