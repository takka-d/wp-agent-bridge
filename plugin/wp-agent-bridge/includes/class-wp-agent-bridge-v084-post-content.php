...
final class WP_Agent_Bridge_V084_Post_Content
{
    ...
    public static function search(array $params)
    {
        ...
        if ($query === '' || strlen($query) > self::MAX_QUERY_BYTES) {
            return new WP_Error('wpab_post_content_query', 'query must be a non-empty string within the search limit.', ['status' => 400]);
        }

        $content = (string) $context['post']->post_content;
        if (strlen($content) > self::MAX_CONTENT_BYTES) {
            return new WP_Error('wpab_post_content_size', 'Post content is too large for Bridge content tooling.', ['status' => 413]);
        }
    ...
    public static function read_range(array $params)
    {
        ...
        if ($start_line > $total_lines) {
            return new WP_Error('wpab_post_content_range', 'start_line is beyond the end of the post content.', [
                'status' => 416,
        ...
        for ($index = $start_line - 1; $index < $total_lines && count($selected) < $max_lines; $index++) {
            ...
            if ($returned_bytes + $addition > self::MAX_READ_RANGE_BYTES) {
                if (!$selected) {
                    return new WP_Error('wpab_post_content_range_line_too_large', 'The requested first line exceeds the read-range byte limit. Use post.content.search with a targeted query.', [
                        'status' => 413,
                        'start_line' => $start_line,
    ...
    public static function apply(array $params)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', 'Post content changes require confirm=true.', ['status' => 400]);
        }
        $plan = self::plan($params);
        if (is_wp_error($plan)) return $plan;
        ...
        if ($expected_before === '' || !hash_equals((string) $plan['before_sha256'], $expected_before)) {
            return new WP_Error('wpab_post_content_changed', 'Post content changed after preview or expected_before_sha256 is missing.', [
                'status' => 409,
        ...
        if ($expected_plan === '' || !hash_equals((string) $plan['plan_hash'], $expected_plan)) {
            return new WP_Error('wpab_post_content_plan_changed', 'Patch plan changed after preview or expected_plan_hash is missing.', ['status' => 409, 'current_plan_hash' => $plan['plan_hash']]);
        }
        if (!$plan['changed']) {
            return new WP_Error('wpab_post_content_no_change', 'Patch would not change post content.', ['status' => 409]);
        }
        if (!in_array($plan['status'], ['draft', 'pending', 'auto-draft'], true) && empty($params['confirm_live'])) {
            return new WP_Error('wpab_post_content_live_confirmation_required', 'Changing non-draft post content requires confirm_live=true.', ['status' => 400, 'status_value' => $plan['status']]);
        }
        ...
        if (!$post) return new WP_Error('wpab_post_not_found_after_update', 'Post disappeared after update.', ['status' => 500]);
        $actual_sha = hash('sha256', (string) $post->post_content);
        if (!hash_equals((string) $plan['after_sha256'], $actual_sha)) {
            return new WP_Error('wpab_post_content_verify_failed', 'Post content did not match the planned result after update.', ['status' => 500, 'actual_sha256' => $actual_sha]);
        }
    ...
    private static function plan(array $params)
    {
        ...
        if ($find === '') return new WP_Error('wpab_post_content_find', 'find must be a non-empty string.', ['status' => 400]);
        if (strlen($find) > self::MAX_REPLACEMENT_BYTES || strlen($replace) > self::MAX_REPLACEMENT_BYTES) {
            return new WP_Error('wpab_post_content_patch_size', 'find/replace exceeds the patch size limit.', ['status' => 413]);
        }
        ...
        if (!$replace_all && $expected_matches !== 1) {
            return new WP_Error('wpab_post_content_unique_required', 'Single replacement mode requires expected_matches=1. Use a more specific find string or replace_all=true.', ['status' => 400]);
        }
        $match_count = substr_count($content, $find);
        if ($match_count !== $expected_matches) {
            return new WP_Error('wpab_post_content_match_count', 'Current exact match count differs from expected_matches.', ['status' => 409, 'expected_matches' => $expected_matches, 'actual_matches' => $match_count]);
        }
