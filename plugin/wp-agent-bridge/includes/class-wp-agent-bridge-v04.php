 ...
 */
final class WP_Agent_Bridge_V04
{
    private const ROUTE_NAMESPACE = 'wp-agent-bridge/v1';
    private const ROUTE = '/wp-agent-bridge/v1/manage';
    ...
    {
        if ($request->get_route() !== '/wp-agent-bridge/v1/health') {
            return $response;
        }
    ...
    public static function authorize_request(WP_REST_Request $request)
    {
        ...
        if ($secret === '' || $user_id < 1) {
            return new WP_Error('wpab_not_configured', 'WP Agent Bridge WordPress Bridge is not configured.', ['status' => 503]);
        }
        ...
        if ($timestamp === '' || $signature === '' || !ctype_digit($timestamp)) {
            return new WP_Error('wpab_invalid_signature_headers', 'Missing or invalid bridge signature headers.', ['status' => 401]);
        }
    ...
    public static function manage(WP_REST_Request $request)
    {
        ...
        if (!is_array($json) || !isset($json['payload_b64']) || !is_string($json['payload_b64'])) {
            return new WP_Error('wpab_manage_missing_envelope', 'Missing payload_b64.', ['status' => 400]);
        }

        $payload_b64 = trim($json['payload_b64']);
        if ($payload_b64 === '' || strlen($payload_b64) > self::MAX_ENVELOPE_BYTES) {
            return new WP_Error('wpab_manage_envelope_size', 'Management envelope is empty or too large.', ['status' => 413]);
        }

        $decoded = base64_decode($payload_b64, true);
        if (!is_string($decoded)) {
            return new WP_Error('wpab_manage_invalid_base64', 'Management envelope is not valid Base64.', ['status' => 400]);
        }
        $payload = json_decode($decoded, true);
        if (!is_array($payload) || json_last_error() !== JSON_ERROR_NONE) {
            return new WP_Error('wpab_manage_invalid_json', 'Decoded management envelope is not valid JSON.', ['status' => 400]);
        }
        ...
        if ($action === '' || !in_array($action, self::ACTIONS, true)) {
            return new WP_Error('wpab_manage_unknown_action', 'Unknown or blocked v0.4 action.', [
                'status' => 400,
                'action' => $action,
            ]);
        }
        ...
        try {
            ...
            return new WP_Error('wpab_v04_exception', $e->getMessage(), [
                'status' => 500,
                'type' => get_class($e),
            ]);
        }

        return new WP_Error('wpab_v04_dispatch_error', 'v0.4 dispatch fell through unexpectedly.', ['status' => 500]);
    }
    ...
    private static function plugin_install(array $params)
    {
        ...
        foreach ($before as $file) {
            if (dirname($file) === $slug || basename($file, '.php') === $slug) {
                return new WP_Error('wpab_plugin_already_installed', 'Plugin appears to be already installed.', [
                    'status' => 409,
                    'file' => $file,
                ]);
            }
        ...
        if (empty($api->download_link)) {
            return new WP_Error('wpab_plugin_download_missing', 'WordPress.org did not provide a plugin download URL.', ['status' => 502]);
        }
    ...
    private static function plugin_delete(array $params)
    {
        ...
        require_once ABSPATH . 'wp-admin/includes/plugin.php';
        if (is_plugin_active($file) || (is_multisite() && is_plugin_active_for_network($file))) {
            return new WP_Error('wpab_plugin_active_delete_blocked', 'Deactivate the plugin before deleting it.', ['status' => 409]);
        }
    ...
    private static function theme_install(array $params)
    {
        ...
        if (wp_get_theme($slug)->exists()) {
            return new WP_Error('wpab_theme_already_installed', 'Theme is already installed.', ['status' => 409]);
        }
        ...
        if (empty($api->download_link)) {
            return new WP_Error('wpab_theme_download_missing', 'WordPress.org did not provide a theme download URL.', ['status' => 502]);
        }
        $skin = new Automatic_Upgrader_Skin();
    ...
    private static function theme_activate(array $params)
    {
        ...
        if ($after !== $stylesheet) {
            return new WP_Error('wpab_theme_activation_failed', 'Theme did not become active.', ['status' => 500]);
        }
    ...
    private static function theme_delete(array $params)
    {
        ...
        if ($stylesheet === get_stylesheet() || $stylesheet === get_template()) {
            return new WP_Error('wpab_active_theme_delete_blocked', 'Cannot delete the active theme or its active parent.', ['status' => 409]);
        }
    ...
    private static function theme_file_patch(array $params)
    {
        ...
        if (!array_key_exists('replace', $params) || !is_string($params['replace'])) {
            return new WP_Error('wpab_patch_missing_replace', 'replace must be a string.', ['status' => 400]);
        }
        if ($find === '') {
            return new WP_Error('wpab_patch_empty_find', 'find must not be empty.', ['status' => 400]);
        }
        ...
        if (!isset($read['content']) || !is_string($read['content'])) {
            return new WP_Error('wpab_patch_read_invalid', 'Theme read response did not contain text content.', ['status' => 500]);
        }

        $before = $read['content'];
        if (strlen($before) > self::MAX_PATCH_FILE_BYTES) {
            return new WP_Error('wpab_patch_file_too_large', 'Theme file exceeds patch size limit.', ['status' => 413]);
        }
        if (isset($params['expected_sha256']) && is_string($params['expected_sha256']) && $params['expected_sha256'] !== '') {
            if (!hash_equals(strtolower($params['expected_sha256']), hash('sha256', $before))) {
                return new WP_Error('wpab_patch_sha_conflict', 'Theme file changed since it was inspected.', [
                    'status' => 409,
        ...
        if ($expected < 0 || $count !== $expected) {
            return new WP_Error('wpab_patch_match_conflict', 'Replacement count did not match expectation.', [
                'status' => 409,
    ...
    private static function media_upload_base64(array $params)
    {
        ...
        if (!isset($params['data_b64']) || !is_string($params['data_b64']) || trim($params['data_b64']) === '') {
            return new WP_Error('wpab_media_missing_base64', 'data_b64 is required.', ['status' => 400]);
        }
        $binary = base64_decode(trim($params['data_b64']), true);
        if (!is_string($binary)) {
            return new WP_Error('wpab_media_invalid_base64', 'data_b64 is not valid Base64.', ['status' => 400]);
        }
        if (strlen($binary) < 1 || strlen($binary) > self::MAX_MEDIA_BYTES) {
            return new WP_Error('wpab_media_size', 'Decoded media is empty or exceeds the size limit.', ['status' => 413]);
        }
        ...
        if ($safe_name === '' || strpos($safe_name, '.') === false) {
            return new WP_Error('wpab_media_filename', 'filename must include an allowed file extension.', ['status' => 400]);
        }
        ...
        if (!$tmp || file_put_contents($tmp, $binary, LOCK_EX) === false) {
            ...
            return new WP_Error('wpab_media_temp_write', 'Could not create temporary upload file.', ['status' => 500]);
        }
    ...
    private static function cron_schedule(array $params)
    {
        ...
        if ($timestamp < time() - 60) {
            return new WP_Error('wpab_cron_timestamp', 'timestamp must be now or in the future.', ['status' => 400]);
        }
        if (!has_action($hook)) {
            return new WP_Error('wpab_cron_unregistered_hook', 'No callback is currently registered for this hook.', ['status' => 409]);
        }
        ...
        if ($recurrence !== '' && !array_key_exists($recurrence, wp_get_schedules())) {
            return new WP_Error('wpab_cron_recurrence', 'Unknown cron recurrence.', ['status' => 400]);
        }
    ...
    private static function cron_run(array $params)
    {
        ...
        if (!$event) {
            return new WP_Error('wpab_cron_event_not_found', 'Matching scheduled event was not found.', ['status' => 404]);
        }
    ...
    private static function cron_unschedule(array $params)
    {
        ...
        $timestamp = isset($params['timestamp']) ? (int) $params['timestamp'] : 0;
        if ($timestamp < 1) {
            return new WP_Error('wpab_cron_timestamp', 'A valid timestamp is required.', ['status' => 400]);
        }
        $args = isset($params['args']) && is_array($params['args']) ? $params['args'] : [];
        $event = wp_get_scheduled_event($hook, $args, $timestamp);
        if (!$event) {
    ...
    private static function admin_run(array $params)
    {
        ...
        switch ($command) {
            ...
            default:
                return new WP_Error('wpab_admin_command_blocked', 'Unknown or non-allowlisted admin command.', [
                    'status' => 403,
                    'command' => $command,
                ]);
        }
    ...
    private static function legacy_action(string $action, array $params)
    {
        $secret = (string) get_option(self::OPTION_SECRET, '');
        if ($secret === '') {
            return new WP_Error('wpab_not_configured', 'Bridge secret is missing.', ['status' => 503]);
        }
        $route = '/wp-agent-bridge/v1/execute';
        $body = wp_json_encode(['action' => $action, 'params' => $params], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        if (!is_string($body)) {
            return new WP_Error('wpab_internal_encode', 'Could not encode internal bridge request.', ['status' => 500]);
        }
        ...
        if ($status >= 400) {
            ...
            return new WP_Error('wpab_legacy_action_failed', 'Internal legacy bridge action failed.', ['status' => $status, 'data' => $data]);
        }
    ...
    private static function check_upgrader_result($result, $skin, string $fallback)
    {
        ...
        if ($result === false) {
            ...
            return new WP_Error('wpab_upgrader_failed', $fallback, ['status' => 409]);
        }
        return true;
    }
    ...
    private static function required_plugin_file(array $params)
    {
        ...
        if (!isset($plugins[$file])) {
            return new WP_Error('wpab_plugin_not_found', 'Installed plugin file not found.', ['status' => 404]);
        }
        return $file;
    }
    ...
    private static function required_theme_stylesheet(array $params)
    {
        ...
        if (basename($stylesheet) !== $stylesheet) {
            return new WP_Error('wpab_theme_stylesheet_invalid', 'Invalid theme stylesheet.', ['status' => 400]);
        }
        $theme = wp_get_theme($stylesheet);
        if (!$theme->exists()) {
            return new WP_Error('wpab_theme_not_found', 'Installed theme not found.', ['status' => 404]);
        }
        return $stylesheet;
    }
    ...
    private static function required_slug(array $params, string $key)
    {
        ...
        if (!preg_match('/^[a-z0-9][a-z0-9._-]*$/', $slug)) {
            return new WP_Error('wpab_invalid_slug', 'Slug contains unsupported characters.', ['status' => 400]);
        }
        return $slug;
    }
    ...
    private static function required_hook(array $params)
    {
        ...
        if (strlen($hook) > 191 || preg_match('/[^A-Za-z0-9_.:\/-]/', $hook)) {
            return new WP_Error('wpab_invalid_hook', 'Invalid cron hook name.', ['status' => 400]);
        }
        return $hook;
    }
    ...
    private static function require_confirm(array $params, string $message)
    {
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_confirmation_required', $message, ['status' => 400]);
        }
        return true;
    }
}
