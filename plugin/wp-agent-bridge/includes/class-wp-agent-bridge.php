...
{
    private const REST_NAMESPACE = 'wp-agent-bridge/v1';
    private const VERSION = '0.3.0';
    private const OPTION_SECRET = 'wpab_secret';
    private const OPTION_USER_ID = 'wpab_user_id';
    private const OPTION_DRAFTS = 'wpab_draft_themes';
    private const OPTION_BACKUPS = 'wpab_theme_backups';
    private const TRANSIENT_SECRET_PREFIX = 'wpab_secret_once_';
    private const TRANSIENT_PREVIEW_PREFIX = 'wpab_preview_';
    ...
    public static function init(): void
    {
        ...
        add_action('admin_post_wpab_generate_key', [self::class, 'handle_generate_key']);
    ...
    {
        add_management_page(
            'WP Agent Bridge WordPress Bridge',
            'WP Agent Bridge WP Bridge',
            'manage_options',
            'wp-agent-bridge',
    ...
    public static function render_settings_page(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1>WP Agent Bridge WordPress Bridge</h1>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <input type="hidden" name="action" value="wpab_generate_key">
                <?php wp_nonce_field('wpab_generate_key'); ?>
    ...
    public static function handle_generate_key(): void
    {
        ...
        check_admin_referer('wpab_generate_key');
        ...
        wp_safe_redirect(admin_url('tools.php?page=wp-agent-bridge&generated=1'));
        exit;
    }
    ...
    public static function authorize_request(WP_REST_Request $request)
    {
        ...
        if ($secret === '' || $user_id < 1) {
            return new WP_Error(
                'wpab_not_configured',
                'WP Agent Bridge WordPress Bridge is not configured.',
                ['status' => 503]
            );
        }
        ...
        if ($timestamp === '' || $signature === '') {
            return new WP_Error('wpab_missing_signature', 'Missing bridge signature.', ['status' => 401]);
        }
        if (!ctype_digit($timestamp)) {
            return new WP_Error('wpab_invalid_timestamp', 'Invalid bridge timestamp.', ['status' => 401]);
        }
        if (abs(time() - (int) $timestamp) > self::MAX_CLOCK_SKEW) {
            return new WP_Error('wpab_expired_signature', 'Bridge signature has expired.', ['status' => 401]);
        }
        ...
        if (!hash_equals($expected, $signature)) {
            return new WP_Error('wpab_bad_signature', 'Invalid bridge signature.', ['status' => 401]);
        }

        wp_set_current_user($user_id);
        if (!current_user_can('manage_options')) {
            return new WP_Error('wpab_user_forbidden', 'Bridge user no longer has administrator capability.', ['status' => 403]);
        }
    ...
    private static function rest_call(array $params)
    {
        ...
        if (strpos($route, '/' . self::REST_NAMESPACE) === 0) {
            return new WP_Error('wpab_recursive_rest_call', 'Calling the bridge recursively is blocked.', ['status' => 400]);
        }

        $method = strtoupper(isset($params['method']) ? (string) $params['method'] : 'GET');
        if (!in_array($method, ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'], true)) {
            return new WP_Error('wpab_invalid_method', 'Unsupported REST method.', ['status' => 400]);
        }
    ...
    private static function option_get(array $params)
    {
        ...
        if (self::is_sensitive_option($name)) {
            return new WP_Error('wpab_sensitive_option', 'Reading this option is blocked.', ['status' => 403]);
        }

        $sentinel = new stdClass();
        $value = get_option($name, $sentinel);
        if ($value === $sentinel) {
            return new WP_Error('wpab_option_not_found', 'Option not found.', ['status' => 404]);
        }
    ...
    private static function option_update(array $params)
    {
        ...
        if (!array_key_exists('value', $params)) {
            return new WP_Error('wpab_missing_value', 'Missing value.', ['status' => 400]);
        }
        if (self::is_critical_option($name) || self::is_sensitive_option($name)) {
            return new WP_Error('wpab_blocked_option', 'Updating this option is blocked.', ['status' => 403]);
        }
    ...
    private static function post_meta_get(array $params)
    {
        ...
        if (!get_post($post_id)) {
            return new WP_Error('wpab_post_not_found', 'Post not found.', ['status' => 404]);
        }
    ...
    private static function post_meta_update(array $params)
    {
        ...
        if (!current_user_can('edit_post', $post_id)) {
            return new WP_Error('wpab_cannot_edit_post', 'You cannot edit this post.', ['status' => 403]);
        }
    ...
    private static function media_upload_from_url(array $params)
    {
        ...
        if (!wp_http_validate_url($url)) {
            return new WP_Error('wpab_invalid_url', 'Invalid media URL.', ['status' => 400]);
        }
    ...
    private static function theme_files_search(array $params)
    {
        ...
        if (strlen($query) > 300) {
            return new WP_Error('wpab_search_query_too_long', 'Search query is too long.', ['status' => 400]);
        }
    ...
    private static function theme_file_read(array $params)
    {
        ...
        if ($content === false) {
            return new WP_Error('wpab_file_read_failed', 'Could not read theme file.', ['status' => 500]);
        }
    ...
    private static function theme_file_lint(array $params)
    {
        ...
        if (array_key_exists('content', $params)) {
            if (!is_string($params['content'])) {
                return new WP_Error('wpab_invalid_content', 'Content must be a string.', ['status' => 400]);
            }
            ...
            return new WP_Error('wpab_missing_parameter', 'Provide path or content.', ['status' => 400]);
        }
    ...
    private static function theme_file_write(array $params)
    {
        ...
        if (strlen($content) > self::MAX_FILE_BYTES) {
            return new WP_Error('wpab_file_too_large', 'File exceeds the bridge size limit.', ['status' => 413]);
        }
        ...
        if (!is_dir($dir) && !wp_mkdir_p($dir)) {
            return new WP_Error('wpab_directory_create_failed', 'Could not create target directory.', ['status' => 500]);
        }

        $before = is_file($target) ? @file_get_contents($target) : null;
        $tmp = $target . '.wp-agent-bridge-tmp-' . wp_generate_password(8, false, false);
        if (@file_put_contents($tmp, $content, LOCK_EX) === false) {
            return new WP_Error('wpab_file_write_failed', 'Could not write temporary file.', ['status' => 500]);
        }
        if (!@rename($tmp, $target)) {
            @unlink($tmp);
            return new WP_Error('wpab_file_replace_failed', 'Could not replace target file.', ['status' => 500]);
        }
    ...
    private static function theme_file_delete(array $params)
    {
        ...
        if (!@unlink($target)) {
            return new WP_Error('wpab_file_delete_failed', 'Could not delete theme file.', ['status' => 500]);
        }
    ...
    private static function draft_theme_create(array $params)
    {
        ...
        if ($source === false || !is_dir($source)) {
            return new WP_Error('wpab_theme_root_missing', 'Active stylesheet directory was not found.', ['status' => 500]);
        }

        $id = 'draft-' . gmdate('Ymd-His') . '-' . strtolower(wp_generate_password(6, false, false));
        $slug = 'wp-agent-bridge-' . $id;
        $theme_root = realpath(get_theme_root());
        if ($theme_root === false) {
            return new WP_Error('wpab_theme_root_missing', 'Theme root was not found.', ['status' => 500]);
        }

        $target = $theme_root . DIRECTORY_SEPARATOR . $slug;
        if (file_exists($target)) {
            return new WP_Error('wpab_draft_exists', 'Draft directory already exists.', ['status' => 409]);
        }
    ...
    private static function draft_theme_preview_url(array $params)
    {
        ...
        if ($path === '' || $path[0] !== '/' || strpos($path, '://') !== false) {
            return new WP_Error('wpab_invalid_preview_path', 'Preview path must be a local path beginning with /.', ['status' => 400]);
        }

        $url = add_query_arg('wpab_theme_preview', rawurlencode($token), home_url($path));
    ...
    private static function draft_theme_publish(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_publish_confirmation_required', 'Set confirm=true to publish a draft theme.', ['status' => 400]);
        }
        ...
        $staging = $theme_root . DIRECTORY_SEPARATOR . $active_slug . '-wp-agent-bridge-staging-' . strtolower(wp_generate_password(6, false, false));
        $old = $theme_root . DIRECTORY_SEPARATOR . $active_slug . '-wp-agent-bridge-old-' . strtolower(wp_generate_password(6, false, false));
        ...
        if (!@rename($active_root, $old)) {
            self::remove_tree($staging);
            return new WP_Error('wpab_theme_swap_failed', 'Could not move the active theme aside.', ['status' => 500]);
        }

        if (!@rename($staging, $active_root)) {
            @rename($old, $active_root);
            self::remove_tree($staging);
            return new WP_Error('wpab_theme_swap_failed', 'Could not activate the staged theme files. Original files were restored.', ['status' => 500]);
        }
    ...
    private static function draft_theme_discard(array $params)
    {
        ...
        if (!$removed && is_dir($root)) {
            return new WP_Error('wpab_draft_delete_failed', 'Could not remove draft theme directory.', ['status' => 500]);
        }
    ...
    private static function theme_rollback(array $params)
    {
        ...
        if (empty($params['confirm'])) {
            return new WP_Error('wpab_rollback_confirmation_required', 'Set confirm=true to rollback.', ['status' => 400]);
        }

        $backups = self::get_backups();
        if (!isset($backups[$backup_id])) {
            return new WP_Error('wpab_backup_not_found', 'Theme backup not found.', ['status' => 404]);
        }
        $backup = $backups[$backup_id];
        $backup_root = isset($backup['path']) ? (string) $backup['path'] : '';
        if ($backup_root === '' || !is_dir($backup_root)) {
            return new WP_Error('wpab_backup_missing', 'Theme backup files are missing.', ['status' => 410]);
        }
        ...
        $active_slug = basename($active_root);
        $staging = $theme_root . DIRECTORY_SEPARATOR . $active_slug . '-wp-agent-bridge-rollback-' . strtolower(wp_generate_password(6, false, false));
        $old = $theme_root . DIRECTORY_SEPARATOR . $active_slug . '-wp-agent-bridge-old-' . strtolower(wp_generate_password(6, false, false));
        ...
        if (!@rename($staging, $active_root)) {
            @rename($old, $active_root);
            self::remove_tree($staging);
            return new WP_Error('wpab_theme_swap_failed', 'Rollback swap failed. Original files were restored.', ['status' => 500]);
        }
    ...
    private static function db_select(array $params)
    {
        ...
        if (strlen($sql) > 20000) {
            return new WP_Error('wpab_sql_too_long', 'SQL query is too long.', ['status' => 400]);
        }

        $trimmed = trim($sql);
        $trimmed = rtrim($trimmed, " \t\n\r\0\x0B;");
        if (!preg_match('/^SELECT\b/i', $trimmed)) {
            return new WP_Error('wpab_select_only', 'Only SELECT statements are allowed.', ['status' => 403]);
        }
        if (strpos($trimmed, ';') !== false) {
            return new WP_Error('wpab_multiple_statements', 'Multiple SQL statements are not allowed.', ['status' => 403]);
        }
        ...
        foreach ($blocked_fragments as $fragment) {
            if (strpos($upper, $fragment) !== false) {
                return new WP_Error('wpab_sql_blocked', 'SQL contains a blocked operation.', ['status' => 403]);
            }
        }
        ...
        if ($wpdb->last_error) {
            return new WP_Error('wpab_db_error', $wpdb->last_error, ['status' => 400]);
        }
        if (!is_array($rows)) {
            $rows = [];
        }
    ...
    private static function abilities_list(array $params)
    {
        if (!function_exists('wp_get_abilities')) {
            return new WP_Error('wpab_abilities_unavailable', 'WordPress Abilities API is unavailable.', ['status' => 501]);
        }
    ...
    private static function abilities_get(array $params)
    {
        ...
        if (!$ability) {
            return new WP_Error('wpab_ability_not_found', 'Ability not found.', ['status' => 404]);
        }
    ...
    private static function abilities_run(array $params)
    {
        ...
        if (!empty($annotations['destructive']) && empty($params['confirm_destructive'])) {
            return new WP_Error(
                'wpab_destructive_confirmation_required',
    ...
    {
        if (empty($_GET['wpab_theme_preview']) || !is_string($_GET['wpab_theme_preview'])) {
            return null;
        }
        $token = sanitize_text_field(wp_unslash($_GET['wpab_theme_preview']));
        if ($token === '') {
            return null;
        }
    ...
    private static function theme_context(array $params, bool $write)
    {
        ...
        if ($draft_id !== '') {
            ...
            if (!isset($drafts[$draft_id])) {
                return new WP_Error('wpab_draft_not_found', 'Draft theme not found.', ['status' => 404]);
            }
            $draft = $drafts[$draft_id];
            $root = self::draft_root($draft);
            if (!is_dir($root)) {
                return new WP_Error('wpab_draft_missing', 'Draft theme directory is missing.', ['status' => 410]);
            }
            return [
                'scope' => 'draft',
                'draft_id' => $draft_id,
        ...
        if ($write && empty($params['confirm_active'])) {
            return new WP_Error(
                'wpab_active_write_confirmation_required',
        ...
        if ($root === false) {
            return new WP_Error('wpab_theme_root_missing', 'Active theme directory not found.', ['status' => 500]);
        }
    ...
    {
        if (!is_dir($root)) {
            return new WP_Error('wpab_theme_root_missing', 'Theme directory not found.', ['status' => 500]);
        }
    ...
    private static function resolve_theme_file(string $root, string $relative, bool $must_exist)
    {
        if ($relative === '' || strpos($relative, "\0") !== false) {
            return new WP_Error('wpab_invalid_path', 'Invalid path.', ['status' => 400]);
        }
        $relative = str_replace('\\', '/', $relative);
        if ($relative[0] === '/' || preg_match('~(^|/)\.\.(/|$)~', $relative)) {
            return new WP_Error('wpab_path_escape', 'Path escapes the theme directory.', ['status' => 403]);
        }

        $extension = strtolower(pathinfo($relative, PATHINFO_EXTENSION));
        if (!self::is_allowed_theme_extension($extension)) {
            return new WP_Error('wpab_file_type_blocked', 'File type is not supported by theme editing.', ['status' => 403]);
        }
        ...
        if ($must_exist) {
            ...
            if ($real === false || !is_file($real)) {
                return new WP_Error('wpab_file_not_found', 'Theme file not found.', ['status' => 404]);
            }
    ...
    private static function lint_theme_php(string $root)
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (!is_string($content)) {
                return new WP_Error('wpab_file_read_failed', 'Could not read PHP file for syntax validation.', [
                    'status' => 500,
    ...
    private static function create_theme_backup(string $reason)
    {
        ...
        if (!empty($uploads['error'])) {
            return new WP_Error('wpab_upload_dir_error', (string) $uploads['error'], ['status' => 500]);
        }

        $id = 'backup-' . gmdate('Ymd-His') . '-' . strtolower(wp_generate_password(6, false, false));
        $base = trailingslashit($uploads['basedir']) . 'wp-agent-bridge/backups/' . $id;
        if (!wp_mkdir_p($base)) {
            return new WP_Error('wpab_backup_dir_failed', 'Could not create backup directory.', ['status' => 500]);
        }
    ...
    {
        if (!is_dir($source)) {
            return new WP_Error('wpab_copy_source_missing', 'Copy source directory does not exist.', ['status' => 500]);
        }
        if (!wp_mkdir_p($target)) {
            return new WP_Error('wpab_copy_target_failed', 'Could not create destination directory.', ['status' => 500]);
        }
        ...
        foreach ($iterator as $item) {
            ...
            if ($item->isDir()) {
                if (!is_dir($destination) && !wp_mkdir_p($destination)) {
                    return new WP_Error('wpab_copy_failed', 'Could not create directory while copying theme.', [
                        'status' => 500,
                        'path' => $relative,
                    ]);
                }
            ...
            if (!@copy($item->getPathname(), $destination)) {
                return new WP_Error('wpab_copy_failed', 'Could not copy theme file.', [
                    'status' => 500,
                    'path' => $relative,
                ]);
            }
    ...
    private static function required_string(array $params, string $key)
    {
        if (!isset($params[$key]) || !is_scalar($params[$key]) || trim((string) $params[$key]) === '') {
            return new WP_Error('wpab_missing_parameter', 'Missing required parameter: ' . $key, ['status' => 400]);
        }
    ...
    private static function required_int(array $params, string $key)
    {
        if (!isset($params[$key]) || !is_numeric($params[$key])) {
            return new WP_Error('wpab_missing_parameter', 'Missing required integer parameter: ' . $key, ['status' => 400]);
        }
        $value = absint($params[$key]);
        if ($value < 1) {
            return new WP_Error('wpab_invalid_parameter', 'Invalid integer parameter: ' . $key, ['status' => 400]);
        }
        return $value;
    }
