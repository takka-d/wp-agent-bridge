...
function get_option($name, $default = null) { return $name === 'takka_bridge_secret' ? str_repeat('a', 64) : $default; }
...
function rest_do_request($request) {
    ...
    $data = invoke_private(TakKa_WordPress_Bridge_V096_Theme_Files::class, 'search_files', $params)->get_data();
    return new WP_REST_Response(['status' => 200, 'data' => $data]);
}
$includes = ($argv[1] ?? __DIR__ . '/../plugin/wp-agent-bridge/includes') . '/';
require_once $includes . 'class-takka-wordpress-bridge-v095-outline.php';
require_once $includes . 'class-takka-wordpress-bridge-v096-theme-files.php';
require_once $includes . 'class-takka-wordpress-bridge-v098-read-batch.php';
require_once $includes . 'class-takka-wordpress-bridge-direct-runtime.php';
...
try {
    ...
    $range = fn($params) => TakKa_WordPress_Bridge_V095_Outline::read_range(['path' => 'range.txt'] + $params);
    ...
    check(is_wp_error(TakKa_WordPress_Bridge_V095_Outline::read_range(['path' => '../outside.txt'])), 'Traversal accepted');
    $many = invoke_private(TakKa_WordPress_Bridge_V096_Theme_Files::class, 'read_many', ['files' => [['path' => 'range.txt', 'start_line' => 120, 'max_lines' => 110]]])->get_data();
    ...
    $search = fn($params) => invoke_private(TakKa_WordPress_Bridge_V096_Theme_Files::class, 'search_files', $params)->get_data();
    ...
    $batch = fn($operations) => invoke_private(TakKa_WordPress_Bridge_V098_Read_Batch::class, 'readonly_batch', ['operations' => $operations]);
    ...
    foreach ([200, 400] as $status) {
        ...
        $outcome = invoke_private(TakKa_WordPress_Bridge_Direct_Runtime::class, 'summarize_outcome', $wrapped);
