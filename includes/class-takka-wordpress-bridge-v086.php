...
final class TakKa_WordPress_Bridge_V086
{
    private const VERSION = '0.8.6.1';
    private const INTERNAL_NAMESPACE = 'takka-v086/v1';
    private const INTERNAL_ROUTE = '/takka-v086/v1/manage';
    ...
    public static function internal_permission()
    {
        if (!self::$internal_allowed || !current_user_can('manage_options')) {
            return new WP_Error('takka_bridge_v086_internal_only', 'This route is only callable through the signed Bridge REST proxy.', ['status' => 403]);
        }
    ...
    public static function dispatch(WP_REST_Request $request)
    {
        ...
        if (!is_array($json)) return new WP_Error('takka_bridge_v086_json', 'JSON body is required.', ['status' => 400]);
        ...
        if (!in_array($action, self::ACTIONS, true)) {
            return new WP_Error('takka_bridge_v086_unknown_action', 'Unknown or blocked v0.8.6 action.', ['status' => 400, 'action' => $action]);
        }
        ...
        try {
            switch ($action) {
                ...
                case 'post.table.inspect':
                    $response = TakKa_WordPress_Bridge_V086_Table::inspect($params);
                    break;
                case 'post.table.patch.preview':
                    $response = TakKa_WordPress_Bridge_V086_Table::preview($params);
                    break;
                case 'post.table.patch.apply':
                    $response = TakKa_WordPress_Bridge_V086_Table::apply($params);
                    TakKa_WordPress_Bridge_V07_Audit::record(self::$request_id, $action, self::audit_params($params), $response);
                    break;
                default:
                    $response = new WP_Error('takka_bridge_v086_dispatch', 'Dispatch fell through.', ['status' => 500]);
            }
        } catch (Throwable $e) {
            $response = new WP_Error('takka_bridge_v086_exception', $e->getMessage(), ['status' => 500, 'type' => get_class($e)]);
        }
        return $response;
    }
